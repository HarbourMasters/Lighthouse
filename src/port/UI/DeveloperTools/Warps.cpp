...
constexpr int32_t warpCount = (int32_t)(sizeof(warpsInfo) / sizeof(warpsInfo[0]));

constexpr int32_t LevelBucket(int32_t level) {
    return (level > 0 && level < levelCount) ? level : 0;
}

constexpr std::array<LevelSpan, levelCount> BuildLevelSpans() {
    std::array<LevelSpan, levelCount> spans{};
    for (int32_t i = 0; i < warpCount; i++) {
        LevelSpan& span = spans[LevelBucket(warpsInfo[i].level)];
        if (span.count == 0) {
            span.first = i;
        }
        span.count++;
...
constexpr std::array<LevelSpan, levelCount> levelSpans = BuildLevelSpans();

// The spans only work while each level's rows stay together.
constexpr bool LevelSpansAreContiguous() {
    for (int32_t level = 0; level < levelCount; level++) {
        for (int32_t i = 0; i < levelSpans[level].count; i++) {
            if (LevelBucket(warpsInfo[levelSpans[level].first + i].level) != level) {
                return false;
            }
        }
    }
...
static_assert(LevelSpansAreContiguous(), "warpsInfo rows must stay grouped by level");
...
void DrawDestinationList(int32_t currentMap) {
    ...
    if (ImGui::BeginChild("WarpDestinations")) {
        ...
        for (int32_t level = 0; level < levelCount; level++) {
            const LevelSpan& span = levelSpans[level];
            if (span.count == 0) {
                continue;
            }
            ImGui::PushID(level);
            if (ImGui::CollapsingHeader(fmt::format("{} ({})", LevelDisplayName(level), span.count).c_str())) {
                ...
                if (ImGui::BeginTable("LevelWarps", 2, ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_RowBg)) {
                    ...
                    for (int32_t i = 0; i < span.count; i++) {
                        const WarpEntry& entry = warpsInfo[span.first + i];
                        ImGui::PushID(i);
