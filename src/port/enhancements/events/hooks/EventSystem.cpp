...
EventID EventSystem::RegisterEvent(const char *name) {
  ...
  this->mEventRegistry[id] =
      EventRegistration{.name = name == nullptr ? "Unknown" : name};
  return id;
}
...
ListenerID EventSystem::RegisterListener(EventID id, EventCallback callback,
  ...
  if (id == -1) {
    throw std::runtime_error(
        "Trying to register listener for unregistered event");
  }
...
void EventSystem::CallEvent(const EventID id, IEvent *event, const char *file,
...
extern "C" void EventSystem_CallEvent(EventID id, void *event, const char *file,
                                      int line, const char *key) {
  EventSystem::Instance->CallEvent(id, static_cast<IEvent *>(event), file, line,
                                   key);
}
