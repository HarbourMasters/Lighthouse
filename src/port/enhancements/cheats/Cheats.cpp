...
void RegisterInfiniteLives_Init() {
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_INFINITE_LIVES, 0),
              [](IEvent* event) {
...
void RegisterInfiniteEggs_Init() {
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_INFINITE_EGGS, 0),
              [](IEvent* event) {
                  // Only refill during actual gameplay (not in file select or demo mode)
                  if (gsworld_getMap() != MAP_91_FILE_SELECT && getGameMode() != GAME_MODE_7_ATTRACT_DEMO) {
                      s32 currentCount = item_getCount(ITEM_D_EGGS);
                      s32 maxCount = port_getRomhackMaxEggs();
                      if (maxCount < 0) maxCount = 100; // Default when romhack doesn't override
...
void RegisterInfiniteRedFeathers_Init() {
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_INFINITE_RED_FEATHERS, 0),
              [](IEvent* event) {
                  // Only refill during actual gameplay (not in file select or demo mode)
                  if (gsworld_getMap() != MAP_91_FILE_SELECT && getGameMode() != GAME_MODE_7_ATTRACT_DEMO) {
                      s32 currentCount = item_getCount(ITEM_F_RED_FEATHER);
                      s32 maxCount = port_getRomhackMaxRedFeathers();
                      if (maxCount < 0) maxCount = 50; // Default when romhack doesn't override
                      if (currentCount < maxCount) {
    ...
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_INFINITE_GOLD_FEATHERS, 0),
              [](IEvent* event) {
                  ...
                  if (gsworld_getMap() != MAP_91_FILE_SELECT && getGameMode() != GAME_MODE_7_ATTRACT_DEMO) {
                      ...
                      if (maxCount < 0) maxCount = 10; // Default when romhack doesn't override
                      if (currentCount < maxCount) {
...
void RegisterInfiniteTimers_Init() {
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_INFINITE_TIMERS, 0),
              [](IEvent* event) {
...
void RegisterTalonTrotCycle_Init() {
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_TALON_TROT_CYCLE, 0),
              [](IEvent* event) {
                  ...
                  if (bakey_pressed(BUTTON_D_RIGHT) || bakey_pressed(BUTTON_D_LEFT)) {
                      ...
                      bool inTalonTrot = bsbtrot_inSet((enum bs_e)currentState) ||
                                         bslongleg_inSet(0); // Also in longleg state
...
void RegisterLevitate_Init() {
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_LEVITATE, 0),
              [](IEvent* event) {
...
void RegisterFreezeTimers_Init() {
    ...
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_FREEZE_TIMERS, 0),
              [](IEvent* event) {
...
void RegisterCycleTransform_Init() {
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_CYCLE_TRANSFORM, 0),
              [](IEvent* event) {
...
void RegisterFastTransform_Init() {
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_FAST_TRANSFORM, 0),
              [](IEvent* event) {
...
void RegisterNoMumboUntransform_Init() {
    COND_HOOK(GameFrameUpdate, EVENT_PRIORITY_NORMAL, CVarGetInteger(CVAR_NO_MUMBO_UNTRANSFORM, 0),
              [](IEvent* event) {
...
static RegisterShipInitFunc initInfiniteRedFeathersFunc(RegisterInfiniteRedFeathers_Init, { CVAR_INFINITE_RED_FEATHERS });
static RegisterShipInitFunc initInfiniteGoldFeathersFunc(RegisterInfiniteGoldFeathers_Init, { CVAR_INFINITE_GOLD_FEATHERS });
