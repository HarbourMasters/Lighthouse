 ...
 * JIGGY_SPAWN
 *
 * Sync of dynamically spawned jiggies.A jiggy spawned by jiggy_spawn fires OnJiggySpawned -> SendPacket_SpawnJiggy; the spawn is
...
struct SpawnedJiggy {
    ...
    float y;
    float z;
    // Prevent bundle-respawn while waiting for spawned actor to set marker
    bool inFlight = false;
};
...
// Re-apply the team's spawned jiggies for the map we're in
...
// Authoritative team-state snapshot/restore
...
// Drop a spawned-jiggy record (all maps) — for a timed jiggy whose hourglass ran out
