...
static void patchModelDL(BKModelBin *model_bin,
                         uintptr_t seg_start, uintptr_t seg_end,
                         const void *tex_addr, s32 tex_w, s32 tex_h,
                         void (*set_uv)(Vtx *v),
    ...
    while (cur_gfx < end_gfx) {
        if ((cur_gfx->words.w0 >> 24) == G_SETTIMG) {
            s32 match = ((cur_gfx->words.w1 & ~(uintptr_t)1) >= seg_start &&
                         (cur_gfx->words.w1 & ~(uintptr_t)1) < seg_end);
            if (uv_only_in_tile) in_tile = match;
            if (match) {
                Gfx *g = cur_gfx;
                Gfx *ng = cur_gfx + 1;
                u8 fmt = (ng->words.w0 >> 21) & 0x7;
                gDPLoadTextureTile(g++, tex_addr, fmt, G_IM_SIZ_16b,
                    tex_w, tex_h, 0, 0, tex_w - 1, tex_h - 1,
...
void port_patchPictureModel(BKModelBin *model_bin, s32 min_xy, s32 max_xy, s32 min_z, s32 max_z, u32 from) {
    if (model_bin->pad0[0] == 0xBA) return;
    model_bin->pad0[0] = 0xBA;
    sPicFrom = from; sPicMinXY = min_xy; sPicMaxXY = max_xy; sPicMinZ = min_z; sPicMaxZ = max_z;
    patchModelDL(model_bin, 0x04000000, 0x04100000, (const void *)SEG4_TAGGED,
                 IMAGE_WIDTH, IMAGE_HEIGHT, setPictureVertexTexcoord, G_TF_BILERP, 1);
}
...
s32 port_getTransitionGpuFbId(void) {
    if (sTransitionGpuFbId < 0) {
        sTransitionGpuFbId = gfx_create_framebuffer(
...
void port_patchTransitionModel(BKModelBin *model_bin) {
    ...
    if (sTransitionXCompensate < 1.01f) sTransitionXCompensate = 1.0f;
    patchModelDL(model_bin, 0x02000000, 0x02028000, sTransitionFbDummy,
                 DEFAULT_FRAMEBUFFER_WIDTH, DEFAULT_FRAMEBUFFER_HEIGHT,
                 setTransitionVertexTexcoord, G_TF_POINT, 0);
}
