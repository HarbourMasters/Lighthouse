...
/* .data */
ActorInfo D_8038F190 = {0x1B6, 0x2A3,         0x487, 0x0, NULL, func_8038AA8C,
                        NULL,  func_8038AA38, 0,     0,   0.0f, 0};

Struct_CCW_4530_0 D_8038F1B4[] = {{0.15f, 0.9f, SFX_A5_EYRIE_CRY, 25000},
...
void func_8038A950(Actor *this, s32 next_state) {
  ...
  if (next_state == 2) {
    gcdialog_showText(0xCDC, 0xA, this->position, this->marker, func_8038A920,
                      NULL);
  }
  if (next_state == 3) {
...
void func_8038AA8C(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    if ((!fileProgressFlag_get(FILEPROG_E8_AUTMN_EYRIE_FED) ||
  ...
  if (this->state == 1) {
    ...
    if ((sp50 < 0.21) && (0.21 <= sp4C)) {
      func_8030E878(0xA5, randf2(0.9f, 1.1f), 32000, this->position, 500.0f,
                    2500.0f);
    }
  ...
  if (this->state == 3) {
    ...
    for (iPtr = &D_8038F1B4[0]; iPtr->unk0 > 0.0f; iPtr++) {
      if ((sp44 < iPtr->unk0) && (iPtr->unk0 <= sp40)) {
        func_8030E878(iPtr->unk8, iPtr->unk4, iPtr->unkA, this->position,
                      500.0f, 2500.0f);
      }
    }
