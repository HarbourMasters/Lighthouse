...
enum ccw_season_e { SPRING, SUMMER, AUTUMN, WINTER };
...
ActorAnimationInfo chGrublinHood_animations[] = {
    {0x000, 0.0f}, {0x243, 4.0f},   {0x243, 0.7f},  {0x245, 0.7f},
    {0x244, 0.9f}, {0x246, 1.6f},   {0x243, 1.5f},  {0x245, 0.5f},
    {0x243, 1.5f}, {0x243, 1e+06f}, {0x243, 1e+06f}};

ActorInfo chGrublinHood = {MARKER_1E2_GRUBLIN_HOOD,
                           ACTOR_375_GRUBLIN_HOOD,
                           ASSET_52C_MODEL_GRUBLIN_HOOD,
                           0x1,
                           chGrublinHood_animations,
                           chgrublinhood_update,
...
/* .code */
void __chgrublinhood_emitHat(ParticleEmitter *pCtrl, Actor *actor,
...
void __chgrublinhood_die(ActorMarker *marker, s32 arg1) {
  ...
  spawnQueue_add_4((GenFunction_4)spawnQueue_actor_f32, ACTOR_4C_STEAM,
                     reinterpret_cast(s32, actor->position_x),
                     reinterpret_cast(s32, actor->position_y),
                     reinterpret_cast(s32, actor->position_z));
  actor_collisionOff(actor);
  actor->has_met_before = TRUE;
}

Actor *chgrublinhood_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx,
                          Vtx **vtx) {
  Actor *this;
  ...
  func_8033A45C(10, (local->season < AUTUMN)    ? 0
                    : (local->season == AUTUMN) ? 1
                                                : 2);
  func_8033A45C(11, (local->season < AUTUMN)    ? 0
                    : (local->season == AUTUMN) ? 1
                                                : 2);
  ...
  if (local->season < 4) {
    ...
    if (this->state == 5) {
      if (actor_animationIsAt(this, 0.18f)) {
        FUNC_8030E8B4(SFX_2_CLAW_SWIPE, 1.0f, 28000, this->position, 1250,
                      2500);
      }
      if (actor_animationIsAt(this, 0.7f)) {
        FUNC_8030E8B4(SFX_1F_HITTING_AN_ENEMY_3, 1.0f, 28000, this->position,
                      1250, 2500);
      }
    }
  }
