...
#include "bk_math.h" // for SQ()

void chVacationTextTrigger_update(Actor *this);

/* .data */

ActorInfo gChVacationTextTrigger = {MARKER_1E3_VACATION_TEXT_TRIGGER,
                                    ACTOR_374_VACATION_TEXT_TRIGGER,
                                    0x0,
                                    0x0,
                                    NULL,
                                    chVacationTextTrigger_update,
...
static bool __playerIsWithinTrigger(s32 x, s32 z, s32 distance) {
  ...
  return (SQ(x - player_position[0]) + SQ(z - player_position[2])) <
         SQ(distance);
}
