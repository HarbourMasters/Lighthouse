...
/* .data */
ActorInfo D_8038EBD0 = {MARKER_1AE_ZUBBA,
                        ACTOR_29B_ZUBBA,
...
void chwasp_setState(Actor *this, s32 next_state) {
  ...
  if (next_state == 2) {
    ...
    if ((local->unk8[0] == 0.0f) && (local->unk8[1] == 0.0f) &&
        (local->unk8[2] == 0.0f)) {
      local->unk8[1] = -1.0f;
    }
  }
  if (next_state == 3) {
    FUNC_8030E8B4(SFX_1F_HITTING_AN_ENEMY_3, 1.2f, 32200, this->position, 500,
                  3000);
    playSoundEffectWithPositionAtTime(randf2(0.1f, 0.4f), 0x3FC, randf2(0.95f, 1.1f), 32000,
  ...
  if (next_state == 4) {
    ...
    FUNC_8030E8B4(SFX_1F_HITTING_AN_ENEMY_3, 1.2f, 32200, this->position, 500,
                  3000);
    playSoundEffectWithPositionAtTime(0.1f, 0x66, randf2(1.6f, 1.7f), 32000, this->position, 500.0f,
                  3000.0f);
  ...
  if (next_state == 5) {
    func_8030E878(SFX_A_BANJO_LANDING_05, randf2(0.85f, 0.95f), 18000,
                  this->position, 500.0f, 3000.0f);
...
void chwasp_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    marker_setCollisionScripts(this->marker, func_80387124, NULL,
                               CCW_func_80387150);
  ...
  if (this->state == 1) {
    ...
    if ((temp_f0 < 0.0f) && (this->position[1] >= 0.0f)) {
      func_8030E878(SFX_A_BANJO_LANDING_05, randf2(1.0f, 1.1f), 18000,
                    this->position, 500.0f, 3000.0f);
    }
    player_getPosition(sp50);
    ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f, sp50, &sp4C,
                                &sp48, &sp44);
  ...
  sfxsource_setSampleRate(local->sfxsourceIdx,
                          (s32)((local->unk18 / local->unk14) * 10000.0f));
}
