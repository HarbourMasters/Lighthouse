...
ActorInfo D_8038F35C = {MARKER_1BB_NABNUT,
                        ACTOR_2A8_NABNUT,
...
void chnabnut_setState(Actor *this, s32 next_state) {
  ...
  if (next_state == NABNUT_STATE_2_WAIT) {
    timedFunc_set_2(1.0f, (GenFunction_2)__chnabnut_setState_method,
                    (s32)this->marker, NABNUT_STATE_3_BACKFLIP);
  }
...
void chnabnut_update(Actor *this) {
  ...
  if (this->state == NABNUT_STATE_1_SAD) {
    ...
    if (!this->has_met_before &&
        (ml_vec3f_distance(this->position, sp30) < 400.0f)) {
      this->has_met_before = TRUE;
      gcdialog_showText(0xCCA, 0xE, this->position, NULL, NULL, NULL);
    }
    if (item_getCount(ITEM_23_ACORNS) > 0) {
      ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f, sp30,
                                  &sp2C, &sp28, &sp24);
      this->yaw += sp24 * 10.0f;
    }
    if (this->has_met_before && !func_803114B0()) {
      player_setCarryObjectPoseInCylinder(this->position, 500.0f, 200.0f,
                                          ACTOR_2A9_ACORN, &this);
      if ((carriedObj_getActorId() == ACTOR_2A9_ACORN) &&
          (ml_vec3f_distance(this->position, sp30) < 300.0f) &&
