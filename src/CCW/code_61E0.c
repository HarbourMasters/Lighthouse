...
/* .data */
ActorInfo D_8038F460 = {0x1BC, 0x2A9,      0x48E, 0x0, NULL, func_8038C7A8,
                        NULL,  actor_draw, 0,     0,   0.8f, 0};

/* .code */
void CCW_func_8038C5D0(ActorMarker *marker) {
  Actor *actor = marker_getActor(marker);
  func_8030E878(SFX_3F2_UNKNOWN, randf2(0.95f, 1.05f), 26000, actor->position,
                500.0f, 1000.0f);
}

void CCW_func_8038C638(ActorMarker *marker) {
  Actor *actor = marker_getActor(marker);
  func_8030E878(SFX_5_BANJO_LANDING_01, randf2(0.95f, 1.05f), 22000,
                actor->position, 500.0f, 1000.0f);
}
...
void CCW_func_8038C6A0(Actor *this, s32 next_state) {
  ...
  if (next_state == 1) {
    ...
    skeletalAnim_setCallback_1(this->unk148, 0.5f,
                               (GenFunction_1)CCW_func_8038C5D0,
                               (s32)this->marker);
    skeletalAnim_setCallback_1(this->unk148, 0.7f,
                               (GenFunction_1)CCW_func_8038C638,
                               (s32)this->marker);
  }
  if (next_state == 3) {
...
void func_8038C7A8(Actor *this) {
  ...
  if (this->state == 3) {
    ...
    ml_vec3f_interpolate_fast(this->position, local->unk0, local->unkC,
                              local->unk18);
