...
/* .data */
ActorInfo D_8038EBA0 = {0x1AD, 0x299,      0x443, 0x0, NULL, func_8038687C,
...
void CCW_func_80386550(ActorMarker *marker) {
  ...
  comusic_playTrackWithVolumeOverride(COMUSIC_3D_JIGGY_SPAWN, 28000);
}
...
void func_803865F4(Actor *this, s32 next_state) {
  ...
  if (next_state == 3) {
    ...
    comusic_playTrackWithVolumeOverride(COMUSIC_4B_CCW_ZUBBA_FIGHT, 30000);
  }
  ...
  if (next_state == 4) {
    if (volatileFlag_get(VOLATILE_FLAG_2_FF_IN_MINIGAME)) {
      ...
      comusic_fadeTrackWithArgsNoDelay(COMUSIC_4B_CCW_ZUBBA_FIGHT, 0, 400);
      comusic_stopTrackById(COMUSIC_4B_CCW_ZUBBA_FIGHT);
      ...
      timedFunc_set_1(2.0f, (GenFunction_1)CCW_func_80386550,
                      (s32)this->marker);
...
void func_8038687C(Actor *this) {
  ...
  if (ml_timer_update(&local->unkC, sp88)) {
    if ((local->unk4 < 3) && ((local->unk8 + local->unk4) < 10)) {
      ...
      spawnQueue_add_4((GenFunction_4)spawnQueue_actor_f32, 0x29B,
                         reinterpret_cast(s32, sp4C[0]),
                         reinterpret_cast(s32, sp4C[1]),
                         reinterpret_cast(s32, sp4C[2]));
      local->unk4++;
    }
    local->unkC = randf2(0.5 - ((local->unk8 / 10) * 0.4),
                         1.0 - ((local->unk8 / 10) * 0.8));
  }
  if (this->state == 1) {
    if (volatileFlag_get(VOLATILE_FLAG_2_FF_IN_MINIGAME) &&
      ...
      func_803865F4(this, 3);
    } else if ((local->unk0 != NULL) &&
               (map_get() == MAP_5A_CCW_SUMMER_ZUBBA_HIVE)) {
      player_getPosition(sp40);
      if ((ml_vec3f_distance(this->position, sp40) < 300.0f) &&
          (player_getTransformation() == TRANSFORM_1_BANJO)) {
        func_803865F4(this, 2);
      }
    }
  }
  if ((this->state == 3) && volatileFlag_get(VOLATILE_FLAG_2_FF_IN_MINIGAME) &&
