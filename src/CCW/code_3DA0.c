...
/* .data */
ActorInfo chCaterpillar = {MARKER_1B5_CATERPILLAR,
                           ACTOR_2A2_CATERPILLAR,
                           ASSET_485_MODEL_CATERPILLAR,
                           0x0,
                           NULL,
                           chcaterpillar_update,
                           NULL,
                           chcaterpillar_draw,
                           0,
                           0,
...
Actor *chcaterpillar_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx,
                          Vtx **vtx) {
...
void chcaterpillar_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    if (sp8C) {
      ...
      if (!fileProgressFlag_get(FILEPROG_E6_SPRING_EYRIE_HATCHED) ||
          (map_id == MAP_45_CCW_AUTUMN &&
           !fileProgressFlag_get(FILEPROG_E7_SUMMER_EYRIE_FED))) {
  ...
  if (this->state == 1) {
    ...
    if (ml_vec3f_distance(this->position, local->unk0) < 10.0f) {
      for (i = 0; i < 10; i++) {
        ...
        if (!(ml_vec3f_distance(local->unk0, this->position) < 50.0f) &&
            func_80329210(this, local->unk0))
          break;
      } // L8038A544
      if (i == 10) {
        local->unk0[0] = this->position[0];
    ...
    if (sp64 <= 0.1 && 0.1 < sp60) {
      func_8030E878(SFX_AD_CATERPILLAR_SQUEAK, randf2(0.78f, 0.81f), 0x3a98,
                    this->position, 500.0f, 1500.0f);
    } // L8038A5EC

    if (0.65 < sp60) {
      ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f,
                                  local->unk0, &sp70, &sp6C, &sp68);
    ...
    if (ml_vec3f_distance(this->position, sp74) < 50.0f) {
      ...
      FUNC_8030E8B4(SFX_C5_TWINKLY_POP, 1.0f, 25000, this->position, 0x1f4,
                    0x9c4);
      marker_despawn(this->marker);
    }
  } // L8038A794
  ...
  if (this->state == 3) {
    ...
    ml_vec3f_interpolate_fast(this->position, local->unkC, local->unk18,
                              local->unk24);
