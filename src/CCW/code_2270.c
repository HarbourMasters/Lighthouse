...
/* .data */
Struct_CCW_2270_0 D_8038ECD0[] = {{MAP_44_CCW_SUMMER, 0xCDE, 0xCDF, 0x000},
                                  {MAP_45_CCW_AUTUMN, 0x000, 0x000, 0xCE0},
                                  0};

ActorInfo D_8038ECE8 = {MARKER_1B1_CCW_GOBI,
                        ACTOR_29E_CCW_GOBI,
...
void CCW_func_8038868C(Actor *this, s32 next_state) {
  ...
  if (next_state == 2) {
    ...
    timedFunc_set_2(3.2f, (GenFunction_2)CCW_func_80388660, (s32)this->marker,
                    3);
  }
  if (next_state == 3) {
    func_803883F4();
...
void func_8038894C(ActorMarker *marker, ActorMarker *other_marker) {
  ...
  if (actor->state == 1) {
    ...
    timedFunc_set_2(0.5f, (GenFunction_2)CCW_func_80388660, (s32)actor->marker,
                    2);
  }
}
...
Actor *chGobiCCW_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx, Vtx **vtx) {
  ...
  if (this->state == 2) {
    ...
    modelRender_setBoneTransformList(
        skeletalAnim_getBoneTransformList(local->unk4));
    modelRender_setDepthMode(MODEL_RENDER_DEPTH_COMPARE);
    modelRender_draw(gfx, mtx, this->position, sp2C, 1.0f, NULL,
                     local->spit_model);
  }
...
void chGobiCCW_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    if ((map_get() == MAP_44_CCW_SUMMER) &&
        ...
        !fileProgressFlag_get(FILEPROG_E4_CCW_FLOWER_SUMMER)) {
      CCW_func_8038868C(this, 1);
    } else if ((map_get() == MAP_45_CCW_AUTUMN) &&
               fileProgressFlag_get(FILEPROG_E4_CCW_FLOWER_SUMMER) &&
               !fileProgressFlag_get(FILEPROG_E5_CCW_FLOWER_AUTUMN)) {
      CCW_func_8038868C(this, 1);
    } else {
      marker_despawn(this->marker);
    }
  ...
  if (this->state == 4) {
    ...
    if (skeletalAnim_getAnimId(this->unk148) == ASSET_177_ANIM_GOBI_SLEEP) {
      ...
      if ((sp44 < 0.1) && (0.1 <= (f64)sp40)) {
        FUNC_8030E8B4(SFX_5E_BANJO_PHEWWW, 0.8f, 15000, this->position, 500,
                      1500);
      }
      if ((sp44 < 0.8) && (0.8 <= (f64)sp40)) {
        FUNC_8030E8B4(SFX_5D_BANJO_RAAOWW, 0.8f, 15000, this->position, 500,
                      1500);
      }
    }
  }
