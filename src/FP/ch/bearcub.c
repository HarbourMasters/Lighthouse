...
void chcaterpillar_draw(ActorMarker *caller, enum asset_e text_id, s32 arg1){
    if(text_id == 0xc19){
        ...
        case 1://L8038A5B0
            ...
            if(levelSpecificFlags_get(D_80391E80[sp3C].unk0)){
                ...
                if (sp38 == ASSET_C19_DIALOG_UNKNOWN) {
                    gcdialog_showText(sp38, 0x2f, this->position, this->marker, chcaterpillar_draw, NULL);
                }
                else {
                    gcdialog_showText(sp38, 0x3, this->position, this->marker, chcaterpillar_draw, NULL);
                }
            }
            else{//L8038A73C
