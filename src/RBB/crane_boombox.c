...
void chTNTDownSwitch_emits(ActorMarker *marker){
    ...
    timedFunc_set_2(4.0f, (GenFunction_2) chTNTDownSwitch_setStateByMarker, (uintptr_t)actor->marker, 3);
}
...
void chTNTDownSwitch_dropTNTActions(ActorMarker *marker){
    ...
    timedFunc_set_1(1.5f, (GenFunction_1) chTNTDownSwitch_emits, (uintptr_t)actor->marker);
}
...
void chTNTDownSwitch_setState(Actor *this, s32 next_state){
    ...
    if(this->state == CH_TNT_DOWN_SWITCH_STATE_2_PRESSED){
        ...
        timedFunc_set_1(1.6f, (GenFunction_1)chTNTDownSwitch_dropTNTActions, (uintptr_t)this->marker);
