...
void yaw_update_limitless(f32 arg0) {
  f32 dYaw_deg;
  f32 sp20;
  ...
  sp20 =
      (mlAbsF(dYaw_deg) > 180.0f) ? arg0 : ((dYaw_deg < 0.0f) ? -arg0 : arg0);
...
static void yaw_update_limited(f32 limit_degPerSec, f32 step_percent) {
  f32 dyaw_deg;
  f32 val_deg;
  ...
  val_deg = (val_deg < 0) ? ml_clamp_f(val_deg, -max_deg, -0.1f)
                          : ml_clamp_f(val_deg, 0.1f, max_deg);

  yaw_deg =
      (mlAbsF(val_deg) <= mlAbsF(dyaw_deg)) ? yaw_deg + val_deg : yawIdeal_deg;
...
void yaw_update(void) {
  ...
  case YAW_STATE_1_DEFAULT:
    yaw_update_limited(700.0f, 7.5f);
    break;
  case YAW_STATE_2_UNBOUNDED:
    yaw_update_limitless(D_8037C69C);
    break;
  case YAW_STATE_3_BOUNDED:
    yaw_update_limited(D_8037C6A0, D_8037C6A4);
    break;
  }
}

void yaw_setUpdateState(s32 arg0) { yawUpdateState = arg0; }

void yaw_setIdeal(f32 arg0) { yawIdeal_deg = mlNormalizeAngle(arg0); }

void yaw_set(f32 arg0) { yaw_deg = mlNormalizeAngle(arg0); }

void yaw_applyIdeal(void) { yaw_deg = yawIdeal_deg; }

s32 yaw_getUpdateState(void) { return yawUpdateState; }

f32 yaw_get(void) { return yaw_deg; }

f32 yaw_getIdeal(void) { return yawIdeal_deg; }
...
void yaw_setVelocityUnbounded(f32 arg0) { D_8037C69C = arg0; }
