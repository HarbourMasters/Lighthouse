...
/* .data */
BKSprite_s *gSpriteHealth = NULL;
BKSprite_s *gSpriteRedHealth = NULL;
Gfx D_8036A918[] = {
    gsDPPipeSync(),
...
void fxhealthscore_draw(enum item_e item_id, struct8s *arg1, Gfx **gfx,
  ...
  __gSPDisplayList((*gfx)++, D_8036A918);
  func_80347FC0(gfx, gSpriteHealth, 0, 0, 0, 0, 0, 2, 2, &honeycomb_width,
                &honeycomb_height);
  ...
  for (i = gTotalHealth - 1; i >= 0; i--) { // L80300E40
    ...
    if (gHealth > i) {
      if (0 < (gHealth - 8.0f) && (gHealth - 8.0f) > i) {
        if (!is_red_health_initialized) {
          func_80347FC0(gfx, gSpriteRedHealth, 0, 0, 0, 0, 0, 2, 2,
    ...
    f14 = (f32)gFramebufferHeight / 2 - func_802FB0E4(arg1) - D_80381F08[s6] -
          -48.0f;
    ...
    for (tmp_v1 = 0; tmp_v1 < 2; tmp_v1++) {   // L8030101C
      for (tmp_v0 = 0; tmp_v0 < 2; tmp_v0++) { // L80301030
        (*vtx)->v.ob[0] = (((honeycomb_width * D_80381EFC) * tmp_v0 -
                            (honeycomb_width * D_80381EFC) / 2) +
                           f20) *
                          4.0f;
        (*vtx)->v.ob[1] = (((honeycomb_height * D_80381EFC) / 2 -
                            (honeycomb_height * D_80381EFC) * tmp_v1) +
                           f14) *
                          4.0f;
        (*vtx)->v.ob[2] = -0x14;
