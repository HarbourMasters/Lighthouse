...
/* .data */
static BKSprite_s *s_sprite = NULL;
...
void fxairscore_draw(enum item_e item_id, struct8s *arg1, Gfx **gfx, Mtx **mtx,
                     Vtx **vtx) {
  f32 y;
  f32 x;
  s32 texture_width;
  ...
  if (s_sprite != 0) {
    __gSPDisplayList((*gfx)++, s_fxairscore_context);
    func_80347FC0(gfx, s_sprite, 0, 0, 0, 0, 0, 2, 2, &texture_width,
                  &texture_height);
    ...
    for (i_part = 0; i_part < AIRSCORE_COUNT; i_part++) {
      ...
      for (v_y = 0; v_y < 2; v_y++) {
        for (v_x = 0; v_x < 2; v_x++) {
          (*vtx)->v.ob[0] = (x + (((texture_width * s_texture_scale) * v_x) -
                                  ((texture_width * s_texture_scale) / 2))) *
                            4.0f;
          (*vtx)->v.ob[1] = (y + (((texture_height * s_texture_scale) / 2) -
                                  (texture_height * s_texture_scale) * v_y)) *
                            4.0f;
          (*vtx)->v.ob[2] = -0x14;
...
s32 fxairscore_count_to_time(s32 count) { return (s32)(count * 60 * 60) / 6; }
...
void fxairscore_update(enum item_e item_id, struct7s *arg1) {
  ...
  case 1: // load sprite
    if (s_sprite == NULL) {
      s_sprite = (BKSprite_s *)assetcache_get(ASSET_7E5_SPRITE_WATER_HONEYCOMB);
    }
    break;
  case 0:
