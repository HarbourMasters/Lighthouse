// BanjoDecomp: core2/code_72060.c
#include <ultra64.h>
...
static u32 sSnowNextId = 0; // [port] Monotonic spawn id
...
void func_802F962C(Gfx **gfx, Mtx **mtx, Vtx **vtx) {
    ...
    if ((D_80369280 != NULL) && (D_80369284 != 0)) {
        ...
        for(phi_s0 = D_80369280->unk1C; phi_s0 < D_80369280->unk1C + D_80369284; phi_s0++) {
            // [port] Scope keyed on spawn id
            FrameInterpolation_RecordOpenChild("weather_snow", phi_s0->id);
...
bool func_802F989C(Gfx **gfx, Mtx **mtx, f32 arg2[3]) {
    ...
    if( ((-17000.0f < D_80381070[0]) &&(D_80381070[0] < 17000.0f)) 
        ...
        // [port] Cancelled by PerfPatches.cpp, which applies the same rotations
        // with the trig cached across flakes.
        if (EventSystem_Should(VB_SNOW_CAMERA_ROTATION, true, D_80381060)) {
