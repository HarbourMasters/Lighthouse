    ...
    for(phi_s0 = temp_s2->begin; phi_s0 < temp_s2->current; phi_s0++){
        if (phi_s0->unk0 == 2) {
            ...
            if (phi_s0->unk8[1] <= phi_s0->unk50) {
                phi_s0->unk0 = 1;
                phi_s0->unk3C = (ParticleEmitter *)(uintptr_t)dustEmitter_returnGiven(1); // [port] s32 to ParticleEmitter* via uintptr_t
                if (phi_s0->unk3C != NULL) { // [port] was != 0
                    dustEmitter_emit(phi_s0->unk8, (f32 *)sp70, phi_s0->unk40, 1, D_80366330, D_80366334, (s32)D_80366338, (s32)D_8036633C, 1); // [port] s32* D_80366418 reinterpreted as f32[3]
                }
            }
        }
        if (phi_s0->unk0 == 1) {
            if (!dustEmitter_isActive((s32)(intptr_t)phi_s0->unk3C)) { // [port] ParticleEmitter* to s32 — N64 stored pointer as s32
                phi_s0->unk0 = 0;
            }
            phi_s0->unk8[1] -= 2.0f;
        }
