...
void fxcommon3score_draw(enum item_e item_id, void *arg1, Gfx **gfx, Mtx **mtx, Vtx **vtx){
    ...
    if(a1->model != NULL && func_802FB0D4(arg1)){
        // [port] These are centered collection popups, not edge-anchored HUD elements.
        // Native coordinates already map to screen center in widescreen.
        draw_x = a1->unk30;
        a1->value_string[0] = '\0';
        ...
        sp50[2] = 0.0f;
        func_8033A308(sp50);
        // [port] On N64 depth precision kept HUD items in front of world geometry.
        // On PC the jiggy model fails the Z test and clips behind the level.
        // Use DEPTH_NONE so HUD items always render on top, matching pause menu behavior.
