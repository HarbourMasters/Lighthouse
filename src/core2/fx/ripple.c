...
extern bool isOverWater(f32[3], f32);
extern void particleEmitter_setUnknown(ParticleEmitter *, s32, f32);
extern void particleEmitter_setYPosition(ParticleEmitter *, f32);
...
bool __fxRipple_802F3370(f32 position[3], s32 *arg1, bool arg2) {
  ...
  *arg1 = 0;
  if (arg2) {
    ...
    arg2 = isOverWater(position, 20.0f);
    if (!arg2) {
      return FALSE;
    }
  }

  if (getCollisionHeightFromObject(position, arg2, arg1, &sp1C)) {
    position[1] = sp1C;
  }
  return TRUE;
}
...
ParticleEmitter *__fxRipple_create(s32 arg0, f32 position[3], bool arg2) {
  ...
  p_emitter = getOrCreateParticleEmitter(D_80380A60);
  particleEmitter_setSprite(p_emitter, ASSET_70C_SPRITE_RIPPLE);
  particleEmitter_setDrawMode(p_emitter, PART_EMIT_ROTATABLE);
  if (sp44) {
    particleEmitter_setUnknown(p_emitter, sp44, spawn_position[1]);
  } else {
    particleEmitter_setUnknown(p_emitter, 0, 0.0f);
  }
  ...
  particleEmitter_setStartingScaleRange(p_emitter, var_s0->unkC[0],
                                        var_s0->unkC[1]);
  particleEmitter_setFinalScaleRange(p_emitter, var_s0->unk14[0],
                                     var_s0->unk14[1]);
  particleEmitter_setParticleLifeTimeRange(p_emitter, var_s0->lifetime.min,
                                           var_s0->lifetime.max);
  particleEmitter_setCullDistance(p_emitter, 100.0f);
  return p_emitter;
}
...
void fxRipple_free(void) { freeParticleEmitter(D_80380A60); }

void fxRipple_init(void) { D_80380A60 = allocateParticleEmitter(0x19); }
...
void fxRipple_802F363C(f32 arg0) {
  particleEmitter_setYPosition(getOrCreateParticleEmitter(D_80380A60), arg0);
}
