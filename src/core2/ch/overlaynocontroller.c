...
Actor *chOverlayNoController_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx,
                                  Vtx **vtx);
void chOverlayNoController_update(Actor *this);

/* .data */
ActorAnimationInfo D_80368150[] = {{0, 0.0f}, {0x286, 1.1f}, {0x287, 1.6f}};
f32 D_80368168[3] = {0.0f, 0.0f, 0.0f};
ActorInfo D_80368174 = {MARKER_178_NO_CONTROLLER_OVERLAY,
                        ACTOR_1DF_NO_CONTROLLER_OVERLAY,
                        ASSET_55D_MODEL_NO_CONTROLLER_OVERLAY,
                        0x0,
                        D_80368150,
                        chOverlayNoController_update,
                        actor_update_func_80326224,
                        chOverlayNoController_draw,
                        0,
                        0,
                        0.0f,
                        0};
...
/* .code */
Actor *chOverlayNoController_draw(ActorMarker *marker, Gfx **gdl, Mtx **mptr,
                                  Vtx **vptr) {
  Actor *actor;
  f32 sp58[3];
  f32 sp4C[3];
...
void __chOverlayNoController_spawn(void) {
  if (chOverlayNoController_marker == NULL) {
    chOverlayNoController_marker =
        actor_spawnWithYaw_f32(ACTOR_1DF_NO_CONTROLLER_OVERLAY, D_80368168, 0)
            ->marker;
  }
}
...
void chOverlayNoController_spawn(s32 arg0, s32 arg1) {
  if (chOverlayNoController_marker == NULL) {
    spawnQueue_add_0(__chOverlayNoController_spawn);
  }
}
