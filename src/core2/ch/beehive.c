...
ActorInfo chBeehive = {MARKER_50_BEEHIVE,
                       ACTOR_12_BEEHIVE,
...
void chBeehive_die(ActorMarker *this, ActorMarker *other) {
  ...
  FUNC_8030E8B4(SFX_11_WOOD_BREAKING_1, 1.0f, 28000, actor->position, 300,
                3000);
  FUNC_8030E8B4(SFX_D_EGGSHELL_BREAKING, 1.0f, 28000, actor->position, 300,
                3000);
  ...
  spawnQueue_add_4((GenFunction_4)spawnQueue_actor_f32, ACTOR_4C_STEAM,
                     reinterpret_cast(s32, actor->position[0]),
                     reinterpret_cast(s32, actor->position[1]),
                     reinterpret_cast(s32, actor->position[2]));
...
void chBeehive_update(Actor *this) {
  if (!this->volatile_initialized) {
    ...
    this->unk38_0 = volatileFlag_get(VOLATILE_FLAG_1) |
  ...
  case CHBEEHIVE_STATE_1_IDLE:
    if (!fileProgressFlag_get(FILEPROG_D_BEEHIVE_TEXT) &&
      ...
      if (player_movementGroup() == BSGROUP_0_NONE ||
        ...
        if (player_getTransformation() == TRANSFORM_1_BANJO &&
            gcdialog_showText(ASSET_D96_DIALOG_BEEHIVE_MEET, 0, NULL, NULL,
                              NULL, 0)) {
    ...
    if (actor_animationIsAt(this, 0.45f) || actor_animationIsAt(this, 0.55f) ||
      ...
      if (!this->unk38_0) {
        ...
        sfxsource_playSfxAtVolume(
            this->unk44_31, (anctrl_getAnimTimer(this->anctrl) + 0.9) - 0.4);
