...
s32 func_802D76E0(Actor *this, f32 *arg1) {
  ...
  sp28 = findClosestActorPosition2D(sp40, 0x27, this->position);
  temp_v0 = findClosestActorPosition2D(sp34, 0x28, this->position);
  if (!sp28 && !temp_v0) {
    return FALSE;
  }
  if (sp28 && temp_v0) {
    ...
    if (((sp4C[0] * sp4C[0]) + (sp4C[2] * sp4C[2])) < (sp3C * sp3C)) {
      if ((this->position[1] < sp40[1]) && (sp40[1] <= local->unk0[1])) {
        if (volatileFlag_get(VOLATILE_FLAG_2_FF_IN_MINIGAME) == 0) {
          player_setClimbParams(this->position, local->unk0, (f32)this->unkF4_8,
                                local->unkC);
        }
      }
    }
