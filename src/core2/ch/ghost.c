...
/* .data */
ActorAnimationInfo D_80372BE0[] = {{0x00, 0.0f},
...
/* .code */
void func_8035AFE0(f32 scale, f32 pos[3], s32 cnt, enum asset_e sprite_id,
                   s32 arg4[3]) {
  ...
  particleEmitter_setSpawnPositionRange(pCtrl, -100.0f * scale, 20.0f * scale,
                                        -100.0f * scale, 100.0f * scale,
                                        200.0f * scale, 100.0f * scale);
  particleEmitter_setAccelerationRange(pCtrl, 0.0f, -10.0f * scale, 0.0f, 0.0f,
                                       -10.0f * scale, 0.0f);
  particleEmitter_setParticleVelocityRange(
      pCtrl, -100.0f * scale, -20.0f * scale, -100.0f * scale, 100.0f * scale,
      100.0f * scale, 100.0f * scale);
...
void func_8035B1CC(ActorMarker *this_marker, ActorMarker *other_marker) {
  ...
  sp30 = this->scale;
  func_8030E878(SFX_121_AWAWAU, 1.3f, 32000, this->position,
                this->scale * 400.0f, this->scale * 1800.0f);
  func_8030E878(SFX_30_MAGIC_POOF, 1.0f, 32000, this->position,
                this->scale * 400.0f, this->scale * 1800.0f);
...
void func_8035B49C(Actor *this) {
  ...
  this->velocity_z +=
      (this->velocity_y +=
       (0.0f <= this->velocity_z) ? -(0.1 * this->scale) : 0.1 * this->scale);
...
void func_8035B6CC(Actor *this) {
  ...
  func_8030E878(0x3F4, randf2(1.0f, 1.2f), 32000, this->position,
                this->scale * 400.0f, this->scale * 1800.0f);
}
...
void func_8035B75C(Actor *this) {
  ...
  this->actor_specific_1_f =
      volatileFlag_get(VOLATILE_FLAG_C1_IN_FINAL_CHARACTER_PARADE) ? 0.0 : 12.0;
  func_8030E878(0x3F4, randf2(1.0f, 1.2f), 32000, this->position,
                this->scale * 400.0f, this->scale * 1800.0f);
}
...
void func_8035B900(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    marker_setCollisionScripts(this->marker, NULL, func_8035B3B4,
                               func_8035B1CC);
