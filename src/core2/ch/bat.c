...
ActorInfo gChBat = {MARKER_127_BAT,
                    ACTOR_163_BAT,
                    ASSET_3CA_MODEL_BAT,
                    CH_BAT_STATE_ROOSTING,
                    sChBatAnimations,
                    chbat_update,
...
bool func_8035FC98(Actor *this, f32 arg1) {
  ...
  if (findCollisionTriAlongPath3(sp28, sp1C, sp34, 0x5E0000) != NULL)
    return TRUE;
  return FALSE;
}
...
bool func_80360198(Actor *this) {
  ...
  this->actor_specific_1_f +=
      (this->velocity[1] * 0.45) - (0.001 * this->actor_specific_1_f);
...
bool chbat_updateRoam(Actor *this) {
  ...
  if (this->lifetime_value == 0.0f) {
    ...
    this->yaw_ideal = ml_angle_between_points_2D(
...
void chBat_dieCollision(ActorMarker *this_marker, ActorMarker *other_marker) {
  ...
  FUNC_8030E8B4(SFX_115_BUZZBOMB_DEATH, 1.3f, 26000, this->position, 1250,
                2500);
}

void chBat_attackCollision(ActorMarker *this_marker,
...
void chbat_update(Actor *this) {
  ...
  if (!this->initialized) {
    ...
    marker_setCollisionScripts(this->marker, NULL, chBat_attackCollision,
                               chBat_dieCollision);
    this->unk38_0 = FALSE;
    this->actor_specific_1_f = 0.0f;
  ...
  case CH_BAT_STATE_EXIT_ROOST: // L80360938
    if (0.98 < anctrl_getAnimTimer(this->anctrl) || !func_8035FD28(this)) {
      chbat_chase(this);
    }
    break;
  case CH_BAT_STATE_CHASE: // L8036097C
    ...
    if (!chbat_nearPlayer(this)) {
      ...
      if (!(globalTimer_getTime() & 0xf)) {
        if (randf() < 0.35) {
          FUNC_8030E8B4(SFX_419_UNKNOWN, 1.0f, 28000, this->position, 1250,
                        2500);
        }
      }
    } // L80360A40
    if (actor_animationIsAt(this, 0.5f)) {
      func_8030E878(SFX_2_CLAW_SWIPE, randf2(1.0f, 1.2f), 10000, this->position,
                    833.0f, 2500.0f);
    }
    break;
