...
extern struct5Bs *getMapModelUnk24(void);

/* .data */
f32 D_80365DD0[3][3] = {{-320.0f, 340.0f, 350.0f},
...
s32 D_80365E00 = -1;
f32 D_80365E04[3][3] = {{-435.0f, 278.0f, -159.0f},
                        {444.635437f, 216.0f, -356.591675f},
                        {55.0f, 191.822906f, -905.96875f}};

ActorAnimationInfo D_80365E28[] = {{0x000, 0.0f}, {0x24D, 9e+09f},
                                   {0x24D, 2.0f}, {0x24E, 1.0f},
                                   {0x24F, 0.6f}, {0x24D, 2.0f}};
ActorInfo D_80365E58 = {0xE4,
                        0x195,
                        0x532,
                        0x1,
                        D_80365E28,
...
ActorAnimationInfo D_80365E7C[] = {
    {0x000, 0.0f}, {0x250, 9e+09f}, {0x250, 4.5f},
    {0x251, 1.0f}, {0x252, 0.67f},  {0x250, 4.5f},
};
ActorInfo D_80365EAC = {0xE5,
                        0x196,
                        0x532,
                        0x1,
                        D_80365E7C,
...
ActorAnimationInfo D_80365ED0[] = {{0x000, 0.0f}, {0x24A, 9e+09f},
                                   {0x24A, 1.0f}, {0x24B, 1.0f},
                                   {0x24C, 1.0f}, {0x24A, 1.0f}};
ActorInfo D_80365F00 = {0xE6,
                        0x197,
                        0x532,
                        0x1,
                        D_80365ED0,
...
void *func_802C44EC(f32 arg0[3], f32 arg1[3], f32 arg2) {
  ...
  for (i = 0; i < 3; i++) {
    D_8037DD10[D_8037DD38][i] =
        arg0[i] +
        ((arg1[i] - arg0[i]) *
         sinf((((arg2 / 0.75) * 3.1415926535897931) / 2) * D_8037DD3C)) /
            D_8037DD40;
    D_8037DCF8[D_8037DD38][i] +=
        (D_8037DD10[D_8037DD38][i] - D_8037DCF8[D_8037DD38][i]) / 5.0;
  }
...
void func_802C4AF0(Actor *this) {
  ...
  comusic_fadeTrackWithArgsNoDelay(COMUSIC_73_GAMEBOY, 0, 4000);
  comusic_stopTrackById(COMUSIC_73_GAMEBOY);
...
void func_802C4C14(Actor *this) {
  ...
  if (!this->initialized) {
    spawnQueue_add_1((GenFunction_1)func_802C4BB4,
  ...
  if (!sp80) {
    ...
    pfsManager_getSideButtonState(0, sp74);
    ...
    if (!func_8038AAB0()) {
      ...
      case 1: // L802C4F10
        if (sp84 == 1) {
          ...
          timedFunc_set_3(0.25f, (GenFunction_3)comusic_fadeTrackWithArgsNoDelay,
                          COMUSIC_73_GAMEBOY, -1, 2000);
          playMusicWithFade(0, 2000);
        } else {
          comusic_fadeTrackWithArgsNoDelay(COMUSIC_73_GAMEBOY, 0, 4000);
          playMusicWithFade(-1, 2000);
        }
        ...
        break;
      case 5: // L802C5040
        if (D_8037DD2C == 0 && (sp5C[FACE_BUTTON(BUTTON_A)] == 1 ||
                                sp5C[FACE_BUTTON(BUTTON_B)] == 1)) {
      ...
      case 4: // L802C50C8
        if (anctrl_isStopped(this->anctrl)) {
          ...
          if (!gameFile_isNotEmpty(sp84)) {
            timedFunc_set_3(0.0f, (GenFunction_3)game_setMapWithTransition,
                            MAP_85_CS_SPIRAL_MOUNTAIN_3, 0, 1);
          } else { // L802C511C
            sp44 = 0.0f;
            if (this->state == 4 && (sp84 == 0 || sp84 == 1))
              sp44 = 0.25f;
            if (chmole_learnedAllSpiralMountainAbilities() &&
                fileProgressFlag_get(FILEPROG_BD_ENTER_LAIR_CUTSCENE)) {
              timedFunc_set_2(
                  sp44, (GenFunction_2)warp_lairEnterLairFromSMLevel, 0, 0);
            } else { // L802C5188
              timedFunc_set_2(sp44, (GenFunction_2)warp_smExitBanjosHouse, 0,
                              0);
            } // L802C51A0
            timedFunc_set_1(sp44, (GenFunction_1)fadeAllTracks, 1);
          } // L802C51B8
          this->state = 6;
        }
        break;
      case 2: // L802C51CC
        if (sp74[0] == 1) {
          if (gameFile_isNotEmpty(sp84)) {
            func_8031877C(chGameSelectTopZoombox);
            func_803183A4(chGameSelectTopZoombox,
                          (&D_80365DFC)[code94620_func_8031B5B0()]);
            D_8037DD2C = 1;
          ...
          if (gameFile_isNotEmpty(sp84)) {
            if (randf() < 0.1) {
              ...
              case 0: // L802C52B8
                ...
                timedFunc_set_2(0.4f, (GenFunction_2)sfxsource_play,
                                SFX_13A_GLASS_BREAKING_7, 0x7fff);
                timedFunc_set_2(0.9f, (GenFunction_2)sfxsource_play,
                                SFX_150_PORCELAIN_CRASH, 0x7fff);
                timedFunc_set_2(1.0f, (GenFunction_2)sfxsource_play,
                                SFX_151_CAT_MEOW, 0x7fff);
                break;
              case 1: // L802C5320
                timedFunc_set_2(0.4f, (GenFunction_2)sfxsource_play,
                                SFX_31_BANJO_OHHWAAOOO, 28000);
                timedFunc_set_2(0.2f, (GenFunction_2)sfxsource_play,
                                SFX_E_SHOCKSPRING_BOING, 28000);
                func_8030E540(SFX_2D_KABOING);
                break;
              case 2: // L802C5364
                timedFunc_set_2(0.15f, (GenFunction_2)sfxsource_play,
                                SFX_32_BANJO_EGHEE, 28000);
        ...
        if (D_8037DD2C == 0) {
          ...
          if (20.0 < D_8037DD34) {
            func_8031877C(chGameSelectTopZoombox);
            gczoombox_setStrings(chGameSelectTopZoombox, 2,
                                 (char **)&D_8037DCE0);
            D_8037DD34 = 0.0f;
          }
        }
    ...
    ncStaticCamera_setPositionAndTarget(
        func_802C44EC(D_80365DD0[D_8037DCE8], D_80365DD0[D_80365E00],
                      D_8037DD30),
        func_802C44EC(D_80365E04[D_8037DCE8], D_80365E04[D_80365E00],
                      D_8037DD30));
    if (this->marker->unk14_21)
      osViBlack(0);
  } // L802C5734
  ...
  if (!this->initialized) {
    ...
    if (chGameSelectBottomZoombox == NULL) {
      chGameSelectBottomZoombox =
          gczoombox_new(0xA0, ZOOMBOX_SPRITE_C_BANJO_2, 2, 0, NULL);
      gczoombox_open(chGameSelectBottomZoombox);
      gczoombox_func_803184C8(chGameSelectBottomZoombox, 30.0f, 5, 2, 0.4f, 0,
                              0);
    } // L802C57FC

    if (chGameSelectTopZoombox == NULL) {
      chGameSelectTopZoombox =
          gczoombox_new(0xA, ZOOMBOX_SPRITE_D_KAZOOIE_1, 2, 1, func_802C44D0);
      gczoombox_setStrings(chGameSelectTopZoombox, 2, (char **)&D_8037DCE0);
    ...
    comusic_playTrackWithVolumeOverride(COMUSIC_73_GAMEBOY, 0);
  } // L802C5940
  if (!func_8038AAB0()) {
    if (chGameSelectBottomZoombox)
...
void func_802C5994(void) {
  ...
  if (a1 || t6) {
    if (D_80365E00 != -1 && !game_isSpecialMode() &&
...
s32 func_802C5A30(void) { return D_80365E00; }

void func_802C5A3C(s32 arg0) { D_80365E00 = arg0; }

void func_802C5A48(void) { D_80365E00 = -1; }
