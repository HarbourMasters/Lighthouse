...
void chShrapnel_explode(ActorMarker *marker, ActorMarker *other_marker) {
    ...
    SFX_PLAYFADE(SFX_1B_EXPLOSION_1, 1.0f, 32736, this->position, 1250, 2500);
...
void chShrapnel_func_802D0FC8(Actor *this) {
    ...
    if (MAX(0.25, (12.0 - this->actor_specific_1_f) / 12.0) < this->unk4C) {
        *(s32 *)(&this->local) = NOT(*(s32 *)(&this->local));
        this->unk4C = 0.0f;
        if (*(s32 *)(&this->local)) {
            SFX_PLAYFADE(SFX_2A_CLOCK_TIC_1, 0.5f, 12000, this->position, 1250, 2500);
        }
        else{
            SFX_PLAYFADE(SFX_51_CLOCK_TIC_2, 0.5f, 12000, this->position, 1250, 2500);
        }
    }
}
        ...
        case 2:
            ...
            if (func_80329480(this)) {
                if (250.0 > ABS(player_position[1] - this->unk1C[1])) {
                    ...
                    SFX_PLAYFADE(SFX_C4_TWINKLY_MUNCHER_GRR, 0.6f, 32750, this->position, 1250, 2500);
