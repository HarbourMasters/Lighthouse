...
#define _HorzDist3v(v1, v2)                                                    \
  ((v1[0] - v2[0]) * (v1[0] - v2[0]) + (v1[2] - v2[2]) * (v1[2] - v2[2]))
...
/* .data */
ActorAnimationInfo D_80372B50[] = {{0x000, 0.0f},
...
ActorInfo D_80372B80 = {MARKER_250_ICECUBE_A,
                        ACTOR_37D_ICECUBE_A,
...
ActorInfo D_80372BA4 = {MARKER_25F_ICECUBE_B,
                        ACTOR_3A0_ICECUBE_B,
                        ASSET_504_MODEL_ICECUBE,
                        1,
                        D_80372B50,
...
int func_80359F40(Actor *this, f32 arg1[3]) {
  ...
  sp24 = ml_angle_between_points_2D(this->position[0], this->position[2],
                                    arg1[0], arg1[2]);
...
void func_8035A04C(f32 position[3], s32 cnt, enum asset_e model_id, f32 scale) {
  ...
  particleEmitter_setSpawnPositionRange(pCtrl, -100.0f * scale, 0.0f,
  ...
  particleEmitter_setParticleVelocityRange(
      pCtrl, -400.0f * scale, 450.0f * scale, -400.0f * scale, 400.0f * scale,
      600.0f * scale, 400.0f * scale);
  particleEmitter_setAngularVelocityRange(pCtrl, 100.0f * scale, 100.0f * scale,
                                          100.0f * scale, 250.0f * scale,
                                          250.0f * scale, 250.0f * scale);
...
void func_8035A228(f32 position[3], s32 cnt, enum asset_e sprite_id,
                   f32 scale) {
  ParticleEmitter *pCtrl = partEmitMgr_newEmitter(cnt);
  ...
  particleEmitter_setPosition(pCtrl, position);
  particleEmitter_setSpawnPositionRange(pCtrl, -50.0f * scale, 0.0f,
                                        -50.0f * scale, 50.0f * scale,
                                        200.0f * scale, 50.0f * scale);
  particleEmitter_setAccelerationRange(pCtrl, 0.0f, -10.0f, 0.0f, 0.0f, -10.0f,
                                       0.0f);
  particleEmitter_setParticleVelocityRange(pCtrl, -280.0f * scale, 0.0f,
                                           -280.0f * scale, 280.0f * scale,
                                           280.0f * scale, 280.0f * scale);
...
void func_8035A3F8(f32 position[3], s32 cnt, enum asset_e sprite_id,
                   f32 scale) {
  ParticleEmitter *pCtrl = partEmitMgr_newEmitter(cnt);
  ...
  particleEmitter_setAccelerationRange(pCtrl, 0.0f, -10.0f, 0.0f, 0.0f, -10.0f,
                                       0.0f);
  particleEmitter_setParticleVelocityRange(pCtrl, -60.0f * scale, 0.0f,
                                           -60.0f * scale, 60.0f * scale,
                                           60.0f * scale, 60.0f * scale);
...
void func_8035A594(f32 position[3], f32 arg1, f32 arg2) {
  ...
  for (i = 60; i != 380; i += 160) {
    ...
    if (v0 < arg1 && arg2 < v0) {
      func_8030E878(SFX_28_RUSTLING_NOISE, randf2(1.6f, 1.7f), 17000, position,
                    500.0f, 1000.0f);
    }
  }
}
...
void __chicecube_ow(ActorMarker *marker, ActorMarker *other_marker) {
  Actor *actor = marker_getActor(marker);
  FUNC_8030E8B4(SFX_1D_HITTING_AN_ENEMY_1, 0.9f, 22000, actor->position, 1500,
                3000);
...
void __chicecube_die(ActorMarker *marker, ActorMarker *other_marker) {
  Actor *actor = marker_getActor(marker);
  FUNC_8030E8B4(SFX_B6_GLASS_BREAKING_1, 1.0f, 32000, actor->position, 1500,
                4500);
  actor->velocity[1] = 0.0f;
  func_8035A04C(actor->position, 12, ASSET_505_MODEL_ICECUBE_CHUNK,
                actor->scale);
  func_8035A228(actor->position, 6, ASSET_700_SPRITE_DUST, actor->scale);
  if (actor->unkF4_8 != 1) {
    spawnQueue_add_1((GenFunction_1)__chicecube_spawnHalfCubes,
                       reinterpret_cast(s32, actor->marker));
  }
...
void func_8035A9E0(Actor *this) {
  subaddie_set_state_with_direction(this, 3, anctrl_getAnimTimer(this->anctrl),
                                    1);
...
void chicecube_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    marker_setCollisionScripts(this->marker, __chicecube_ow, NULL,
                               __chicecube_die);
  ...
  case 2: // L8035AD10
    if (actor_animationIsAt(this, 0.1f)) {
      FUNC_8030E8B4(SFX_112_TINKER_ATTENTION, 1.3f, 23000, this->position, 1500,
                    4500);
    }
  ...
  case 3: // L8035AD88
    ...
    if (!func_80359DF4(this, 1300)) {
      subaddie_set_state_with_direction(this, 4,
                                        anctrl_getAnimTimer(this->anctrl), 1);
      actor_loopAnimation(this);
      this->unk38_31 = 1;
    } else {
