...
ActorInfo D_80367A20 = {MARKER_11_WADING_BOOTS,
                        ACTOR_65_WADING_BOOTS,
                        ASSET_366_MODEL_WADING_BOOTS,
                        0x0,
                        D_80367A00,
                        chwadingboots_update,
...
/* .code */
Actor *chwadingboots_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx,
                          Vtx **vtx) {
...
void chwadingboots_update(Actor *this) {
  ...
  if (!this->initialized) {
    ...
    this->unk10_12 = !volatileFlag_get(VOLATILE_FLAG_1F_IN_CHARACTER_PARADE) &&
                     !volatileFlag_get(VOLATILE_FLAG_1);
    subaddie_set_state(this, 0);
  }

  if (!volatileFlag_get(VOLATILE_FLAG_10_HAS_MEET_WADING_BOOTS) &&
  ...
  case 0: // L802D6C60
    ...
    if (volatileFlag_get(VOLATILE_FLAG_10_HAS_MEET_WADING_BOOTS))
      break;
    if (!func_80329530(this, 250))
      break;
    if (player_getTransformation() != TRANSFORM_1_BANJO)
      break;

    if (gcdialog_showText(ASSET_DA5_DIALOG_WADINGBOOTS_MEET, 0, NULL, NULL,
                          NULL, NULL)) {
      volatileFlag_set(VOLATILE_FLAG_10_HAS_MEET_WADING_BOOTS, TRUE);
    }
...
f32 chwadingboots_802D6E4C(Actor *this) { return this->velocity[0]; }
