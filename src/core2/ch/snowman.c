...
Actor *chSnowman_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx,
                      Vtx **vtx);   // chSirSlush_draw
...
ActorInfo chSnowman = {MARKER_B1_SIR_SLUSH,
                       ACTOR_124_SIR_SLUSH,
                       ASSET_377_MODEL_SIR_SLUSH,
                       CHSNOWMAN_STATE_1_IDLE,
                       chSnowmanAnimations,
                       chSnowman_update,
...
void __chSnowman_spawnSnowballParticles(f32 position[3], s32 count) {
  static ParticleScaleAndLifetimeRanges D_80368694 = {
      {0.4f, 1.55f}, {0.0f, 0.0f}, {0.0f, 0.01f}, {4.0f, 4.0f}, 0.0f, 0.3f};
  ...
  particleEmitter_setVelocityAccelerationAndPositionRanges(particleSpawner,
                                                           &D_803686BC);
  particleEmitter_setAngularVelocityRange(particleSpawner, -600.0f, -600.0f,
                                          -600.0f, 600.0f, 600.0f, 600.0f);
  particleEmitter_setBounceFactor(particleSpawner, 0.01f);
  particleEmitter_setCollisionCount(particleSpawner, 3);
  particleEmitter_setSfxPitchRange(particleSpawner, 1.0f, 1.3f);
...
int __chSnowman_isPlayerInAttackRange(Actor *this, s32 min_distance,
                                      s32 max_distance) {
  f32 player_position[3];
  if (this->unkF4_8 == 0x33) {
    player_getPosition(player_position);
    if ((this->position[1] + 500.0f < player_position[1]) ||
        (player_position[1] < this->position[1] - 500.0f))
      return 0;
  } // L802E1F28
...
void __chSnowman_deathCallback(ActorMarker *marker, ActorMarker *other_marker) {
  ...
  FUNC_8030E8B4(SFX_15_METALLIC_HIT_2, 1.0f, 30000, actor->position, 1500,
                4500);
  ...
  spawnQueue_add_1((GenFunction_1)__chSnowman_spawnHat, (s32)actor->marker);
...
int __chSnowman_CCW_playerInProtectedZone(void) {
  ...
  if (map_get() == MAP_46_CCW_WINTER) {
    player_getPosition(player_position);
    if (ml_vec3f_within_horizontal_distance(player_position, ccw_no_attack_zone,
                                            900.0f))
      return 1;
  }
  return 0;
...
void chSnowman_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    marker_setCollisionScripts(this->marker, NULL, NULL,
                               __chSnowman_deathCallback);
    ...
    if (!func_80329530(this, 3150)) {
      __chSnowman_enterDeath(this);
               ...
               fileProgressFlag_get(
                   FILEPROG_13_COMPLETED_TWINKLIES_MINIGAME)) { // L802E2318
      if (0.0 < local->unk4) {
        if ((globalTimer_getTime() & 1) ||
            player_movementGroup() == BSGROUP_A_FLYING) { // L802E236C
          local->unk4 -= dt;
        }
      } else { // L802E2380
        if (__chSnowman_isPlayerInAttackRange(this, 500, 2750) &&
          ...
          subaddie_set_state_with_direction(this, CHSNOWMAN_STATE_2_ATTACK,
                                            0.01f, 1);
          actor_playAnimationOnce(this);
        }
      }
  ...
  case CHSNOWMAN_STATE_2_ATTACK: // L802E23E8
    if (!func_80329530(this, 3150)) {
    ...
    else if (0.98 < anctrl_getAnimTimer(this->anctrl) ||
      ...
      if (actor_animationIsAt(this, 0.19f) ||
        ...
        FUNC_8030E8B4(SFX_A7_WOODEN_SWOSH, 1.3f, 18000, this->position, 800,
                      3050);
      } // L802E24FC
      if (actor_animationIsAt(this, 0.15f)) {
        func_8030E878(SFX_3F5_UNKNOWN, randf2(1.35f, 1.5f), 32000,
                      this->position, 800.0f, 3050.0f);
      } // L802E2558

      if (actor_animationIsAt(this, 0.45f)) {
        local->unk9 = TRUE;
      } else if (actor_animationIsAt(this, 0.58f) && local->unkB) {
        func_8030E878(SFX_8F_SNOWBALL_FLYING, randf2(0.95f, 1.05f), 30000,
                      this->position, 800.0f, 3050.0f);
        spawnQueue_add_1((GenFunction_1)__chSnowman_spawnSnowball,
                           (s32)this->marker);
        local->unk9 = FALSE;
      }
    }
