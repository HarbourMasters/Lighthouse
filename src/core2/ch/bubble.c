...
extern void addStructWithParams(s32, f32[3], f32, ActorMarker *,
                          s32 (*)(f32[3], f32, ActorMarker *));

/* .data */
ActorInfo chBubble = {MARKER_6B_GLOOPBUBBLE,
                      ACTOR_E7_GLOOP_BUBBLE,
                      ASSET_704_SPRITE_BUBBLE,
                      0,
                      NULL,
                      chBubble_update,
...
void __chBubble_freeCallback(Actor *this) {
  func_8030E988(SFX_C5_TWINKLY_POP, 1.0f, 32000, this->position, 100.0f,
                3000.0f);
}

void __chBubble_collisionCallback(ActorMarker *this_marker,
...
void chBubble_update(Actor *this) {
  if (!this->initialized) {
    ...
    marker_setCollisionScripts(this->marker, __chBubble_collisionCallback, NULL,
                               NULL);
    marker_setFreeMethod(this->marker, __chBubble_freeCallback);
    addStructWithParams(func_8032994C(), this->position, 200.0f, this->marker,
                  __chBubble_func_802C5C70);
