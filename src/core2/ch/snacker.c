...
ActorInfo gChSnacker = {MARKER_14_SNACKER,
                        ACTOR_68_SNACKER,
                        ASSET_3B0_SNACKER,
                        0x1,
                        sChSnackerAnimations,
                        chsnacker_update,
...
/* .code */
void func_802E0CD0(Actor *this) { this->actor_specific_1_f = 4.0f; }
  ...
  if (this->unk38_31 != 0) {
    ...
    if (level_get() == LEVEL_2_TREASURE_TROVE_COVE) {
      ...
      if (!this->has_met_before) {
        if (text_index < 4) {
          if (gcdialog_showText(0xA1B + text_index, 0, NULL, NULL, NULL,
                                NULL)) {
            text_index++;
...
bool func_802E0DC0(f32 snacker_position[3]) {
  ...
  return (4000000.0f <
          ml_distanceSquared_vec3f(player_position, snacker_position)) ||
         ((snacker_position[1] - mapModel_getFloorY(snacker_position) <
           70.0f) &&
          (mapModel_findFloorYWithFlags(snacker_position) - snacker_position[1] < 70.0f));
}
...
void func_802E0EC8(void) {
  ...
  if (local->ctl != CH_SNACKER_OPA_2_FADE) {
    ...
    comusic_fadeTrackWithArgsNoDelay(COMUSIC_34_SNACKER_DANGER, 0, 750);
    comusic_stopTrackById(COMUSIC_34_SNACKER_DANGER);
...
void func_802E0FC4(Actor *this) {
  ...
  if (comusic_isTrackQueued(COMUSIC_34_SNACKER_DANGER)) {
    comusic_stopTrackById(COMUSIC_34_SNACKER_DANGER);
    comusic_stopTrack(COMUSIC_34_SNACKER_DANGER);
  }
}
...
void chsnacker_update(Actor *this) {
  ...
  if (!this->initialized) {
    ...
    marker_setCollisionScripts(this->marker, __chsnacker_ow, func_802E1010,
                               __chsnacker_die);
  }
  _player_getPosition(player_position);
  controller_state = snackerctl_get_state();

  if (func_802E0DC0(this->position) ||
      ((controller_state != SNACKER_CTL_STATE_1_RBB) &&
       (controller_state != SNACKER_CTL_STATE_2_TTC))) {
  ...
  switch (this->state) {
  case 1: // 802E12C8
    if (subaddie_maybe_set_state_position_direction(this, 2, 0.0f, 1, 0.03f) !=
        0) {
      func_802E0CD0(this);
    }
  ...
  this->depth_mode = (255.0 == local->opa) ? MODEL_RENDER_DEPTH_FULL
...
void chSnacker_spawn(void) {
  ...
  if (volatileFlag_get(VOLATILE_FLAG_C1_IN_FINAL_CHARACTER_PARADE) != 0) {
    nodeprop_getPosition(nodeprop_findByActorIdAndPosition_f32(
                             ACTOR_3CB_UNKNOWN, spawn_position),
                         spawn_position);
  } else {
    ...
    if (findCollisionTriAlongPath3(sp38, sp2C, sp20, 0xF800FF0F)) {
  ...
  if (volatileFlag_get(VOLATILE_FLAG_C1_IN_FINAL_CHARACTER_PARADE) == 0) {
    ...
    comusic_playTrackWithVolumeOverride(COMUSIC_34_SNACKER_DANGER, 0);
    comusic_fadeTrackWithArgsNoDelay(COMUSIC_34_SNACKER_DANGER, 0x7FFF, 750);
...
void chsnacker_setControlState(SnackerCtlState control_state) {
  ...
  if (game_getMode() == GAME_MODE_A_SNS_PICTURE ||
      game_getMode() == GAME_MODE_5_UNKNOWN ||
      game_getMode() == GAME_MODE_6_FILE_PLAYBACK) {
    return;
  }

  if ((s_chSnacker_marker == NULL)) {
    s_chSnacker_spawnTimer += dt;
    if ((s_chSnacker_respawnDelay_s < s_chSnacker_spawnTimer) &&
        (control_state != SNACKER_CTL_STATE_0_INACTIVE)) {
      spawnQueue_add_0(chSnacker_spawn);
      s_chSnacker_respawnDelay_s = 1.0f;
    }
  }
}
