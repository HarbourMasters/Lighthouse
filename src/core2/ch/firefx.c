...
/* .data */
ActorInfo gChFireFxInfo = {MARKER_256_FIRE_FX,
                           ACTOR_383_FIRE_FX,
...
void __chfirefx_spawnSmoke(f32 position[3], f32 scale) {
  ...
  particleEmitter_setSpawnPositionRange(pCtrl, 0.0f, 110.0f * scale, 0.0f, 0.0f,
                                        110.0f * scale, 0.0f);
  particleEmitter_setParticleVelocityRange(pCtrl, 0.0f, 40.0f * scale, 0.0f,
                                           0.0f, 90.0f * scale, 0.0f);
...
void __chfirefx_spawnSpark(f32 position[3], f32 scale) {
  ...
  particleEmitter_setStartingFrameRange(pCtrl, 1, 6);
  particleEmitter_setPosition(pCtrl, position);
  particleEmitter_setSpawnPositionRange(pCtrl, 0.0f, 20.0f * scale, 0.0f, 0.0f,
                                        20.0f * scale, 0.0f);
  particleEmitter_setParticleVelocityRange(
      pCtrl, -30.0f * scale, 120.0f * scale, -30.0f * scale, 60.0f * scale,
      360.0f * scale, 60.0f * scale);
  particleEmitter_setAccelerationRange(pCtrl, 0.0f, -50.0f, 0.0f, 0.0f, -90.0f,
                                       0.0f);
...
void __chfirefx_hiss(ActorMarker *marker, ActorMarker *other_marker) {
  ...
  FUNC_8030E8B4(SFX_96_HOTSAND_EEL_HISS, 1.0f, 32000, actor->position, 1000,
                2000);
}
