...
void __chdrips_particleCallback(ParticleEmitter *pCtrl, f32 position[3]) {
  if (isOverWater(position, 4.0f)) {
    position[1] += 2.0f;
    func_80359A40(position, &D_80372AF0, 1);
    func_8030E878(SFX_413_WATER_RIPPLE, randf2(1.0f, 1.4f), 20000, position,
                  0.0f, 3500.0f);
  }
}
...
void chdrips_update(Actor *this) {
  ...
  if (__chdrips_playerWithinDist(this, 5000) &&
      randf() < this->lifetime_value) {
