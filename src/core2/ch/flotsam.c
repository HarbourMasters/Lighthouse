...
/* .data */
ActorInfo D_80372C80 = {MARKER_C9_FLOTSAM,
                        ACTOR_13B_FLOTSAM,
...
ParticleScaleAndLifetimeRanges D_80372CA4 = {
    {1.0f, 1.2f}, {1.6f, 2.0f}, {0.05f, 0.05f}, {0.3f, 0.5f}, 0.1f, 0.5f};
...
void func_8035C080(Actor *this, s32 next_state) {
  ...
  if (next_state == 3 || next_state == 4 || next_state == 7 ||
      next_state == 6) {
    ...
    if (next_state == 5) {
      func_8030E878(0x3f2, randf2(1, 1.2f), 0x7530, this->position, 100.0f,
                    10000.0f);
  ...
  if (next_state == 8) {
    ...
    particleEmitter_setVelocityAccelerationAndPositionRanges(local->pCtrl_8,
                                                             &D_80372CCC);
...
void func_8035C8F4(Actor *this) {
  ...
  if (this->state == 0) {
    ...
    marker_setCollisionScripts(this->marker, func_8035C6C4, func_8035C6C4,
                               func_8035C6F0);
  ...
  if (this->state == 1) {
    if (sp3C < 800.0f && func_80329210(this, plyr_pos) &&
  ...
  if (this->state == 5) {
    ...
    if (1.0f <= local->unk0)
      local->unk0 = 1.0f;
    ml_vec3f_interpolate_fast(this->position, local->unk10, local->unk1C,
                              local->unk0);
    this->position_y += 100.0f * sinf(local->unk0 * 3.141592654);
    this->yaw =
        local->unk0 * (local->unk34[2] - local->unk34[1]) + local->unk34[1];
