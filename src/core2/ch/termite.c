...
void __chTermite_updateAnimationSpeed(Actor *this) {
  if ((this->velocity[0] - 0.1 <= this->actor_specific_1_f) &&
      (this->actor_specific_1_f <= this->velocity[0] + 0.1)) {
    this->actor_specific_1_f = this->velocity[0];
  } else {
    this->actor_specific_1_f +=
        (this->velocity[0] < this->actor_specific_1_f) ? -0.2 : 0.2;
  }

  anctrl_setDuration(this->anctrl, ml_map_f(this->actor_specific_1_f, 5.0f,
                                            12.0f, 0.54f, 0.36f));
}
...
void __chTermite_updateWalkSFX(Actor *this) {
  ...
  sp20 = ml_map_f((300.0f - sp24[0]) * (300.0f - sp24[0]) +
                      (this->position[1] - sp24[1]) *
                          (this->position[1] - sp24[1]) +
                      (-858.0f - sp24[2]) * (-858.0f - sp24[2]),
                  7617600.0f, 8236900.0f, 8000.0f, 1000.0f);
...
void __chTermite_emitLegs(ParticleEmitter *pCtrl, f32 position[3]) {
  ...
  particleEmitter_setAccelerationRange(pCtrl, 0.0f, -1800.0f, 0.0f, 0.0f,
                                       -1800.0f, 0.0f);
  particleEmitter_setModel(pCtrl, ASSET_393_MODEL_TERMITE_LEG);
  particleEmitter_setStartingScaleRange(pCtrl, 0.5f, 0.8f);
  particleEmitter_setAngularVelocityRange(pCtrl, -800.0f, -800.0f, -800.0f,
                                          800.0f, 800.0f, 800.0f);
  particleEmitter_setParticleVelocityRange(pCtrl, -200.0f, 850.0f, -200.0f,
...
void __chTermite_emitHead(ParticleEmitter *pCtrl, f32 position[3]) {
  ...
  particleEmitter_setParticleVelocityRange(pCtrl, -50.0f, 750.0f, -50.0f,
...
void __chTermite_emitEyes(ParticleEmitter *pCtrl, f32 position[3]) {
  __chTermite_setupParticleEmitter(pCtrl, position);
  particleEmitter_setAccelerationRange(pCtrl, 0.0f, -1400.0f, 0.0f, 0.0f,
                                       -1400.0f, 0.0f);
  ...
  particleEmitter_setParticleVelocityRange(pCtrl, -80.0f, 400.0f, -80.0f,
                                           160.0f, 860.0f, 160.0f);
...
void __chTermite_testCallback(ActorMarker *caller, enum asset_e text_id,
                              s32 arg2) {
  Actor *this;
...
void chTermite_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    marker_setCollisionScripts(this->marker, NULL, __chTermite_ow,
                               __chTermite_die);
  ...
  if (map_get() == MAP_C_MM_TICKERS_TOWER && !mapSpecificFlags_get(0) &&
      func_80329530(this, 700) && !func_80329530(this, 150) &&
      player_movementGroup() == BSGROUP_0_NONE &&
      player_getTransformation() == TRANSFORM_1_BANJO) {
    gcdialog_showText(ASSET_B43_DIALOG_TERMITE_MEET_AS_BEAR, 7, this->position,
                      this->marker, __chTermite_testCallback, NULL);
    mapSpecificFlags_set(0, TRUE);
    levelSpecificFlags_set(LEVEL_FLAG_D_MM_UNKNOWN, TRUE);
    this->has_met_before = TRUE;
  }

  if (func_80329530(this, 300) && !func_80329530(this, 150) &&
      player_movementGroup() == BSGROUP_0_NONE &&
      player_getTransformation() == TRANSFORM_2_TERMITE) {
    if (!levelSpecificFlags_get(LEVEL_FLAG_B_MM_UNKNOWN)) {
      if (gcdialog_showText(ASSET_B41_DIALOG_TERMITE_COOL_SHORTS, 0, NULL, NULL,
                            NULL, NULL)) {
        levelSpecificFlags_set(LEVEL_FLAG_B_MM_UNKNOWN, TRUE);
        this->unk138_23 = TRUE;
      }
    } else if (!levelSpecificFlags_get(LEVEL_FLAG_C_MM_UNKNOWN) &&
               !this->unk138_23 &&
               (gcdialog_showText(ASSET_B42_DIALOG_TERMITE_COOL_BACKPACK, 0,
                                  NULL, NULL, NULL, NULL))) {
  ...
  case 2:
    ...
    if (this->unk38_31 != 0) {
      ...
      if (isRandfInRange(0.73f, 0.76f) &&
          func_80329078(this, (s32)this->yaw, 750)) {
...
ActorInfo chTermite = {MARKER_4_TERMITE,
                       ACTOR_5_TERMITE,
