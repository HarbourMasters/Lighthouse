...
void __baanim_update_scaleToHorizontalVelocity(void) {
  ...
  scale =
      (baAnimScale.scalable_duration != 0) ? baAnimScale.duration_scale : 1.0f;
  baphysics_get_velocity(velocity);
  temp_f12 = ml_mapRange_f(
      gu_sqrtf(velocity[0] * velocity[0] + velocity[2] * velocity[2]),
      baAnimScale.velocity_min, baAnimScale.velocity_max,
      baAnimScale.duration_min * scale, baAnimScale.duration_max * scale);
  anctrl_setDuration(playerAnimCtrl, ml_clamp_f(temp_f12, baAnimMinDuration,
                                                baAnimMaxDuration));
...
void __baanim_update_scaleToVerticalVelocity(void) {
  anctrl_setDuration(
      playerAnimCtrl,
      ml_clamp_f(
          ml_mapRange_f(mlAbsF(baphysics_get_vertical_velocity()),
                        baAnimScale.velocity_min, baAnimScale.velocity_max,
                        baAnimScale.duration_min, baAnimScale.duration_max),
          baAnimMinDuration, baAnimMaxDuration));
  anctrl_update(playerAnimCtrl);
}
...
void __baanim_oscillateScale(f32 dst[3], f32 x, f32 min, f32 osc_size) {
  ...
  for (phi_s0 = 0; phi_s0 < 3; phi_s0++) {
    dst[phi_s0] =
        min + osc_size * (0.5 + 0.5 * sinf((2.0 * BAD_PI) *
                                           ml_fractional_part(
                                               x + (2.0 * ((f32)phi_s0 / 3.0)),
                                               2.0f)));
  };
}
...
void __baanim_applyBottlesBonus(s32 arg0, s32 arg1) {
  ...
  if (map_get() == MAP_A_TTC_SANDCASTLE) {
    if ((D_8037BF4C & BAANIM_WISHYWASHY) &&
...
void baAnim_init(void) {
  ...
  anctrl_setUnk23(playerAnimCtrl, 0);
  anctrl_setSmoothTransition(playerAnimCtrl, FALSE);
  anctrl_setCallback(playerAnimCtrl, __baanim_applyBottlesBonus);
...
void baAnim_free(void) { anctrl_free(playerAnimCtrl); }
...
void baAnim_defrag(void) { playerAnimCtrl = anctrl_defrag(playerAnimCtrl); }

enum baanim_update_type_e baanim_getUpdateType(void) { return baAnimState; }
...
void baanim_setVelocityMapRanges(f32 vel_min, f32 vel_max, f32 dur_slowest,
                                 f32 dur_fastest) {
...
f32 baanim_getTimer(void) { return anctrl_getAnimTimer(playerAnimCtrl); }

bool baanim_isAnimID(enum asset_e anim_id) {
  return anctrl_getIndex(playerAnimCtrl) == anim_id;
}

bool baanim_isStopped(void) { return anctrl_isStopped(playerAnimCtrl); }

bool baanim_isAt(f32 time) { return anctrl_isAt(playerAnimCtrl, time); }
...
void baanim_playForDuration_loopStartingAt(enum asset_e anim_id, f32 duration,
                                           f32 start_position) {
  anctrl_reset(playerAnimCtrl);
  anctrl_setIndex(playerAnimCtrl, anim_id);
  anctrl_setDuration(playerAnimCtrl, duration);
...
void baanim_playForDuration_onceSmoothStartingAt(enum asset_e anim_id,
                                                 f32 duration,
                                                 f32 start_position) {
  anctrl_reset(playerAnimCtrl);
  anctrl_setSmoothTransition(playerAnimCtrl, FALSE);
