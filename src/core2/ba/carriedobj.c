...
void bacarriedobj_spawn(enum actor_e actor_id) {
  ...
  if (marker && actor->modelCacheIndex == actor_id) {
    ...
    spawnQueue_add_1((GenFunction_1)__bscarriedobj_spawnMethod,
                       baMarker_getCarriedObjectActorId());
  }
}
...
void func_8028DEEC(enum actor_e actor_id, Actor *actor) {
  ...
  nodeprop_getPosition(nodeprop_findByActorIdAndActorPosition(actor_id, actor),
                       sp1C);
