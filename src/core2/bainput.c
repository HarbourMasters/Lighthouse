...
int bainput_should_peck(void) { return bakey_pressed(BUTTON_B) && can_peck(); }

int bainput_should_dive(void) {
  return bakey_pressed(BUTTON_B) && (bainput_diveCooldown == 0.0f) &&
         can_dive();
}

int bainput_isEnabled(s32 arg0) { return bainput_enableMask & (1 << arg0); }
...
void bainput_update(void) {
  if (bainput_diveCooldown != 0.0f) {
    bainput_diveCooldown =
        ml_max_f(bainput_diveCooldown - time_getDelta(), 0.0f);
  }
}
