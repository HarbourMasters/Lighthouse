...
s32 coords_getMaxDotProduct(s32 min[3], s32 max[3], f32 position[3],
                         f32 direction[3]) {
  f32 corner[3];
  f32 dot_product;
  f32 result;
  ...
  for (ix = 0; ix < 2; ix++) {
    for (iy = 0; iy < 2; iy++) {
      for (iz = 0; iz < 2; iz++) {
        ...
        dot_product = (direction[0] * corner[0]) + (direction[1] * corner[1]) +
                      (direction[2] * corner[2]);
...
bool coords_isPointInsideWithOffset(s32 min[3], s32 max[3], s32 point[3],
                                    s32 offset) {
  int i;
