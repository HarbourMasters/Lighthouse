...
void ncDynamicCam1_update(void) {
  ...
  sp4C += ml_clamp_abs((sp3C - sp4C) * temp_f20 * 2, temp_f20 * 120.0f);
  ml_vec3f_yaw_towards(sp50, &sp48);
  sp40 = ml_clamp_abs(
      mlDiffDegF(mlNormalizeAngle(180.0f + player_getYaw()), sp48) *
          (temp_f20 * 1),
      temp_f20 * 50.0f);
  sp48 = mlNormalizeAngle(sp48 + sp40);
  ml_vec3f_rotate_direction(sp68, 0.0f, sp48, 0.0f, 0.0f, sp4C);
  ml_vec3f_add(sp80, sp8C, sp68);
  sp80[1] = sp74[1] + ((func_802BD51C() - sp74[1]) * temp_f20 * 2);
  ncDynamicCamera_setPosition(sp80);
  ml_vec3f_angles_between_points(sp8C[0], sp8C[1], sp8C[2], sp80[0], sp80[1],
                                 sp80[2], &sp5C[0], &sp5C[1]);
  sp5C[0] = -sp5C[0];
  sp5C[2] = 0.0f;
  func_802BD720(sp5C);
}
