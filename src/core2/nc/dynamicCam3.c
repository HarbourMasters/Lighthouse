...
void func_802C1B20(f32 arg0) { D_8037DC10 = arg0; }
...
void ncDynamicCam3_update(void) {
  ...
  sp3C += ml_clamp_abs(sp30 * (sp34 - sp3C) * 2, sp30 * 800.0f);
  ml_vec3f_angles_between_points(sp7C[0], sp7C[1], sp7C[2], sp64[0], sp64[1],
                                 sp64[2], &sp4C[0], &sp4C[1]);
  sp40[1] = sp30 * 0.77 *
            mlDiffDegF(mlNormalizeAngle(player_getYaw() + 180.0f), sp4C[1]);
  sp40[1] = ml_clamp_abs(sp40[1], sp30 * 300.0f);
  sp4C[1] = mlNormalizeAngle(sp4C[1] + sp40[1]);
  ml_vec3f_rotate_direction(&sp58, 0.0f, sp4C[1], 0.0f, 0.0f, sp3C);
  sp70[0] = sp7C[0] + sp58[0];
  sp70[1] = sp64[1];
  sp70[2] = sp7C[2] + sp58[2];
  sp40[1] = sp7C[1] - sp64[1];
  if (mlAbsF(sp40[1]) > 200.0f) {
    sp70[1] = sp64[1] - ((sp40[1] > 0.0f) ? sp30 * (200.0f - sp40[1]) * 2
                                          : sp30 * (-200.0f - sp40[1]) * 2);
  }
