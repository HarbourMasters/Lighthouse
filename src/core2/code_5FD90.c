...
extern void points_to_boundingBoxWithMargin(f32 arg0[3], f32 arg1[3],
                                            f32 margin, f32 min[3], f32 max[3]);
...
/* .bss */
struct {
  BKCollisionGeo *unk0[100]; //collisionGeoArray
  BKCollisionGeo **unk190; //collisionGeoPtr
} collisionGeometryData;
f32 collisionTriVertices[3][3];
Struct_core2_5FD90_0 collisionTriData[100];

/* .code */
void updateCollisionTriVertices(BKCollisionTri *arg0, BKVertexList *vtx_list) {
  Vtx *vtx;
  Vtx *i_vtx;
  s32 i;
  ...
  for (i = 0; i < 3; i++) {
    ...
    collisionTriVertices[i][0] = (f32)i_vtx->v.ob[0];
    collisionTriVertices[i][1] = (f32)i_vtx->v.ob[1];
    collisionTriVertices[i][2] = (f32)i_vtx->v.ob[2];
  }
}

void collisionList_getIntersecting(BKCollisionList *collision_list, f32 arg1[3],
                                   f32 arg2[3], BKCollisionGeo ***arg3,
                                   BKCollisionGeo ***arg4) {
  s32 sp3C[3];
  s32 sp30[3];
  ...
  if (collision_list->unk12 == 0) {
    collisionGeometryData.unk190 = &collisionGeometryData.unk0[0];
    *(collisionGeometryData.unk190++) = (s32)collision_list + sizeof(BKCollisionList);
    *arg3 = &collisionGeometryData.unk0[0];
    *arg4 = collisionGeometryData.unk190;
  } else {
    for (i = 0; i < 3; i++) {
      if (arg1[i] >= 0.0f) {
    ...
    for (z = sp3C[2]; z <= sp30[2]; z++) {
      for (y = sp3C[1]; y <= sp30[1]; y++) {
        for (x = sp3C[0]; x <= sp30[0]; x++) {
          *(collisionGeometryData.unk190++) = ((collision_list->unkE * z) +
                                    (BKCollisionGeo *)(collision_list + 1)) +
                                   x + (y * collision_list->unkC);
        }
      }
    }
    *arg3 = &collisionGeometryData.unk0[0];
...
void getIntersectingCollisionGeos(BKCollisionList *collision_list, s32 min[3],
                                  s32 max[3], BKCollisionGeo ***begin_geo_ptr,
                                  BKCollisionGeo ***end_geo_ptr) {
  s32 sp3C[3];
  s32 sp30[3];
  ...
  if (collision_list->unk12 == 0) {
    collisionGeometryData.unk190 = &collisionGeometryData.unk0[0];
    *(collisionGeometryData.unk190++) = (s32)collision_list + sizeof(BKCollisionList);
    *begin_geo_ptr = &collisionGeometryData.unk0[0];
    *end_geo_ptr = collisionGeometryData.unk190;
  } else {
    for (i = 0; i < 3; i++) {
      if (min[i] >= 0) {
...
void copyCollisionTriVertices(f32 arg0[3][3]) {
  s32 i;
  for (i = 0; i < 3; i++) {
    arg0[i][0] = collisionTriVertices[i][0];
    arg0[i][1] = collisionTriVertices[i][1];
    arg0[i][2] = collisionTriVertices[i][2];
  }
}

s32 countSpecialCollisionTris(BKCollisionList *collisionList) {
  ...
  phi_v1 = 0;
  temp_a2 =
      (collisionList->unk10 * 4) + (s32)collisionList + sizeof(BKCollisionList);
...
s32 getCollisionTriCount(BKCollisionList *collisionList) {
  return collisionList->unk14;
}

void collisionList_getTris(BKCollisionList *collision_list,
                           BKCollisionTri **begin_ptr,
                           BKCollisionTri **end_ptr) {
  *begin_ptr = (collision_list->unk10 * 4) + (s32)collision_list +
               sizeof(BKCollisionList);
  *end_ptr = (collision_list->unk14 * 0xC) + (s32)*begin_ptr;
}

bool isPointInSphere(f32 arg0[3], f32 arg1, f32 arg2[3], f32 arg3[3]) {
  f32 sp24[3];
  f32 sp20;
...
bool isPointInCylinder(f32 arg0[3], f32 arg1, f32 arg2[3], f32 arg3) {
  return (ml_vec3f_distance(arg0, arg2) < (arg1 + arg3));
}

void calculateBoundsAndDirection(f32 startPoint[3], f32 endPoint[3],
                                 s32 minBounds[3], s32 maxBounds[3],
                                 f32 directionVector[3]) {
  s32 i;
...
BKCollisionTri *findCollisionTriAlongPath2(BKCollisionList *collisionList,
                                           BKVertexList *vertexList,
                                           f32 startPoint[3], f32 endPoint[3],
                                           f32 arg4[3], u32 flagFilter) {
  s32 i;
  s32 j;
  ...
  calculateBoundsAndDirection(startPoint, endPoint, min_bounds, max_bounds,
                              direction_vector);
  for (i = 0; i < 3; i++) {
  ...
  getIntersectingCollisionGeos(collisionList, min_bounds, max_bounds,
                               &start_geo, &end_geo);
  for (i_geo = start_geo; i_geo < end_geo; i_geo++) {
    start_tri = (BKCollisionTri *)((BKCollisionGeo *)(collisionList + 1) +
                                   collisionList->unk10) +
                (*i_geo)->start_tri_index;
    ...
    for (i_tri = start_tri; i_tri < end_tri; i_tri++) {
      ...
      if ((sp164[0]->v.ob[0] < min_bounds[0]) &&
          (sp164[1]->v.ob[0] < min_bounds[0]) &&
          (sp164[2]->v.ob[0] < min_bounds[0]))
        continue;
      if ((max_bounds[0] < sp164[0]->v.ob[0]) &&
          (max_bounds[0] < sp164[1]->v.ob[0]) &&
          (max_bounds[0] < sp164[2]->v.ob[0]))
        continue;

      if ((sp164[0]->v.ob[2] < min_bounds[2]) &&
          (sp164[1]->v.ob[2] < min_bounds[2]) &&
          (sp164[2]->v.ob[2] < min_bounds[2]))
        continue;
      if ((max_bounds[2] < sp164[0]->v.ob[2]) &&
          (max_bounds[2] < sp164[1]->v.ob[2]) &&
          (max_bounds[2] < sp164[2]->v.ob[2]))
        continue;

      if ((sp164[0]->v.ob[1] < min_bounds[1]) &&
          (sp164[1]->v.ob[1] < min_bounds[1]) &&
          (sp164[2]->v.ob[1] < min_bounds[1]))
        continue;
      if ((max_bounds[1] < sp164[0]->v.ob[1]) &&
          (max_bounds[1] < sp164[1]->v.ob[1]) &&
          (max_bounds[1] < sp164[2]->v.ob[1]))
        continue;
      ...
      if ((100000.0f < spBC[0]) || (100000.0f < spBC[1]) ||
          (100000.0f < spBC[2]) || (spBC[0] < -100000.0f) ||
          (spBC[1] < -100000.0f) || (spBC[2] < -100000.0f)) {
        spBC[0] /= 100000.0f;
        spBC[1] /= 100000.0f;
        spBC[2] /= 100000.0f;
      }
      ...
      if ((!((temp_f12_2 >= 0.0f) && (pad >= 0.0f))))
        if (!((temp_f12_2 <= 0.0f) && (pad <= 0.0f))) {
          ...
          temp_f12_3 = spBC[0] * direction_vector[0] +
                       spBC[1] * direction_vector[1] +
                       spBC[2] * direction_vector[2];
          pad = (sp90[0][0] * spBC[0] + sp90[0][1] * spBC[1] +
                 sp90[0][2] * spBC[2]);
          if (temp_f12_3 == 0.0f)
            continue;

          temp_f0_2 = -((spBC[0] * startPoint[0] + spBC[1] * startPoint[1] +
                         spBC[2] * startPoint[2]) -
                        pad) /
                      temp_f12_3;
          ...
          calculateBoundsAndDirection(startPoint, endPoint, min_bounds,
                                      max_bounds, direction_vector);
        }
    }
  }
  updateCollisionTriVertices(result_collision, vertexList);
  return result_collision;
}

BKCollisionTri *findCollisionTriWithTransform(BKCollisionList *collision_list,
                                              BKVertexList *vtxList,
                                              f32 arg2[3], f32 arg3[3],
  ...
  if (!isPointInSphere(arg2, vtxList->global_norm * arg4, arg5, arg6)) {
    return 0;
  } else {
    mlMtxIdent();
    mlMtx_inverse_transform_with_scale(arg2, arg3, arg4, 0);
    mlMtx_apply_vec3f(sp44, arg5);
    mlMtx_apply_vec3f(sp38, arg6);
    sp34 = findCollisionTriAlongPath2(collision_list, vtxList, sp44, sp38, arg7,
                                      arg8);
    if (!sp34) {
      return NULL;
    } else {
      mlMtxIdent();
      mlMtx_transform_with_scale(arg2, arg3, arg4, NULL);
      mlMtx_apply_vec3f(arg6, sp38);

      mlMtxIdent();
      mlMtx_transform_with_scale(NULL, arg3, 1.0f, 0);
      mlMtx_apply_vec3f(arg7, arg7);

      mlMtxIdent();
      mlMtx_transform_with_scale(arg2, arg3, arg4, 0);

      for (i = 0; i < 3; i++) {
        mlMtx_apply_vec3f(collisionTriVertices[i], collisionTriVertices[i]);
      }
    }
  }
  return sp34;
...
//clang-format off
s32 findActiveCollisionTris(BKCollisionList *collisionList, BKVertexList *vertexList, f32 p1[3], f32 p2[3], f32 velocity[3], f32 margin, s32 flagFilter, s32 **activeTriStartPtr, s32 *activeTriEndPtr) {
    BKCollisionGeo **start_geo;
    BKCollisionGeo **i_geo;
    BKCollisionGeo **end_geo;
    f32 spC0[3];
    ...
    for(i_geo = start_geo; i_geo < end_geo; i_geo++){
        ...
        for(i_tri = start_tri; i_tri < end_tri; i_tri++){
            if (!(i_tri->flags & flagFilter)) {
                ...
                var_s2++;
                if ((var_s2 - collisionTriData) > 100) 
                    break;       
            }
        }
        if ((var_s2 - collisionTriData) > 100) 
    ...
    *activeTriStartPtr = (s32) collisionTriData; //activeTriPool
    *activeTriEndPtr = (s32) var_s2; //activeTriPoolEnd
    return var_s2 - collisionTriData > 0; //Count
}

//clang-format on
Struct_core2_5FD90_0 *findCollisionTriAtPoint(Struct_core2_5FD90_0 *startTri,
                                              Struct_core2_5FD90_0 *endTri,
                                              f32 position[3], f32 radius,
                                              f32 arg4[3]) {
  s32 i;
  ...
  for (i_tri = startTri; i_tri < endTri; i_tri++) {
    ...
    temp_f18 = (sp120[0][0] * i_tri->normal[0]) +
               (sp120[0][1] * i_tri->normal[1]) +
               (sp120[0][2] * i_tri->normal[2]);
    if ((-(radius - 0.5) >= temp_f18) || ((radius - 0.5) <= temp_f18))
      continue;

    temp_f8 = i_tri->normal[0];
    normal_length = (temp_f8 * i_tri->normal[0]) +
                    (i_tri->normal[1] * i_tri->normal[1]) +
                    (i_tri->normal[2] * i_tri->normal[2]);
    ...
    var_a2 =
        (ABS_F(i_tri->normal[2]) > ABS_F(i_tri->normal[var_a2])) ? 2 : var_a2;

    spFC[0] = projected_position[(var_a2 + 1) % 3] -
              i_tri->tri_coord[0][(var_a2 + 1) % 3];
    spFC[1] = i_tri->edgeAB[(var_a2 + 1) % 3];
    spFC[2] = i_tri->edgeAC[(var_a2 + 1) % 3];

    spF0[0] = projected_position[(var_a2 + 2) % 3] -
              i_tri->tri_coord[0][(var_a2 + 2) % 3];
    ...
    if ((0.0f <= temp_f12_3) && (temp_f12_3 <= 1.0f) && (0.0f <= temp_f0_4) &&
        (temp_f0_4 <= 1.0f))
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (sp120[i][0] * sp120[i][0] + sp120[i][1] * sp120[i][1] +
              sp120[i][2] * sp120[i][2] <
          radius * radius) {
        spD0 = i_tri;
        arg4[0] = arg4[0] + i_tri->normal[0];
    ...
    for (i = 0; i < 3; i++) {
      ...
      temp_f22 = sp144[i][0] * sp144[i][0] + sp144[i][1] * sp144[i][1] +
                 sp144[i][2] * sp144[i][2];
      ml_vec3f_normalize(sp144[i]);
      temp_f0_3 = (sp144[i][0] * sp120[i][0]) + (sp144[i][1] * sp120[i][1]) +
                  (sp144[i][2] * sp120[i][2]);
      if ((0.0f <= temp_f0_3) && ((temp_f0_3 * temp_f0_3) <= temp_f22)) {
        ...
        if (projected_position[0] * projected_position[0] +
                projected_position[1] * projected_position[1] +
                projected_position[2] * projected_position[2] <
            radius * radius) {
          spD0 = i_tri;
          arg4[0] = arg4[0] + i_tri->normal[0];
          arg4[1] = arg4[1] + i_tri->normal[1];
...
BKCollisionTri *findCollisionTriInSphere(BKCollisionList *collision_list,
                                         BKVertexList *vtx_list, f32 p1[3],
                                         f32 p2[3], f32 radius, f32 arg5[3],
                                         s32 arg6, s32 flagFilter) {
  ...
  sp78[1] = p2[1] - p1[1];
  sp78[2] = p2[2] - p1[2];
  if (!findActiveCollisionTris(collision_list, vtx_list, p1, p2, sp78,
                               (f32)((f64)radius + 0.5), flagFilter,
                               &start_active_tri, &end_active_tri)) {
    return NULL;
  }
  phi_s5 = findCollisionTriAtPoint(start_active_tri, end_active_tri, p2, radius,
                                   sp8C);
  if (phi_s5 == NULL) {
    return NULL;
  }
  ...
  for (phi_s0 = 0; phi_s0 < arg6; phi_s0++) {
    ...
    phi_v0 = findCollisionTriAtPoint(start_active_tri, end_active_tri, sp98,
                                     radius, sp8C);
    if (phi_v0 != NULL) {
      arg5[0] = sp8C[0];
      arg5[1] = sp8C[1];
  ...
  updateCollisionTriVertices(phi_s5->tri_ptr, vtx_list);
  return phi_s5->tri_ptr;
}

BKCollisionTri *findCollisionTriWithTransformAndRadius(
    BKCollisionList *collision_list, BKVertexList *vtx_list, f32 arg2[3],
    s32 arg3, f32 arg4, f32 arg5[3], f32 arg6[3], f32 arg7, f32 arg8[3],
    s32 arg9, s32 flagFilter) {
  f32 sp4C[3];
  f32 sp40[3];
  BKCollisionTri *sp3C;
  s32 i;

  if (((f32)vtx_list->global_norm * arg4) <=
      (ml_vec3f_distance(arg6, arg2) - arg7)) {
    return 0;
  }
  mlMtxIdent();
  mlMtx_inverse_transform_with_scale(arg2, arg3, arg4, 0);
  ...
  sp3C = findCollisionTriInSphere(collision_list, vtx_list, &sp4C, &sp40,
                                  arg7 / arg4, arg8, arg9, flagFilter);
  if (sp3C == NULL) {
    return NULL;
  }
  mlMtxIdent();
...
//clang-format off
BKCollisionTri *findCollisionTriAtPosition(BKCollisionList *collisionList, BKVertexList *vertexList, f32 position[3], f32 radius, f32 arg4[3], s32 flagFilter) {
    BKCollisionGeo **start_geo;
    BKCollisionGeo **i_geo;
    BKCollisionGeo **end_geo;
    s32 i;
    ...
    updateCollisionTriVertices(spD0, vertexList);
    return spD0;
}
//clang-format on

BKCollisionTri *findCollisionTriWithTransformAndScale(
    BKCollisionList *collisionList, BKVertexList *vtxList, f32 posA[3],
    f32 *rotA, f32 scaleA, f32 posB[3], f32 radB, f32 arg7[3], s32 arg8) {
  f32 sp34[3];
  BKCollisionTri *sp30;
  s32 i;

  // check if (sphere around vtx's) <= ((distance between origins) - (radius of
  // B))
  if ((vtxList->global_norm * scaleA) <=
      (ml_vec3f_distance(posB, posA) - radB)) {
    return NULL;
  }
  mlMtxIdent();
  mlMtx_inverse_transform_with_scale(posA, rotA, scaleA, NULL);
  mlMtx_apply_vec3f(sp34, posB);
  sp30 = findCollisionTriAtPosition(collisionList, vtxList, &sp34,
                                    radB / scaleA, arg7, arg8);
  if (sp30 == 0) {
    return NULL;
  }
  mlMtxIdent();
  mlMtx_transform_with_scale(posA, rotA, scaleA, NULL);
  mlMtx_apply_vec3f(posB, sp34);
  mlMtxIdent();
  mlMtx_transform_with_scale(NULL, rotA, 1.0f, NULL);
  mlMtx_apply_vec3f(arg7, arg7);
  mlMtxIdent();
  mlMtx_transform_with_scale(posA, rotA, scaleA, NULL);
