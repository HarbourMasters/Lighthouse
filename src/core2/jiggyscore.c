...
u8 *jiggyscore_getPtr(void) { return jiggyscore.D_803832C0; }

int jiggyscore_isSpawned(enum jiggy_e jiggy_id) {
  return ((jiggyscore.D_803832CD[(jiggy_id - 1) / 8] & (1 << (jiggy_id & 7))) !=
          0) ||
...
u32 jiggyscore_isCollected(enum jiggy_e jiggy_id) {
  if (jiggy_id <= 0 || jiggy_id >= 0x65)
    return 0;
  return (jiggyscore.D_803832C0[(jiggy_id - 1) / 8] & (1 << (jiggy_id & 7))) !=
         0;
}
