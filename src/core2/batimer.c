...
f32 batimer_get(s32 id) { return s_batimer.value[id]; }

int batimer_isAt(s32 id, f32 value_sec) {
  return s_batimer.last[id] <= value_sec && value_sec < s_batimer.value[id];
}
...
int batimer_isNonzero(s32 id) { return (0.0 != s_batimer.value[id]); }

int batimer_isZero(s32 id) { return (0.0 == s_batimer.value[id]); }
