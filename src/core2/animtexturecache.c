...
/* .code */
bool AnimTextureListCache_tryGetTextureOffset(s32 list_index, s32 texture_index,
                                              s32 *current_frame) {
  ...
  if (temp_v0->anim_texture_info[texture_index].frame_size != 0) {
    *current_frame = (s32)temp_v0->current_frame[texture_index] *
                     temp_v0->anim_texture_info[texture_index].frame_size;
    return TRUE;
  }
  return FALSE;
}
...
void AnimTextureListCache_free(void) { freelist_free(AnimTextureListCache); }

void AnimTextureListCache_init(void) {
  AnimTextureListCache =
      (FREE_LIST(AnimTextureList) *)freelist_new(sizeof(AnimTextureList), 2);
}
    ...
    if (freelist_elementIsAlive(AnimTextureListCache, j)) {
      ...
      for (i = 0; i < 4; i++) {
        if (j_ptr->anim_texture_info[i].frame_size != 0) {
          j_ptr->current_frame[i] +=
              (j_ptr->anim_texture_info[i].frame_rate * dt);
          if ((s32)j_ptr->current_frame[i] >=
              j_ptr->anim_texture_info[i].frame_cnt) {
