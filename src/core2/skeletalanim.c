...
extern void animationFile_getBoneTransformList(void *, f32,
                                               BoneTransformList *);
extern void boneTransformList_interpolate(BoneTransformList *,
                                          BoneTransformList *,
                                          BoneTransformList *, f32);
...
static void skeletalAnim_perform_callback(SkeletalAnimationCallback *arg0) {
  if (arg0->arg_count == 0) {
...
f32 skeletalAnim_getProgress(SkeletalAnimation *self) { return self->progress; }

void skeletalAnim_getProgressRange(SkeletalAnimation *self, f32 *prev_progress,
                                   f32 *progress) {
...
BoneTransformList *skeletalAnim_getBoneTransformList(SkeletalAnimation *self) {
  ...
  if (0.0f == self->transition_duration) {
    animationFile_getBoneTransformList(self->animation_bin, self->progress,
                                       self->bone_transform);
    return self->bone_transform;
  }
  ...
  boneTransformList_interpolate(self->bone_transform, self->transition_start,
                                self->transition_target,
...
void skeletalAnim_setCallback_0(SkeletalAnimation *self, f32 when,
                                GenFunction_0 fn) {
...
void skeletalAnim_setCallback_1(SkeletalAnimation *self, f32 when,
                                GenFunction_1 fn, s32 arg) {
  SkeletalAnimationCallback *ptr;
  if (self->callback_list == NULL) {
    self->callback_list = vector_new(sizeof(SkeletalAnimationCallback), 8);
  }
...
SkeletalAnimation *skeletalAnim_new(void) {
  ...
  self = (SkeletalAnimation *)heap_malloc(sizeof(SkeletalAnimation));
  self->bone_transform = NULL;
  self->animation_bin = NULL;
...
void skeletalAnim_markDirty(SkeletalAnimation *self) { self->unk30 = 1; }

void skeletalAnim_set(SkeletalAnimation *self, enum asset_e anim_id,
                      f32 transistion_duration, f32 duration) {
...
void skeletalAnim_swap(SkeletalAnimation *self, enum asset_e anim_id,
                       f32 transistion_duration, f32 duration) {
  f32 sp1C;
...
void skeletalAnim_setBehavior(SkeletalAnimation *self,
                              enum skeletal_anim_e behavior) {
...
void skeletalAnim_update(SkeletalAnimation *self, f32 dt, s32 arg2) {
  ...
  if (self->animation_id != 0) {
    ...
    if (self->duration > 0.0f) {
      ...
      } else if ((self->behavior == SKELETAL_ANIM_2_ONCE) &&
                 (self->loop_count == 0)) {
        // FORWARD ONCE
        self->progress += (dt / self->duration);
      ...
      } else if ((self->behavior == SKELETAL_ANIM_3_BACKWARDS) &&
                 (self->progress > 0.0f)) {
        // BACKWARDS ONCE
        self->progress -= (dt / self->duration);
    ...
    if (self->callback_list != NULL) {
      ...
      for (i_ptr = begin_ptr; i_ptr < end_ptr; i_ptr++) {
        if (((self->prev_progress < i_ptr->when) ||
             (self->progress < self->prev_progress)) &&
            (i_ptr->when <= self->progress)) {
          skeletalAnim_perform_callback(i_ptr);
        }
      }
    }
