...
void func_802AB750(f32 arg0, f32 arg1) { D_8037D3D4 = randf() * arg1 + arg0; }

int func_802AB788(void) {
  return (0.0f < mlAbsF(baphysics_get_target_vertical_velocity())) ||
         (yaw_get() != yaw_getIdeal());
}

int bsclimb_inSet(s32 move_indx) {
  return (move_indx == BS_4F_CLIMB_IDLE) || (move_indx == BS_50_CLIMB_MOVE) ||
         (move_indx == 0x9e);
}
...
void bsclimb_idle_init(void) {
  ...
  if (!bsclimb_inSet(bs_getPrevState())) {
    ml_vec3f_yaw_between(plyr_pos, pole_pos, &angle_towards_pole);
...
void bsclimb_idle_end(void) { func_802AB6F0(); }
...
void bsclimb_move_update(void) {
  ...
  if (map == MAP_B_CC_CLANKERS_CAVERN ||
    ...
    if (baanim_isAt(0.25f))
      basfx_playSfxWithVolume(SFX_D2_QUIET_METALLIC, 1.1f, 32000);

    if (baanim_isAt(0.75f))
      basfx_playSfxWithVolume(SFX_D2_QUIET_METALLIC, 1.2f, 32000);
  } else { // L802ABB34
    if (baanim_isAt(0.25f))
      basfx_playSfxWithVolume(SFX_D3_JINXIE_SNIFFLING_1, 0.77f, 22000);

    if (baanim_isAt(0.75f))
      basfx_playSfxWithVolume(SFX_D3_JINXIE_SNIFFLING_1, 0.87f, 22000);
  } // L802ABB84
  ...
  if (baphysics_get_target_vertical_velocity() < 0.0f &&
      climbGetBottomY() == plyr_pos[1])
    next_state = BS_1_IDLE;

  if (climbIsAtTop() == 2 && 0.0f < baphysics_get_target_vertical_velocity() &&
...
void bsclimb_move_end(void) { func_802AB6F0(); }
...
void func_802ABD40(void) { func_802AB6F0(); }
