...
void func_802B6D00(void) {
  ...
  if (player_isOnDangerousGround()) {
    if (sp18 == 0) {
      ...
      baphysics_set_target_horizontal_velocity(ml_interpolate_f(
          bastick_distance(), bsWalkMinMudVelocity, bsWalkMaxMudVelocity));
    }
  } else { // L802B6D78
    switch (sp18) {
    case 0: // 802B6D98
    ...
    case 1: // 802B6DAC
      baphysics_set_target_horizontal_velocity(ml_interpolate_f(
          sp1C, bsWalkMinCreepVelocity, bsWalkCreepSlowWalkVelocityThreshold));
      break;
    case 2: // 802B6DD0
      baphysics_set_target_horizontal_velocity(
          ml_interpolate_f(sp1C, bsWalkCreepSlowWalkVelocityThreshold,
                           bsWalkSlowWalkWalkVelocityThreshold));
      break;
    case 3: // 802B6DF4
      baphysics_set_target_horizontal_velocity(
          ml_interpolate_f(sp1C, bsWalkSlowWalkWalkVelocityThreshold,
                           bsWalkWalkFastWalkVelocityThreshold));
      break;
    case 4: // 802B6E18
      baphysics_set_target_horizontal_velocity(
          ml_interpolate_f(sp1C, bsWalkWalkFastWalkVelocityThreshold,
                           bsWalkMaxFastWalkVelocity));
      break;
    }
  } // L802B6E34
}
...
void func_802B6E44(void) {
  if (is_player_in_state()) {
    baanim_scaleDuration(
        ml_map_f(baphysics_get_horizontal_velocity_percentage(), 0.0f, 1.0f,
                 0.5f, 0.9f));
  } else {
...
void func_802B6EB0(f32 arg0) { D_8037D5B0 = arg0; }

void func_802B6EBC(void) {
  D_8037D5B0 = ml_max_f(0.0f, D_8037D5B0 - time_getDelta());
}

int func_802B6EF4(void) { return D_8037D5B0 == 0.0f; }

s32 func_802B6F20(s32 arg0) {
  if (bakey_pressed(BUTTON_B)) {
    if (bsWalkWalkFastWalkVelocityThreshold <
...
f32 func_802B6F9C(void) { return bsWalkMaxFastWalkVelocity; }

void func_802B6FA8(void) { func_802B6D00(); }
...
void bswalk_creep_init(void) {
  ...
  baanim_setVelocityMapRanges(
      bsWalkMinCreepVelocity, bsWalkCreepSlowWalkVelocityThreshold,
      bsWalkSlowestCreepDuration, bsWalkFastestCreepDuration);
}
...
void bswalk_slow_init(void) {
  ...
  baanim_setDurationRange(0.3f, 1.5f);
  baanim_setVelocityMapRanges(
      bsWalkCreepSlowWalkVelocityThreshold, bsWalkSlowWalkWalkVelocityThreshold,
      bsWalkSlowestSlowWalkDuration, bsWalkFastestSlowWalkDuration);
}
...
void bswalk_init(void) {
  ...
  baanim_setDurationRange(0.3f, 1.5f);
  baanim_setVelocityMapRanges(
      bsWalkSlowWalkWalkVelocityThreshold, bsWalkWalkFastWalkVelocityThreshold,
      bsWalkSlowestWalkDuration, bsWalkFastestWalkDuration);
  func_802B6EB0(0.3f);
}
...
void bswalk_update(void) {
  ...
  if (is_yaw_difference_large() &&
      bsWalkSkidVelocity < baphysics_get_horizontal_velocity()) {
    next_state = BS_SKID;
  }
...
void bswalk_fast_init(void) {
  ...
  baanim_setDurationRange(0.3f, 1.5f);
  baanim_setVelocityMapRanges(
      bsWalkWalkFastWalkVelocityThreshold, bsWalkMaxFastWalkVelocity,
      bsWalkSlowestFastWalkDuration, bsWalkFastestFastWalkDuration);
...
void bswalk_fast_update(void) {
  ...
  case 2: // L802B7A28
    ...
    break;
  case 3: // L802B7A60
    if (baphysics_is_slower_than(bsWalkWalkFastWalkVelocityThreshold) &&
        func_802B6EF4())
      next_state = BS_WALK;
...
void bswalk_mud_init(void) {
  ...
  func_8029C7F4(BAANIM_UPDATE_2_SCALE_HORZ, 1, 1, BA_PHYSICS_NORMAL);
  baanim_setDurationRange(0.3f, 1.5f);
  baanim_setVelocityMapRanges(bsWalkMinMudVelocity, bsWalkMaxMudVelocity,
                              bsWalkSlowestMudDuration,
                              bsWalkFastestMudDuration);
}
...
void bswalk_drone_init(void) {
  ...
  if (baanim_isAnimID(3)) {
    ...
    baanim_setDurationRange(0.3f, 1.5f);
    baanim_setVelocityMapRanges(bsWalkCreepSlowWalkVelocityThreshold,
                                bsWalkSlowWalkWalkVelocityThreshold,
                                bsWalkSlowestSlowWalkDuration,
                                bsWalkFastestSlowWalkDuration);
  }
}

void bswalk_drone_update(void) { bsdrone_update(); }

void bswalk_drone_end(void) { bsdrone_end(); }
