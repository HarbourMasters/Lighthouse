...
/* .data */
u8 D_80364D20[] = {
    0x8 | 0x1, 0x8 | 0x2, 0x8 | 0x4, 0x10,      0x8 | 0x1, 0x8 | 0x1, 0x8 | 0x2,
    0x8 | 0x1, 0x20,      0x8 | 0x1, 0x8 | 0x2, 0x8 | 0x4, 0x10,      0x8 | 0x1,
    0x8 | 0x1, 0x8 | 0x2, 0x8 | 0x1, 0x10,      0x8 | 0x1, 0x8 | 0x2, 0x20};
...
void func_802B4A10(AnimCtrl *arg0) {
  if (anctrl_isAt(arg0, 0.2057f)) { //(30/145)
    ...
    baMotor_setRumbleParams(0.5f, 0.25f, 0.4f);
  } // L802B4A54
...
void bsstand_update(void) {
  ...
  if (sp18 & 8) {
    if (sp18 & 4) {
      if (anctrl_isAt(anim_ctrl, 0.0909f))
        basfx_playKazooieAuwSfx();
      if (anctrl_isAt(anim_ctrl, 0.0909f))
        func_8029E070(1);
      if (anctrl_isAt(anim_ctrl, 0.6818f))
        func_8029E070(0);
    } // L802B4E70
    if (sp18 & 2) {
      if (anctrl_isAt(anim_ctrl, 0.7727f))
        func_8029E070(1);
      if (anctrl_isAt(anim_ctrl, 0.9999f))
        func_8029E070(0);
    } // L802B50E4
  } else if (sp18 & 0x20) {
    if (anctrl_getIndex(anim_ctrl) == ASSET_95_ANIM_BSSTAND_KAZOOIE_PECK) {
      ...
      if (anctrl_isAt(anim_ctrl, 0.069f))
        basfx_playSfxWithVolume(SFX_4B_GULPING, 1.4f, 0x4650);
      if (anctrl_isAt(anim_ctrl, 0.1677f))
        basfx_playSfxWithVolume(SFX_8B_KAZOOIE_RAWW, 1.0f, 0x4650);
      if (anctrl_isAt(anim_ctrl, 0.2441f))
        basfx_playSfxWithVolume(SFX_8B_KAZOOIE_RAWW, 1.03f, 0x4650);
      if (anctrl_isAt(anim_ctrl, 0.3141f))
        basfx_playSfxWithVolume(SFX_8B_KAZOOIE_RAWW, 1.06f, 0x4650);
      if (anctrl_isAt(anim_ctrl, 0.3859f))
        basfx_playSfxWithVolume(SFX_8B_KAZOOIE_RAWW, 1.5f, 0x7D00);
      if (anctrl_isAt(anim_ctrl, 0.414f))
        basfx_playSfx(SFX_2D_KABOING, 1.0f, 0x4650);
      if (anctrl_isAt(anim_ctrl, 0.55f))
        basfx_playSfx(SFX_A3_BANJO_DOUBLE_COUGH, 1.0f, 0x6D60);
      if (anctrl_isAt(anim_ctrl, 0.6187f))
        basfx_playSfxWithVolume(SFX_A0_COUGHING, 1.7f, 0x4650);
      if (anctrl_isAt(anim_ctrl, 0.7108f))
        basfx_playSfxWithVolume(SFX_A0_COUGHING, 1.6f, 0x4650);
      if (anctrl_isAt(anim_ctrl, 0.7927f))
        basfx_playSfxWithVolume(SFX_A0_COUGHING, 1.5f, 0x4650);
    }
  } else /*L802B50D4*/
    if ((sp18 & 0x10) != 0) {
      func_802B4A10(anim_ctrl);
...
void bsstand_landing_update(void) {
  ...
  if (anctrl_getIndex(sp18) == ASSET_D2_ANIM_BSSPLAT) {
    if (anctrl_isAt(sp18, 0.8264f)) {
      basfx_playSfx(SFX_6F_BANJO_HEADSCRATCH, 1.0f, 0x36b0);
    }
...
void func_802B5350(void) {
  ...
  if (sp1C == 0x7) {
    ...
    func_8029A86C(1);
    if (bsStoredState_getTransformation() == TRANSFORM_1_BANJO &&
        !baflag_isTrue(BA_FLAG_F) && stateTimer_isDone(STATE_TIMER_0_UNKNOWN)) {
