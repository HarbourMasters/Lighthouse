...
/* .code */
int bsjig_inJiggyJig(enum bs_e state) { return state == BS_44_JIG_JIGGY; }
...
void bsjig_jiggy_end(void) {
  ...
  if (jiggyscore_total() == 100 &&
    ...
    if (jiggyscore_total() == 2 && map_get() == MAP_2_MM_MUMBOS_MOUNTAIN) {
      sp2C = 0xb51;
      sp28 = 0xe;
    } else { // L802B0E34
      switch (sp24) {
      case 0x13:
        ...
        break;
      case 0x17:
        if (!levelSpecificFlags_get(LEVEL_FLAG_0_CC_TOKEN_TOOTH_OPEN) ||
...
void bsjig_notedoor_end(void) {
  ...
  func_8029E070(0);
  func_80291548();
  if (D_8037D4B2 && jiggyscore_total() == 100 &&
      fileProgressFlag_get(FILEPROG_FC_DEFEAT_GRUNTY)) {
    func_8028F918(2);
    game_setMapWithTransition(MAP_95_CS_END_ALL_100, 0, 1);
  }
...
void bsjig_notedoor_update(void) {
  ...
  if (baanim_isAt(0.59f) || baanim_isAt(0.84f))
    basfx_playSfx(SFX_3EA_UNKNOWN, 1.0f, 30000);
