...
s16 bsCrocJumpSfxList[] = {SFX_54_BANJO_HOO_1, SFX_55_BANJO_HOO_2,
                           SFX_56_BANJO_HUI};
...
void func_802ABE70(void) {
  ...
  basfx_updateClockSfxSource(stateTimer_getPrevious(STATE_TIMER_3_TURBO_TALON),
                             sp1C);
  if (baflag_isTrue(BA_FLAG_10_TOUCHING_TURBO_TRAINERS) &&
      bs_getState() != BS_17_BTROT_EXIT) {
    baflag_clear(BA_FLAG_10_TOUCHING_TURBO_TRAINERS);
    stateTimer_set(STATE_TIMER_3_TURBO_TALON, get_turbo_duration());
    comusic_playTrackWithVolumeOverride(COMUSIC_8A_GETTING_TURBO_TRAINERS, -1);
    func_8029E0DC(1);
  }
...
void func_802ABF54(void) {
  ...
  if (bastick_getZone() == 0) {
    ...
    baphysics_set_target_horizontal_velocity(ml_interpolate_f(
        sp1C, bsCrocMinWalkVelocity, __bscroc_getMaxVelocity()));
  }
}
...
int bscroc_inSet(enum bs_e state) {
  return state == BS_5E_CROC_IDLE || state == BS_CROC_WALK ||
         state == BS_CROC_JUMP || state == BS_61_CROC_FALL ||
         state == BS_CROC_OW || state == BS_CROC_DIE ||
         state == BS_6E_CROC_BITE || state == BS_CROC_EAT_BAD ||
         state == BS_70_CROC_EAT_GOOD || state == BS_8D_CROC_LOCKED ||
...
void bscroc_walk_init(void) {
  ...
  baanim_setVelocityMapRanges(bsCrocMinWalkVelocity, bsCrocMaxWalkVelocity,
                              bsCrocSlowestWalkDuration,
                              bsCrocFastestWalkDuration);
  func_802900B4();
}
...
void bscroc_jump_update(void) {
  ...
  switch (D_8037D3EC) {
  case 0:
    if (is_player_stable_or_falling(0x82)) {
...
void bscroc_fall_end(void) { func_802ABFBC(); }
...
void bscroc_ow_init(void) { __bscroc_recoil_init(1); }

void bscroc_ow_update(void) { __bscroc_recoil_update(); }

void bscroc_ow_end(void) { __bscroc_recoil_end(); }

void bscroc_bounce_init(void) { __bscroc_recoil_init(2); }

void bscroc_bounce_update(void) { __bscroc_recoil_update(); }

void bscroc_bounce_end(void) { __bscroc_recoil_end(); }
...
int bscroc_hitboxActive(void) { return _bscrocHitboxActive; }

static void func_802ACF58(void) {
  f32 sp2C;
  f32 sp20[3];
  sp2C = player_isStable() ? 500.0f : 400.0f;
  ml_vec3f_set_yaw_length(sp20, yaw_get(), sp2C);
  sp20[1] = 200.0f;
  baphysics_set_velocity(sp20);
}

void bscroc_bite_init(void) {
  baanim_playForDuration_loopStartingAt(ASSET_122_ANIM_BSCROC_BITE, 0.25f,
                                        0.2f);
...
void bscroc_eat_bad_end(void) { func_802ABFBC(); }
...
void func_802AD2A8(Gfx **gdl, Mtx **mPtr, void *arg2) {
  ...
  modelRender_draw(gdl, mPtr, sp28, player_rotation, D_8037D3F0, NULL,
                   D_8037D3E8);
}

void func_802AD318(void) { D_8037D3F5 = 1; }
...
int func_802AD3A0(void) { return D_8037D3F5; }
...
void bscroc_drone_init(void) { bsdrone_init(); }
