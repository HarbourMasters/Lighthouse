...
void func_802A7140() { func_8029C7F4(1, 3, 3, BA_PHYSICS_NO_GRAVITY); }

f32 func_802A716C() {
  f32 sp24[3];
  baphysics_get_velocity(sp24);
  ml_map_f(
      gu_sqrtf((sp24[0] * sp24[0]) + (sp24[1] * sp24[1]) + (sp24[2] * sp24[2])),
...
void func_802A71D8(void) {
  ...
  yaw_setIdeal(mlNormalizeAngle(
      yaw_getIdeal() + ml_map_f(sp30, -1.0f, 1.0f, yaw_range, -yaw_range)));
}
...
bool bsbswim_inSet(enum bs_e move_id) {
  return move_id == BS_2B_DIVE_IDLE || move_id == BS_2C_DIVE_B ||
         move_id == BS_39_DIVE_A || move_id == BS_30_DIVE_ENTER ||
         move_id == BS_78_DIVE_LOCKED || move_id == BS_7F_DIVE_OW ||
         move_id == BS_54_SWIM_DIE || move_id == BS_97_DIVE_LOCKED;
}

bool func_802A7588(void) { return bsbswim_inSet(bs_getState()); }
...
void func_802A7718() { func_802A75B0(); }
...
void func_802A77D8(void) {
  ...
  sp34 = pitch_get();
  sp30 = yaw_get();
  ml_vec3f_rotate_direction(sp24, sp34, sp30, 0.0f, 0.0f, D_8037D390);
...
void func_802A7CA8(void) {
  ...
  particleEmitter_setParticleVelocityRange(p_ctrl, -60.0f, -50.0f, -60.0f,
                                           60.0f, 100.0f, 60.0f);
...
void func_802A7E2C(void) {
  ...
  case 0:
    ...
    if (anctrl_isAt(sp38, 0.7247f)) {
      sp34 = pitch_get();
      sp30 = yaw_get();
      ml_vec3f_rotate_direction(sp24, sp34, sp30, 0.0f, 0.0f, 800.0f);
      baphysics_set_velocity(sp24);
      batimer_set(1, 0.8f);
      D_8037D396 = TRUE;
    }
...
void func_802A7F4C(void) { func_802A75B0(); }

void func_802A7F6C(void) {
  D_8037D394 = BOOL(bs_getPrevState() == BS_41_DIE);

  if (D_8037D394 || level_get() == LEVEL_9_RUSTY_BUCKET_BAY ||
      map_get() == MAP_46_CCW_WINTER) {
    D_8037D395 = 0;
  } else {
    D_8037D395 = 1;
  }
  ...
  batimer_set(1, 0.0f);
  func_802906A4(3);
  basfx_playSfx(SFX_CA_BANJO_DROWNING_1, 1.0f, 24000);
}
...
void func_802A8098(void) {
  ...
  next_state = 0;
  ml_vec3f_rotate_direction(sp40, -90.0f, 0.0f, 0.0f, 0.0f, 100.0f);
  baphysics_set_target_velocity(sp40);
  baphysics_set_velocity(sp40);
  batimer_increment(1);
  if (batimer_isAt(1, 0.2f) || batimer_isAt(1, 0.8f) || batimer_isAt(1, 1.4f)) {
    basfx_playSfx(SFX_CB_BANJO_DROWNING_2, 1.0f, 24000);
  }
  switch (D_8037D396) {
  case 0:
...
void func_802A8330(void) {
  ...
  if (func_80298850() && func_80298800(sp2C)) {
    ...
    ml_vec3f_angles_between_points(sp44[0], sp44[1], sp44[2], sp2C[0], sp2C[1],
                                   sp2C[2], &sp38[0], &sp38[1]);
...
void func_802A844C(void) { func_802A75B0(); }
...
void func_802A85EC(void) {
  ...
  particleEmitter_setParticleVelocityRange(sp34, -30.0f, -30.0f, -30.0f, 30.0f,
                                           30.0f, 30.0f);
...
void func_802A872C(void) { func_802A75B0(); }
...
void func_802A8774(void) { bsdrone_update(); }
