...
/* .code */
void __bsDroneLook_getEyePos(f32 arg0[3]) { func_8028E9C4(5, arg0); }
...
void bsDroneLook_init(void) {
  ...
  basfx_playSfxWithVolume(SFX_12D_CAMERA_ZOOM_CLOSEST, 1.2f, 12000);
...
void bsDroneLook_update(void) {
  ...
  if (ncFirstPersonCamera_getState() == FIRSTPERSON_STATE_2_IDLE) {
    ...
    eye_rotation[2] = 0.0f;
    eye_rotation[0] = (eye_rotation[0] > 180.0f)
                          ? ml_max_f(305.0f, eye_rotation[0])
                          : ml_min_f(70.0f, eye_rotation[0]);
    ncFirstPersonCamera_setZoomedOutRotation(eye_rotation);
    yaw_setIdeal(eye_rotation[1] + 180.0f);

    exit_first_person = FALSE;
    // 1st person cancelled via input
    if (bakey_pressed(BUTTON_B) || bakey_pressed(BUTTON_A) ||
        bakey_pressed(BUTTON_C_UP)) {
    ...
    } else if (is_player_stable_or_falling(25) == 0) {
      exit_first_person += TRUE;
    }
