...
void func_802B40D0(void) {
  ...
  if (D_8037D524) {
    ...
    ml_vec3f_set_yaw_length(sp2C, sp28, randf() * 10.0f + 20.0f);
    sp2C[1] = 0.0f;
  }
  ...
  ml_vec3f_set_yaw_length(sp44, yaw_get(), 40.0f);
  sp44[1] = 50.0f;
...
void bsslide_init(void) {
  ...
  if (player_isSliding()) {
    func_80294480(sp30);
    if (ml_vec3f_angles_from_vector(sp30, &sp28, &sp2C)) {
...
void bsslide_update(void) {
  ...
  if (player_isSliding()) {
    func_80294480(sp30);
    if (ml_vec3f_angles_from_vector(sp30, &sp2C, &sp28)) {
      if (D_8037D520 == 0x5A) {
      ...
      baphysics_set_target_horizontal_velocity(
          ml_map_f(sp28, 20.0f, 60.0f, 550.0f, 700.0f));
