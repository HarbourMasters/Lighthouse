...
void func_802A0704(void) { func_802A02C0(); }
...
void _bsBeeFly_updateYaw(void) {
  ...
  yaw_setIdeal(mlNormalizeAngle(yaw_getIdeal() +
                                ml_map_f(stickX, -1.0f, 1.0f, sp34, -sp34)));
}
  ...
  if (baflag_isTrue(BA_FLAG_9)) {
    ...
    if (sp3C <= 80.0f) {
      baphysics_set_terminal_velocity(
          ml_map_f(sp3C, 60.0f, 80.0f, -99.9, -1266.66));
      sp38 = ml_map_f(sp3C, 60.0f, 80.0f, 600.0f, 60.0f);
    } else {
      baphysics_set_terminal_velocity(
          ml_map_f(sp3C, 300.0f, 310.0f, -399.99, -99.9));
...
void bsbeefly_end(void) { _bsbeefly_end(); }

int bsBeeFly_inSet(s32 move) { return move == 0x8C; }
