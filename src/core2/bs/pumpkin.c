...
u8 D_80364D08 = 0;
s16 D_80364D0C[] = {SFX_54_BANJO_HOO_1, SFX_55_BANJO_HOO_2,
                    SFX_56_BANJO_HUI}; // enum sfx_e
...
void func_802B223C(void) {
  ...
  baphysics_set_target_horizontal_velocity(
      ml_interpolate_f(sp1C, D_80364CF0, D_80364CF4));
}
...
int bspumpkin_inSet(s32 move_indx) {
  return (move_indx == BS_48_PUMPKIN_IDLE) ||
         ...
         (move_indx == BS_4B_PUMPKIN_FALL) || (move_indx == BS_4D_PUMPKIN_OW) ||
...
void bspumpkin_jump_update(void) {
  ...
  case 0:
    if (baphysics_get_vertical_velocity() < 0.0f) {
      if (is_player_stable_or_falling(130)) {
        baeyes_close();
...
void bspumpkin_fall_update(void) {
  ...
  if (player_isStable() && ((bastick_getZone() > 0) ||
                            (D_8037D4E0 == 2 && anctrl_isStopped(anim_ctrl)))) {
...
void bspumpkin_fall_end(void) { func_802B229C(); }
...
void __bspumpkin_bounce_init(s32 arg0) {
  ...
  func_80294980(sp20);
  ml_vec3f_yaw_between(sp20, plyr_pos, &sp38);
...
void __bspumpkin_bounce_update(void) {
  ...
  switch (D_8037D4E0) {
  case 0:
    if (is_player_stable_or_falling(90)) {
...
void __bspumpkin_ow_init(void) { __bspumpkin_bounce_init(1); }

void __bspumpkin_ow_update(void) { __bspumpkin_bounce_update(); }

void __bspumpkin_ow_end(void) { __bspumpkin_bounce_end(); }

void bspumpkin_rebound_init(void) { __bspumpkin_bounce_init(2); }

void bspumpkin_rebound_update(void) { __bspumpkin_bounce_update(); }

void bspumpkin_rebound_end(void) { __bspumpkin_bounce_end(); }
...
void bspumpkin_drone_init(void) { bsdrone_init(); }

void bspumpkin_drone_update(void) { bsdrone_update(); }
