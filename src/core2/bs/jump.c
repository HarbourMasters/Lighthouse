...
void bsjump_init(void) {
  ...
  if (sp30 == BS_11_BPECK) {
    ...
    baphysics_set_horizontal_velocity(
        yaw_getIdeal(), baphysics_get_target_horizontal_velocity());
    if (D_8037D4C1) {
...
void bsjump_update(void) {
  ...
  if ((bakey_released(BUTTON_A) && 0.0f < velocity[1] && !D_8037D4C2) ||
      !can_control_jump_height()) {
...
void bsjump_fall_init(void) {
  ...
  if (baflag_isTrue(BA_FLAG_7_TOUCHING_JIGGY) &&
      700.0f < baphysics_get_vertical_velocity())
...
void func_802B1DA4(void) { baMarker_collisionOn(); }

bool bsjump_jumpingFromWater(void) { return D_8037D4C1; }
...
void bsjump_tumble_init(void) {
  ...
  if (bafalldamage_get_state() == 1) {
    batimer_set(0, 0.5f);
    batimer_set(1, 0.41f);
    basfx_playSfx(SFX_52_BANJO_YAH_OH, 1.0f, 22000);
  } else {
    batimer_set(0, 0.0f);
    batimer_set(1, 0.01f);
  }
...
void bsjump_tumble_end(void) {
  ...
  if (next_state == BS_F_BBUSTER || next_state == BS_BFLAP ||
