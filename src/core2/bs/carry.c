...
void func_802AAE80(void) {
  ...
  if (bastick_getZone() == 0)
    ...
    baphysics_set_target_horizontal_velocity(
        ml_interpolate_f(sp1C, D_80364AF0, D_80364AF4));
}
...
void bscarry_idle_end(void) { func_802AAEE0(); }
...
void bscarry_walk_end(void) { func_802AAEE0(); }

int bscarry_inSet(enum bs_e state) {
  return state == BS_3A_CARRY_IDLE || state == BS_3B_CARRY_WALK;
}
