...
u8 D_80364DF0 = 0;
s16 D_80364DF4[] = {SFX_54_BANJO_HOO_1, SFX_55_BANJO_HOO_2,
                    SFX_56_BANJO_HUI}; // sfx_e
...
void func_802B7E6C(void) {
  ...
  baphysics_set_target_horizontal_velocity(
      ml_interpolate_f(sp1C, D_80364DC0, D_80364DC4));
}
...
void func_802B8110(void) { func_8029CF48(4, 1, 0.15f); }
...
int bswalrus_inSledSet(enum bs_e state) {
  return state == 0x81 || state == BS_7D_WALRUS_SLED ||
         state == BS_7E_WALRUS_SLED || state == 0x82 || state == 0x80 ||
         state == BS_9C_WALRUS_SLED_DRONE;
}

int bswalrus_inSet(enum bs_e state) {
  return state == BS_67_WALRUS_IDLE || state == BS_WALRUS_WALK ||
         state == BS_WALRUS_JUMP || state == BS_6A_WALRUS_FALL ||
         state == BS_WALRUS_OW || state == BS_WALRUS_DIE ||
         state == BS_95_WALRUS_DRONE || bswalrus_inSledSet(state);
}
...
void bswalrus_fall_update(void) {
  ...
  if (player_isStable()) {
    if (bastick_getZone() > 0 || (D_8037D5C8 == 2 && anctrl_isStopped(aCtrl))) {
...
void bswalrus_fall_end(void) { func_802B8048(); }
...
void bswalrus_ow_init(void) { __bswalrus_recoil_init(1); }

void bswalrus_ow_update(void) { __bswalrus_recoil_update(); }

void bswalrus_ow_end(void) { __bswalrus_recoil_end(); }

void bswalrus_bounce_init(void) { __bswalrus_recoil_init(2); }

void bswalrus_bounce_update(void) { __bswalrus_recoil_update(); }

void bswalrus_bounce_end(void) { __bswalrus_recoil_end(); }
...
void bswalrus_die_init(void) {
  ...
  func_80294980(sp20);
  ml_vec3f_yaw_between(sp20, sp2C, &sp38);
  D_8037D5C4 = 250.0f;
...
void bswalrus_drone_update(void) { bsdrone_update(); }
...
void bswalrus_sled_update(void) {
  ...
  if (50.0f < baphysics_get_horizontal_velocity()) {
    ...
    particleEmitter_emitN(createDustParticleEmitter(sp20), 1);
  } // L802B927C
...
void func_802B976C(void) { func_802B8048(); }
...
void bswalrus_sled_drone_update(void) { bsdrone_update(); }
