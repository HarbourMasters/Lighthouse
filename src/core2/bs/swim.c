...
s16 bsSwimAnimations[10] = {
    ...
    ASSET_57_ANIM_BSSWIM_IDLE,  ASSET_58_ANIM_BSSWIM_IDLE2,
    ASSET_57_ANIM_BSSWIM_IDLE,  ASSET_58_ANIM_BSSWIM_IDLE2,
    ASSET_57_ANIM_BSSWIM_IDLE,  ASSET_57_ANIM_BSSWIM_IDLE,
    ASSET_58_ANIM_BSSWIM_IDLE2, ASSET_58_ANIM_BSSWIM_IDLE2};
...
void func_802B5480(void) {
  ...
  if (func_80294574()) {
    ...
    particleEmitter_setParticleVelocityRange(bubble_emitter, -60.0f, -100.0f,
                                             -60.0f, 60.0f, 0.0f, 60.0f);
...
void func_802B55DC(void) {
  ...
  baphysics_set_target_horizontal_velocity(
      ml_interpolate_f(sp1C, bsSwimHorzVelocityMin, bsSwimHorzVelocityMax));
}
...
bool bsswim_inset(enum bs_e state_id) {
  return state_id == BS_2D_SWIM_IDLE || state_id == BS_2E_SWIM ||
         state_id == BS_4C_LANDING_IN_WATER || state_id == BS_77_SWIM_LOCKED ||
...
void func_802B5774(void) {
  ...
  if (bsbswim_inSet(prev_state) != 0) {
    if (prev_state == BS_54_SWIM_DIE) {
      basfx_playSfx(SFX_AF_BANJO_CATCHING_BREATH, 1.0f, 30000);
    } else {
      basfx_playSfx(SFX_AF_BANJO_CATCHING_BREATH, 1.0f,
                     (s32)ml_map_f(item_getCount(ITEM_17_AIR), 3600.0f, 0.0f,
                                   8000.0f, 30000.0f));
    }
    bainput_setDiveCooldown(4, 0.7f);
  }
  if ((anctrl_getIndex(anim_ctrl) == ASSET_57_ANIM_BSSWIM_IDLE) &&
      (prev_state != BS_4C_LANDING_IN_WATER)) {
...
void func_802B5AF8(void) { func_802B56D4(); }
...
void func_802B5B18(void) {
  ...
  baanim_setVelocityMapRanges(bsSwimHorzVelocityMin, bsSwimHorzVelocityMax,
                              bsSwimDurationMin, bsSwimDurationMax);
...
void func_802B5E10(void) { func_802B56D4(); }
...
void func_802B5E30(void) {
  ...
  if (func_80298850() != 0) {
    ...
    if (func_80298800(sp1C) && ml_vec3f_yaw_between(plyr_pos, sp1C, &sp34)) {
      yaw_setIdeal(sp34);
    }
  }
}
...
void func_802B5F38(void) { func_802B56D4(); }
...
void func_802B5F80(void) { bsdrone_update(); }
