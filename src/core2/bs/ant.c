...
u8 D_80364978 = 0;
s16 D_8036497C[3] = {SFX_54_BANJO_HOO_1, SFX_55_BANJO_HOO_2, SFX_56_BANJO_HUI};
...
void func_8029E48C(void) {
  ...
  if (bastick_getZone() == 0) {
    ...
    baphysics_set_target_horizontal_velocity(
        ml_interpolate_f(sp1C, D_80364960, D_80364964));
  }
}
...
int bsant_inSet(s32 move_indx) {
  return (move_indx == BS_35_ANT_IDLE) || (move_indx == BS_ANT_WALK) ||
         (move_indx == BS_ANT_JUMP) || (move_indx == BS_38_ANT_FALL) ||
         (move_indx == BS_3E_ANT_OW) || (move_indx == BS_43_ANT_DIE) ||
         (move_indx == 0x8e) || (move_indx == BS_92_ANT_DRONE);
}
...
void bsant_fall_end(void) { func_8029E4EC(); }
...
void bsant_ow_init(void) { __bsant_recoil_init(1); }

void bsant_ow_update(void) { __bsant_recoil_update(); }

void bsant_ow_end(void) { __bsant_recoil_end(); }

void bsant_bounce_init(void) { __bsant_recoil_init(2); }

void bsant_bounce_update(void) { __bsant_recoil_update(); }

void bsant_bounce_end(void) { __bsant_recoil_end(); }
...
void bsant_drone_update(void) { bsdrone_update(); }
