...
void func_802A524C(void) {
  ...
  if (!bastick_getZone())
    ...
    baphysics_set_target_horizontal_velocity(
        ml_interpolate_f(sp1C, D_80364A40, D_80364A44));
}

int bslongleg_inSet(s32 move_indx) {
  return (move_indx == BS_26_LONGLEG_IDLE) || (move_indx == BS_LONGLEG_WALK) ||
         (move_indx == BS_LONGLEG_JUMP) || (move_indx == BS_LONGLEG_EXIT) ||
...
void func_802A531C(void) {
  basfx_updateClockSfxSource(stateTimer_getPrevious(STATE_TIMER_2_LONGLEG),
                             stateTimer_get(STATE_TIMER_2_LONGLEG));
...
void bsblongleg_stand_end(void) { func_802A5404(); }
...
void bsblongleg_walk_end(void) { func_802A5404(); }
...
void bsblongleg_jump_update(void) {
  ...
  case 1: // L802A5FA8
    ...
    break;
  case 2: // L802A6020
    anctrl_setAnimTimer(aCtrl,
                        ml_map_f(sp30, D_8037D354, 1.0f, D_8037D350, 0.6703f));
    func_80299594(1, 0.5f);
    if (player_isStable()) {
      func_8029C5E8();
...
void bsblongleg_slide_end(void) { func_802A5404(); }

void func_802A6388(f32 arg0) { D_8037D35C = arg0; }
