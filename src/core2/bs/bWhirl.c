...
void func_802AA400(void) {
  ...
  if (!bastick_getZone()) {
    ...
    baphysics_set_target_horizontal_velocity(
        ml_interpolate_f(sp1C, D_80364AD0, D_80364AD4));
  }
}

static void __bsbwhirl_end(void) {
  enum bs_e state = bs_getNextState();
  if (!(state == BS_1B_WONDERWING_IDLE || state == BS_1C_WONDERWING_WALK ||
        state == BS_1D_WONDERWING_JUMP || state == BS_1E_WONDERWING_EXIT ||
        state == BS_A4_WONDERWING_DRONE || state == BS_A5_WONDERWING_UNKA5)) {
    bastick_resetZones();
    func_8029E070(0);
    comusic_updateTrackWithArgs(-1, 0xfa0, 0xd);
    map_worthlessCounter(TRUE);
    comusic_stopTrack(COMUSIC_25_USING_GOLD_FEATHERS);
  }
  baanim_setUpdateType(BAANIM_UPDATE_1_NORMAL);
}

static void __bsbwhirl_spawnSparkle(void) { func_8033E3F0(2, 1); }
...
void bsbwhirl_enter_init(void) {
  ...
  comusic_updateTrackWithArgs(0, 0xfa0, 0xd);
  comusic_playTrackWithVolumeOverride(0x25, 0x6d60);
...
void bsbwhirl_stand_end(void) { __bsbwhirl_end(); }
...
void bsbwhirl_walk_end(void) { __bsbwhirl_end(); }
...
void bsbwhirl_exit_init(void) {
  ...
  func_8029957C(2);
  baphysics_set_target_horizontal_velocity(0.0f);
  comusic_fadeTrackWithArgsNoDelay(COMUSIC_25_USING_GOLD_FEATHERS, 0.0f, 0xFA0);
}
...
void bsbwhirl_exit_end(void) { __bsbwhirl_end(); }

void bsbwhirl_drone_init(void) { bsdrone_init(); }
