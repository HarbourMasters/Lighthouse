...
void bakey_update(void) {
  ...
  pfsManager_getSideButtonState(0, &bakey.pressed_count[BUTTON_Z]);
  pfsManager_getUnknownButtonState(0, &bakey.pressed_count[BUTTON_D_UP]);
  ...
  for (i = 0; i < 0xE; i++) { // L802954A8
    ...
    bakey.released_count[i] =
        (bakey.pressed_count[i]) ? 0 : bakey.released_count[i] + 1;
  }
...
u32 bakey_held(s32 button_indx) { return bakey.pressed_count[button_indx]; }
