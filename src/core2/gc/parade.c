...
enum parade_e { PARADE_0_POST_FURNACE_FUN, PARADE_1_POST_GRUNTY_BATTLE };
...
void gcparade_8031AC8C(void) {
  ...
  if (D_803830F0.parade_element->exit < 0) {
    func_8034BA7C(D_803830F0.parade_element->map,
                  0x5A - D_803830F0.parade_element->exit);
  } else {
    game_setMapWithoutTransition(D_803830F0.parade_element->map, 0x65);
    game_setTransitionWithArgs(1, 8);
    game_setTransition(9);
  }
  D_803830F0.unk5 = 1;
}
...
void gcparade_setState(enum parade_state_e next_state) {
  ...
  case PARADE_STATE_5_WAIT_ON_NAME:
    if ((D_803830F0.parade_id == 1) && (D_803830F0.parade_element->exit >= 0)) {
      timedFunc_set_1(2.0f, (GenFunction_1)gcparade_setState,
                      PARADE_STATE_6_NAME_DISAPPEAR); // queue state transition
    }
    break;
  case PARADE_STATE_7_GOTO_NEXT:
    ...
    timedFunc_set_1(0.25f, (GenFunction_1)gcparade_setState,
                    (D_803830F0.indx == D_803830F0.count)
                        ? PARADE_STATE_8_END
                        : PARADE_STATE_3_WARP);
    break;
  case PARADE_STATE_8_END:
    ...
    game_setTransitionWithArgs(1, 8);
    game_setTransition(0xA);
    if (D_803830F0.parade_id == 0) {
      game_setMapWithoutTransition(MAP_94_CS_INTRO_SPIRAL_7, 0);
    } else if (D_803830F0.jiggyscore < 100) {
      game_setMapWithoutTransition(MAP_20_CS_END_NOT_100, 0);
    } else {
      game_setMapWithoutTransition(MAP_95_CS_END_ALL_100, 0);
    }
    comusic_fadeTrackWithArgsNoDelay(COMUSIC_8E_CREDITS, 0, 1000);
    comusic_stopTrackById(COMUSIC_8E_CREDITS);
    break;
  default:
    break;
  }
...
void func_8031B010(void) { gcparade_setState(PARADE_STATE_6_NAME_DISAPPEAR); }

void gcparade_textCallback(ActorMarker *caller, enum asset_e text_id,
                           s32 arg2) {
  if (D_803830F0.parade_element->exit >= 0) {
      ...
      if (D_803830F0.unk5 == 0) {
        if (map_get() == D_803830F0.parade_element->map) {
          if (D_803830F0.parade_id == 0) {
            ...
            showDelayedTextAtTime(1.0f, D_803830F0.indx + 0x11AF, sp34, NULL, NULL,
                          gcparade_textCallback, NULL);
            timedFunc_set_1(1.0f, (GenFunction_1)func_80311714, 1);
          } else if (D_803830F0.parade_element->exit >= 0) {
            func_8028F918(2);
          }
          timedFunc_set_1(1.0f, (GenFunction_1)gcparade_setState,
                          PARADE_STATE_4_NAME_APPEAR);
          D_803830F0.unk5 = 1;
        }
      }
...
void gcparade_start(void) {
  ...
  if (volatileFlag_getAndSet(VOLATILE_FLAG_C0_BEGIN_FINAL_CHARACTER_PARADE,
                             FALSE))
    ...
    if ((D_803830F0.state == 3)) {
      ...
      if (D_803830F0.parade_element->exit >= 0) {
        ...
        if (D_803830F0.parade_element->unk8 > 0) {
          temp_v0 = findNodePropsByActorIds(&D_8036DDAC, &D_80383100, 0x14);
          for (var_s0 = 0; (var_s0 < temp_v0) && !var_s2; var_s0++) {
            if (nodeprop_getRadius(D_80383100[var_s0]) ==
                D_803830F0.parade_element->unk8) {
...
int gcparade_8031B4CC(void) {
  return ((D_803830F0.parade_element != NULL) &&
          (D_803830F0.parade_element->exit < 0));
}

int gcparade_8031B4F4(void) {
  return (D_803830F0.parade_element != NULL) ? D_803830F0.parade_element->unk8
                                             : 0;
}
