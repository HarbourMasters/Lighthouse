...
void gcdialog_init(void) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    g_Dialog.unk148[i].unk4[0] = g_Dialog.unk148[i].unk4[1] =
        g_Dialog.unk148[i].unk4[2] = 0;
  }

  g_Dialog.state = 0;
  g_Dialog.unk128_31 = g_Dialog.unk128_5 = FALSE;
  g_Dialog.unk12C_31 = g_Dialog.unk12C_29 = g_Dialog.unk12C_27 =
...
void func_8030F1D0(void) {
  if (func_803114B0()) {
    comusic_updateTrackWithArgs(-1, 300, 2);
  }
  func_803114D0();
  func_8030F130();
  g_Dialog.state = 0;
...
void func_8030F218(char *next_state, char *arg1, char *arg2, bool arg3,
                   bool arg4) {
  s32 var_v0;
  s32 i;
  s32 j;
...
void func_8030F338(void) {
  ...
  comusic_updateTrackWithArgs(-1, 0x12c, 2);
  func_8030F130();
}
  ...
  if (g_Dialog.state != v0 || next_state != v0) {
    ...
    case 6:
      for (i = 0; i < 2; i++) { // L8030F59C
        for (j = g_Dialog.string_index[i];
             g_Dialog.string_list[i][j].cmd < -4 ||
             g_Dialog.string_list[i][j].cmd >= 0;
             j++) {
          if (g_Dialog.string_list[i][j].cmd == -7 && g_Dialog.unk140) {
            if (g_Dialog.caller == NULL) {
              g_Dialog.unk140(g_Dialog.caller, g_Dialog.unk130,
                              *g_Dialog.string_list[i][j].str);
            } else {
              if (func_8030EDC0(g_Dialog.caller, g_Dialog.unk138)) {
                g_Dialog.unk140(g_Dialog.caller, g_Dialog.unk130,
                                *g_Dialog.string_list[i][j].str);
...
void func_8030F754(GcZoomboxSprite portrait_id, s32 arg1) {
  ...
  case 4: // L8030F7CC
    ...
    temp_v0 = ((g_Dialog.unk11A[0].unk0_7) ? 1 : 0) +
              ((g_Dialog.unk11A[1].unk0_7) ? 1 : 0);
...
void gcdialog_update(void) {
  ...
  if (g_Dialog.u8.unk128_31 & 0x80) {
    ...
    pfsManager_getFirstControllerSideButtonState(0, controller_side_buttons);
  } else {
    controller_copyFaceButtons(0, controller_face_buttons);
    pfsManager_getSideButtonState(0, controller_side_buttons);
  }
  ...
  case 2:
    if (g_Dialog.zoombox[g_Dialog.u8.active_zoombox] == NULL ||
        func_80318BEC(g_Dialog.zoombox[g_Dialog.u8.active_zoombox]) ||
        g_Dialog.unk11A[g_Dialog.u8.active_zoombox].unk0_5) {
      g_Dialog.u8.active_zoombox ^= 1;
    } else {
      g_Dialog.string_cmd[g_Dialog.u8.active_zoombox] =
          CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox])->cmd;
      g_Dialog.string[g_Dialog.u8.active_zoombox] =
      ...
      case -8: // Conditional text
        do {
          ...
          g_Dialog.string_cmd[g_Dialog.u8.active_zoombox] =
              CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox])->cmd;
        } while (g_Dialog.string_cmd[g_Dialog.u8.active_zoombox] == -8 ||
                 g_Dialog.string_cmd[g_Dialog.u8.active_zoombox] == -9);
        break;

      case -6: // Conditional minimize
        for (spA8 = g_Dialog.string_index[g_Dialog.u8.active_zoombox];
             CMD2(spA8)->cmd < -4; spA8++)
          ;

        if (CMD2(spA8)->cmd >= 0) {
          gczoombox_loadSprite(g_Dialog.zoombox[g_Dialog.u8.active_zoombox],
                               CMD2(spA8)->cmd + 0xC);
        }
      ...
      default:
        if (!gczoombox_strlen(
          ...
          if (CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox] + 1)->cmd ==
              -8) {
            // Conditional text - use callback to determine if text should be
            // shown
            if (g_Dialog.unk144 != NULL) {
              ...
              if (g_Dialog.caller == NULL) {
                ret = g_Dialog.unk144(
                    g_Dialog.caller, g_Dialog.unk130,
                    g_Dialog.string_index[g_Dialog.u8.active_zoombox]);
              } else if (func_8030EDC0(g_Dialog.caller, g_Dialog.unk138)) {
                ret = g_Dialog.unk144(
                    g_Dialog.caller, g_Dialog.unk130,
              ...
              if (ret >= 0 &&
                  CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox] + ret +
                      1)
                          ->cmd == -8) {
                bk_strlen(CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox] +
                           ret + 1)
                           ->str);
                bk_strlen(CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox])
                           ->str);
                ...
                func_8030F218(
                    g_Dialog.output,
                    CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox])->str,
                    CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox] +
                        ret + 1)
                        ->str,
                    func_8031B604(
                        CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox])
          ...
          } else if (CMD(g_Dialog.string_index[g_Dialog.u8.active_zoombox] + 1)
                         ->cmd == -9) {
          ...
          if (gczoombox_loadSprite(
                  g_Dialog.zoombox[g_Dialog.u8.active_zoombox],
                  g_Dialog.string_cmd[g_Dialog.u8.active_zoombox] + 12)) {
            gczoombox_minimize(g_Dialog.zoombox[g_Dialog.u8.active_zoombox]);
            g_Dialog.unk128_6 = TRUE;
          }
          ...
          if (ret == -1) {
            for (spA8 = g_Dialog.string_index[g_Dialog.u8.active_zoombox];
                 CMD(spA8)->cmd ==
                     g_Dialog.string_cmd[g_Dialog.u8.active_zoombox] &&
                 spA8 - g_Dialog.string_index[g_Dialog.u8.active_zoombox] < 8;
                 spA8++) {
              sp4C[spA8 - g_Dialog.string_index[g_Dialog.u8.active_zoombox]] =
                  CMD(spA8)->str;
            }

            gczoombox_setStrings(
                g_Dialog.zoombox[g_Dialog.u8.active_zoombox],
                spA8 - g_Dialog.string_index[g_Dialog.u8.active_zoombox], sp4C);
            g_Dialog.string_index[g_Dialog.u8.active_zoombox] = spA8;
          } else {
            func_803183A4(g_Dialog.zoombox[g_Dialog.u8.active_zoombox],
                          g_Dialog.output);
            g_Dialog.string_index[g_Dialog.u8.active_zoombox]++;
          }
  ...
  case 3:
    if ((g_Dialog.u8.unk128_31 & 1) &&
        controller_face_buttons[FACE_BUTTON(BUTTON_B)] == 1u) {
      gcdialog_setState(6);
      break;
    }
    ...
    if (NOT((g_Dialog.u8.unk128_31 & 0x80)
                ? pfsManager_getButtonPressCount(0, 4)
                : pfsManager_getButtonPressCount(0, 3))) {
      break;
    }
  ...
  case 4: // Waiting for player to make choice
    ...
    if (g_Dialog.u8.unk12C_23 != -1) {
      gcdialog_setState(g_Dialog.unk11A[g_Dialog.u8.active_zoombox].unk0_7 ? 8
                                                                           : 5);
    }
    break;
...
void func_80310574(s32 text_id) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    g_Dialog.string_list[i] =
        (struct13s *)heap_malloc(g_Dialog.string_count[i] * sizeof(struct13s));
...
s32 func_8031068C(s32 next_state) { return (next_state) ? 0 : 0xA0; }

int func_803106A4(s32 next_state) { return (next_state) ? 1 : 0; }

void func_803106BC(s32 text_id, s32 arg1, ActorMarker *marker,
                   void (*callback)(ActorMarker *, s32, s32),
                   void (*arg4)(ActorMarker *, s32, s32), s32 arg5) {
  s32 i;
  s32 j;
  ...
  for (j = 0; j < 2; j++) { // L80310774
    ...
    while (g_Dialog.string_list[j][i].cmd < -4 &&
           i < g_Dialog.string_count[j]) {
      i++;
    };
    ...
    if (g_Dialog.string_list[j][i].cmd >= 0) {
      if (!g_Dialog.unk11A[j].unk0_7) {
        g_Dialog.zoombox[j] = gczoombox_new(
            g_Dialog.unk124[j], g_Dialog.string_list[j][i].cmd + 0xC, 0,
            func_803106A4(j), (void *)func_8030F754);
        if (j == 1) {
          func_80347A14(0);
        }
      } else { // L80310860
  ...
  g_Dialog.unk138 =
      (marker != NULL) ? ((marker->unk5C) ? marker->unk5C : -1) : 0;
  gcdialog_setState(((game_isSpecialMode() ||
                      volatileFlag_get(VOLATILE_FLAG_1F_IN_CHARACTER_PARADE)) &&
                     g_Dialog.unk128_3)
                        ? 6
                        : 1);
...
void func_80310A5C(s32 next_state, s32 arg1, s32 arg2, s32 arg3, s32 arg4) {
  ...
  for (i = 0, tmpf = 0.4f; i < 2; i++) {
    if (g_Dialog.zoombox[i]) {
      gczoombox_func_803184C8(g_Dialog.zoombox[i], arg2, next_state, arg1, tmpf,
                              arg3, arg4);
    }
  }
}

void func_80310B1C(s32 text_id, s32 arg1, ActorMarker *marker,
                   void (*callback)(ActorMarker *, s32, s32),
                   void (*arg4)(ActorMarker *, s32, s32), s32 arg5) {
...
void func_80310BB4(s32 next_state, s32 arg1, s32 arg2) {
  func_80310A5C(arg1, arg2, next_state, g_Dialog.unk128_31 & 2,
                g_Dialog.unk128_31 & 0x80);
}
...
void func_80310D2C(void) {
  ...
  if (game_getMode() == GAME_MODE_3_NORMAL || game_isSpecialMode()) {
    ...
    if (!func_803114B0() && (s32)(g_Dialog.unk12C_15) > 0) {
      ...
      func_80310B1C(sp24->unk0, sp24->unk2, sp24->unk10, sp24->unk18,
                    sp24->unk1C, sp24->unk20);

      g_Dialog.unk138 = sp24->unk14;
      comusic_updateTrackWithArgs(8000, 300, 2);
      if ((sp24->unk2 & 0x8) &&
          !((game_isSpecialMode() ||
             volatileFlag_get(VOLATILE_FLAG_1F_IN_CHARACTER_PARADE)) &&
            g_Dialog.unk128_3)) { // L80310E6C
        func_8028F918(0);
        if (0.0f == sp24->unk4_x && 0.0f == sp24->unk4_y &&
            0.0f == sp24->unk4_z) {
    ...
    if ((g_Dialog.state != 0 && g_Dialog.state != 5 && g_Dialog.state != 7) ||
        ((!g_Dialog.state &&
          (g_Dialog.unk11A[0].unk0_7 || g_Dialog.unk11A[1].unk0_7))) ||
        g_Dialog.unk12C_15) {
      // L80310FF0
      if (func_802FADD4(0)) {
...
int func_803110F8(s32 next_state, s32 arg1, s32 arg2, s32 arg3,
                  void (*arg4)(ActorMarker *, enum asset_e, s32)) {
  comusic_updateTrackWithArgs(15000, 300, 2);
  func_80311174(next_state + 0xe57, 0x84, NULL, NULL, NULL, NULL, arg4);
  func_80310A5C(arg2, arg3, arg1, 0, 0);
  return 1;
}

int func_80311174(s32 text_id, s32 arg1, f32 *pos, ActorMarker *marker,
                  void (*callback)(ActorMarker *, enum asset_e, s32),
                  void (*arg5)(ActorMarker *, enum asset_e, s32), s32 arg6) {
  f32 pad;
  s32 temp_v1;

  if (volatileFlag_get(VOLATILE_FLAG_1) || isInTransitionMap())
    return 0;
  ...
  if (!func_803114B0()) {
    ...
    comusic_updateTrackWithArgs(0x1f40, 0x12c, 2);
    return 1;
  } else { // L803112A0
    if (arg1 & 0x20) {
...
int func_803114B0(void) { return (g_Dialog.unk130 + 1) != 0; }

int func_803114C4(void) { return g_Dialog.unk130; }
...
void func_80311714(int next_state) { g_Dialog.unk128_3 = next_state; }
