...
MapSkyInfo D_8036BD40[] = {
    ...
    {MAP_99_CS_END_SPIRAL_MOUNTAIN_2,
     {{ASSET_7C4_MODEL_SKYBOX_SM, 1.0f, 0.0f}}},
    {MAP_98_CS_END_SPIRAL_MOUNTAIN_1,
     {{ASSET_7C4_MODEL_SKYBOX_SM, 1.0f, 0.0f}}},
    {MAP_95_CS_END_ALL_100,
     {{ASSET_7CD_MODEL_SKYBOX_BEACH_ENDING, 1.0f, 0.0f}}},
    ...
    {MAP_87_CS_SPIRAL_MOUNTAIN_5,
     {{ASSET_7CC_MODEL_SKYBOX_GRUNTYS_FALL, 1.0f, 0.0f}}},
    ...
    {MAP_75_GL_MMM_LOBBY,
     {{ASSET_7CB_MODEL_CLOUDS_LAIR_MMM, 1.0f, 0.5f},
      {ASSET_7CA_MODEL_SKYBOX_LAIR_MMM, 1.0f, 6.0f}}},
    {MAP_1F_CS_START_RAREWARE,
     {{ASSET_7C9_MODEL_SKYBOX_INTRO_N64, 1.0f, 0.0f}}},
    {MAP_7_TTC_TREASURE_TROVE_COVE,
     {{ASSET_7BF_MODEL_SKYBOX_TTC, 1.0f, 0.0f},
      {ASSET_7C0_MODEL_CLOUDS_TTC, 2.0f, 0.5f}}},
    {MAP_12_GV_GOBIS_VALLEY,
     {{ASSET_7C1_MODEL_SKYBOX_GV, 1.0f, 0.0f}, {0x000, 1.0f, 1.0f}}},
    {MAP_1B_MMM_MAD_MONSTER_MANSION,
     {{ASSET_7C2_MODEL_SKYBOX_MMM, 1.0f, 0.0f},
      {ASSET_7C3_MODEL_SKYBOX_MMM, 1.0f, 0.0f}}},
    {MAP_2_MM_MUMBOS_MOUNTAIN,
     {{ASSET_7BD_MODEL_SKYBOX_MM, 1.0f, 0.0f},
      {ASSET_7BE_MODEL_CLOUDS_MM, 1.0f, 1.0f}}},
    ...
    {MAP_27_FP_FREEZEEZY_PEAK,
     {{ASSET_7C6_MODEL_SKYBOX_FP, 1.0f, 1.0f},
      {ASSET_7C7_MODEL_CLOUDS_FP_A, 1.0f, 1.5f},
      {ASSET_7C8_MODEL_CLOUDS_FP_B, 1.0f, 3.0f}}},
...
void sky_draw(Gfx **gfx, Mtx **mtx, Vtx **vtx) {
  ...
  if (gcSky.model_bins[0]) {
    drawRectangle2D(gfx, 0, 0, (s32)(f32)gFramebufferWidth,
                    (s32)(f32)gFramebufferHeight, 0, 0,
                    0); // fill screen with black
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (iAsset) {
        ...
        rotation[2] = 0.0f;
        modelRender_draw(gfx, mtx, position, rotation,
                         gcSky.sky_info->sky_list[i].scale, NULL, iAsset);
      }
    }
  } else { // L8030B200
    drawRectangle2D(gfx, 0, 0, (s32)(f32)gFramebufferWidth,
...
void sky_reset(void) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (gcSky.sky_info->sky_list[i].model_id) {
      gcSky.model_bins[i] =
          assetcache_get(gcSky.sky_info->sky_list[i].model_id);
      if (func_8033A0B0(gcSky.model_bins[i])) {
        gcSky.model[i] = func_8033F5F8(func_8033A0B0(gcSky.model_bins[i]),
                                       model_getVtxList(gcSky.model_bins[i]));
...
void sky_update(void) { gcSky.timer += time_getDelta(); }
