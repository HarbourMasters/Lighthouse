...
extern void game_setTransitionWithArgs(s32, s32);
...
extern void loadFontTextures2(enum asset_e);
extern void initializeFontTextures(void);
extern void func_802FACA4(enum item_e);
extern void assetCache_releaseBKModelBin(void *);
...
struct1Bs D_8036C560[] = {
    {MAP_69_GL_MM_LOBBY, 0x2},  {MAP_6D_GL_TTC_LOBBY, 0x4},
    {MAP_70_GL_CC_LOBBY, 0x2},  {MAP_72_GL_BGS_LOBBY, 0x2},
    {MAP_6F_GL_FP_LOBBY, 0x6},  {-1, -1},
    {MAP_6E_GL_GV_LOBBY, 0x3},  {MAP_79_GL_CCW_LOBBY, 0x6},
    {MAP_77_GL_RBB_LOBBY, 0x2}, {MAP_75_GL_MMM_LOBBY, 0x2},
...
u8 D_8036C604[0x7] = {
    ITEM_14_HEALTH,       ITEM_26_JIGGY_TOTAL, ITEM_25_MUMBO_TOKEN_TOTAL,
    ITEM_10_GOLD_FEATHER, ITEM_F_RED_FEATHER,  ITEM_D_EGGS,
    ITEM_C_NOTE};
u8 D_8036C60C[0xC] = {0x28,
                      0x29,
                      0x00,
...
struct {
  ...
  GcZoombox *zoombox[4];
  f32 unk20;
  BKSprite_s *joystick_sprite;
  f32 unk28;
  BKSprite_s *b_button_sprite;
  u8 b_button_frame;
...
void gcpausemenu_zoomboxes_initMainMenu(void) {
  s32 i;
  for (i = 0; i < 4; i++) {
    D_80383010.zoombox[i] =
        gczoombox_new(D_8036C4E0[i].y, D_8036C4E0[i].portrait, 2, 0,
                      gcpausemenu_zoombox_callback);
...
void gcpausemenu_zoomboxes_initTotalsMenu(void) {
  ...
  for (i = 0; i < 4; i++) {
    D_80383010.zoombox[i] =
        gczoombox_new(D_8036C520[i].y, D_8036C520[i].portrait, (i == 3) ? 1 : 2,
                      0, gcpausemenu_zoombox_callback);
...
void gcpausemenu_getLevelNoteScore(enum level_e level, s32 *valPtr,
                                   s32 *maxPtr) {
  *valPtr = itemscore_noteScores_get(level);
  *maxPtr = 100;
}

void gcpausemenu_getLevelJiggyScore(enum level_e level, s32 *valPtr,
                                    s32 *maxPtr) {
  *valPtr = jiggyscore_leveltotal(level);
  *maxPtr = 10;
}

void gcpausemenu_getLevelHoneycombScore(enum level_e level, s32 *valPtr,
                                        s32 *maxPtr) {
...
void gcpausemenu_printLevelTotals(enum level_e level) {
  ...
  bk_strcat(D_8036C520[3].str,
         gcpausemenu_TimeToA(itemscore_timeScores_get(level)));
}
...
void gcpausemenu_getTotalJiggyScore(s32 *dst) { *dst = jiggyscore_total(); }
...
void gcpausemenu_printTotals(void) {
  ...
  bk_strcat(D_8036C520[3].str,
         gcpausemenu_TimeToA(itemscore_timeScores_getTotal()));
}
...
void gcPauseMenu_setState(enum gcpausemenu_state_e next_state) {
  ...
  case PAUSE_STATE_0_MENU_INIT:
    ...
    D_80383010.left_joystick_visible = D_80383010.right_joystick_visible =
        D_80383010.exit_pause = D_80383010.selection =
  ...
  case PAUSE_STATE_12_SNS_DISPOSE: /* 8B978 80312908 3C128038 */
    D_80383010.selection = D_80383010.page;
    assetCache_releaseBKModelBin(&D_80383010.sns_egg_model); // free
    assetCache_releaseBKModelBin(&D_80383010.ice_key_model); // free
    break;
...
void gcpausemenu_zoombox_callback(s32 portrait_id, s32 zoombox_state) {
  ...
  if ((zoombox_state == 2) && (D_80383010.menu == PAUSE_MENU_0_MAIN) &&
...
bool gcpausemenu_initReturnToLair(void) { return TRUE; }
...
void gcpausemenu_init(void) {
  ...
  D_80383010.sns_items =
      sp38 + sp34 + sp30 + sp2C + sp28 + sp24 + sns_get_item_state(7, 0);
  D_80383010.return_to_lair_disabled = gcpausemenu_initReturnToLair();
  func_80311604();
  gcpausemenu_zoomboxes_initMainMenu();
  D_80383010.joystick_sprite = assetcache_get(0x7EB);
  D_80383010.joystick_frame_count =
      sprite_getFrameCount(D_80383010.joystick_sprite);
  D_80383010.left_joystick_alpha = temp_v1 = D_80383010.right_joystick_alpha =
      0;
  D_80383010.joystick_frame = D_80383010.unk3_4 = temp_v1;

  D_80383010.b_button_sprite = assetcache_get(0x7ED);
  D_80383010.b_button_frame_count =
      sprite_getFrameCount(D_80383010.b_button_sprite);
  ...
  D_80383010.sns_alpha = D_80383010.sns_visible = D_80383010.unk70_31 =
...
s32 gcpausemenu_80312D78(struct1As *arg0, s32 arg1) {
  ...
  for (var_s2 = 0; var_s2 < arg1; var_s2++) {
    if (arg0[var_s2].delay <= D_80383010.unkC) {
      if (!arg0[var_s2].unkF) {
        var_v0 =
            BOOL(func_803183A4(D_80383010.zoombox[var_s2], arg0[var_s2].str));
...
void gcpausemenu_80312E80(struct1As *arg0, s32 arg1) {
  ...
  for (var_s1 = 0; var_s1 < arg1; var_s1++) {
    var_f0 = ((var_s1 == D_80383010.selection) &&
              (D_80383010.menu == PAUSE_MENU_0_MAIN))
                 ? 0.2
                 : arg0[var_s1].unk4;
...
void gcpausemenu_setNextPage(s32 increment) {
  ...
  } while ((D_80383010.page != 0) &&
           itemscore_timeScores_get(D_8036C58C[D_80383010.page].level_id) == 0);

  gcPauseMenu_setState(D_80383010.page == 12 ? PAUSE_STATE_A_TOTALS_CLOSING
                                             : PAUSE_STATE_C_PAGE_CLOSING);
  D_80383010.unk7 = 6;
}
...
void gcpausemenu_updateBButtonAndJoystickSprites(void) {
  ...
  D_80383010.right_joystick_visible =
      (gcpausemenu_getMaxPage() == D_80383010.selection) ? FALSE : TRUE;
...
s32 gcPauseMenu_update(void) {
  ...
  pfsManager_getSideButtonState(0, sp60);
  pfsManager_getUnknownButtonState(0, sp50);
  func_80310D2C();
  ...
  case PAUSE_STATE_1_MENU_OPENING: // opening
    if (gcpausemenu_80312D78(D_8036C4E0, 4) == 4) {
      if (((D_80383010.return_to_lair_disabled) ? 3 : 4) ==
          D_80383010.zoombox_opening_count) {
        D_80383010.zoombox_opening_count = 0;
    ...
    if (controller_getStartButton(0) == 1) {
      ...
      if (0.75 < joystick[JOYSTICK_Y]) {
        if ((s32)D_80383010.selection > 0) {
          ...
          if (D_80383010.return_to_lair_disabled &&
              D_80383010.selection == PAUSE_SELECTION_1_EXIT_TO_WITCH_S_LAIR) {
            D_80383010.selection--;
          }
  ...
  case PAUSE_STATE_4_SELECTION_PROCESSING:
    ...
    case PAUSE_SELECTION_1_EXIT_TO_WITCH_S_LAIR: // L80313908 //return to lair
      ...
      if (map_get() == MAP_8E_GL_FURNACE_FUN) {
        ...
        game_setMapWithTransition(MAP_80_GL_FF_ENTRANCE, 2, 1);
      } else {
        game_setMapWithTransition(D_8036C560[level - 1].map, D_8036C560[level - 1].exit, 1);
      }
  ...
  case PAUSE_STATE_5_SELECTION_CONFIRMATION:
    if (3.0 < D_80383010.unkC) {
      ...
      if (D_8036C4E0[D_80383010.selection].unkF = func_803183A4(
              D_80383010.zoombox[D_80383010.selection],
              (D_80383010.unk3_6) ? "ARE YOU SURE?" : "A - YES, B - NO")) {
        D_80383010.unkC = 0.0;
      }
    } // L80313AF4
    ...
    if (controller_getStartButton(0) == 1) {
      ...
      D_80383010.zoombox_opening_count =
          (D_80383010.return_to_lair_disabled) ? 3 : 4;
  ...
  case PAUSE_STATE_B_TOTALS_DISPOSE:
    if (D_80383010.menu == PAUSE_MENU_3_EXIT) {
      ...
      gcPauseMenu_setState(D_80383010.page == 12 ? PAUSE_STATE_E_SNS_INIT
                                                 : PAUSE_STATE_0_MENU_INIT);
    }
    break;
  ...
  case PAUSE_STATE_12_SNS_DISPOSE:
    if (D_80383010.menu == PAUSE_MENU_3_EXIT) {
      ...
      gcPauseMenu_setState((D_80383010.page != -1) ? PAUSE_STATE_7_TOTALS_INIT
                                                   : PAUSE_STATE_0_MENU_INIT);
    }
    break;
...
void __gcpausemenu_drawSprite(Gfx **gdl, Mtx **mptr, Vtx **vptr,
                              BKSprite_s *sprite, s32 frame, f32 x, f32 y, f32 w,
                              f32 h, u8 a) {
  f32 sp5C[3];
  f32 sp50[3];
  f32 sp44[3];
...
void gcpausemenu_drawSprite(Gfx **gdl, Mtx **mptr, Vtx **vptr, BKSprite_s *sprite,
                            s32 frame, f32 x, f32 y, s32 mirror, u8 a) {
  BKSpriteFrame *_frame;
  s32 w;
  s32 h;
  _frame = sprite_getFramePtr(sprite, frame);
  ...
  h = _frame->h;
  __gcpausemenu_drawSprite(gdl, mptr, vptr, sprite, frame, (x - w * 0.5),
                           (y - h * 0.5), w, h, a);
}
...
void gcpausemenu_draw(Gfx **gfx, Mtx **mtx, Vtx **vtx) {
  ...
  if (game_getMode() != GAME_MODE_4_PAUSED) {
    if (!D_8036C620) {
      finalizeFramebuffer(gfx, mtx, vtx);
    }
    D_8036C620 = TRUE;
    return;
  }
  ...
  if (D_8036C620) {
    ...
    initializeFramebuffer(gfx, mtx, vtx);
    D_8036C620 = FALSE;
  } else {
    updateFramebuffer(gfx, mtx, vtx);
  }
  ...
  gcpausemenu_drawSprite(gfx, mtx, vtx, D_80383010.joystick_sprite,
                         D_80383010.joystick_frame, 30.0f, 196.0f, 1,
                         (s32)D_80383010.left_joystick_alpha);
  gcpausemenu_drawSprite(gfx, mtx, vtx, D_80383010.joystick_sprite,
                         D_80383010.joystick_frame,
                         (f32)(gFramebufferWidth - 0x1E), 196.0f, 0,
                         (s32)D_80383010.right_joystick_alpha);
  var_a0 =
      ((*((u32 *)&D_80383010.state) << 0x1c) >> 0x1f); // left_joystick_visible
  if (var_a0 != 0) {
    if (D_80383010.left_joystick_alpha < 0xFF) {
      D_80383010.left_joystick_alpha =
          (D_80383010.left_joystick_alpha + 0xC < 0xFF)
              ? D_80383010.left_joystick_alpha + 0xC
              : 0xFF;
    }
  }
  var_a0 =
  ...
  if (var_a0 == 0) {
    if (D_80383010.left_joystick_alpha > 0) {
      D_80383010.left_joystick_alpha =
          (D_80383010.left_joystick_alpha - 0xC > 0)
              ? D_80383010.left_joystick_alpha - 0xC
              : 0;
    }
  }
  if (((*((u32 *)&D_80383010.state) << 0x1d) >> 0x1f) !=
      0) { // right_joystick_visible
    if (D_80383010.right_joystick_alpha < 0xFF) {
      D_80383010.right_joystick_alpha =
          (D_80383010.right_joystick_alpha + 0xC < 0xFF)
              ? D_80383010.right_joystick_alpha + 0xC
              : 0xFF;
    }
  }
  if (((*((u32 *)&D_80383010.state) << 0x1d) >> 0x1f) ==
    ...
    if (D_80383010.right_joystick_alpha > 0) {
      D_80383010.right_joystick_alpha =
          (D_80383010.right_joystick_alpha - 0xC > 0)
              ? D_80383010.right_joystick_alpha - 0xC
              : 0;
    }
  }
  gcpausemenu_drawSprite(
      gfx, mtx, vtx, D_80383010.b_button_sprite, (s32)D_80383010.b_button_frame,
      gFramebufferWidth * 0.5, 196.0f, 0, (s32)D_80383010.b_button_alpha);
  var_a0 = ((*((u32 *)&D_80383010.state) << 0x1e) >> 0x1f); // b_button_visible
  if (var_a0 != 0) {
    if (D_80383010.b_button_alpha < 0xFF) {
      D_80383010.b_button_alpha = (D_80383010.b_button_alpha + 0xC < 0xFF)
                                      ? D_80383010.b_button_alpha + 0xC
                                      : 0xFF;
    }
  }
  ...
  if (var_a0 == 0) { // b_button_visible
    if (D_80383010.b_button_alpha > 0) {
      D_80383010.b_button_alpha = (D_80383010.b_button_alpha - 0xC > 0)
                                      ? D_80383010.b_button_alpha - 0xC
                                      : 0;
    }
  }
  ...
  if (D_80383010.sns_alpha != 0) {
    ...
    for (i = 1; i < 7; i++) {
      if (sns_get_item_state(i, 0)) {
        ...
        modelRender_draw(gfx, mtx, sp98, sp8C, 0.8f, sp80,
                         D_80383010.sns_egg_model);
  ...
  if (D_80383010.sns_visible != FALSE) {
    if (D_80383010.sns_alpha < 0xFF) {
      D_80383010.sns_alpha = (D_80383010.sns_alpha + 0xC < 0xFF)
                                 ? D_80383010.sns_alpha + 0xC
                                 : 0xFF;
    }
  }
  ...
  if (D_80383010.sns_visible == FALSE) {
    if (D_80383010.sns_alpha > 0) {
      D_80383010.sns_alpha =
          (D_80383010.sns_alpha - 0xC > 0) ? D_80383010.sns_alpha - 0xC : 0;
    }
  }
...
int gcpausemenu_80314B00(void) { return (!D_80383084) ? 1 : 0; }

void gcpausemenu_80314B24(void) { D_80383084 = 0; }
