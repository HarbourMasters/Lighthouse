...
s16 basfx_owSfxList[8] = {SFX_32_BANJO_EGHEE, SFX_34_BANJO_AGHOAA,
                          SFX_35_BANJO_WOAH,  SFX_37_BANJO_OHWW,
                          SFX_38_BANJO_AYE_1, SFX_39_BANJO_AYE_2,
                          SFX_3A_BANJO_HOUW,  SFX_3B_BANJO_GAAH};
  ...
  if (current_time == 0.0f) {
    ...
    while (f20 > 0.0f) {
      if (f20 <= prev_time && current_time < f20) {
        sfxsource_setSfxId(basfx_timeUpSfxSource,
                           (unks0) ? SFX_2A_CLOCK_TIC_1 : SFX_51_CLOCK_TIC_2);
        sfxsource_setSampleRate(basfx_timeUpSfxSource, 0x7fff);
        func_8030E2C4(basfx_timeUpSfxSource);
        return;
      }
...
void basfx_updateSfxSource(void) {
  f32 f12;
  f32 f2;
...
void basfx_playLandingSfx(u8 indx) {
...
void basfx_reset(void) {
  ...
  func_8030DD54(D_8037C6C1, basfx_playLandingSfx);
...
void basfx_update(void) { basfx_updateSfxSource(); }

void basfx_adjustVolume(void) {
...
void basfx_playJumpSfx(f32 arg0, f32 prev_time) {
  basfx_playSfx(basfx_jumpSfxList[basfx_jumpIndex], randf2(0.93f, 1.09f),
                 25000);
  basfx_jumpIndex++;
...
void basfx_playKazooieAuwSfx(void) { func_8030E5F4(SFX_44_KAZOOIE_AUW, 1.117f); }
...
void basfx_playSfxWithParams(u8 indx, enum sfx_e sfx_id, f32 current_time, s32 arg3) {
...
u8 basfx_getSfxSource(void) { return D_8037C6C2; }

void basfx_stopSfxSource(void) { func_8030E3FC(D_8037C6C2); }

void basfx_playSfx(enum sfx_e sfx_id, f32 prev_time, s32 current_time) {
  basfx_playSfxWithParams(D_8037C6C2, sfx_id, prev_time, current_time);
}

void basfx_playSfxWithVolume(enum sfx_e sfx_id, f32 prev_time, s32 current_time) {
  basfx_playSfxWithParams(D_8037C6C3, sfx_id, prev_time, current_time);
}

void basfx_playRustlingSfx(void) {
  if (func_8029CEB0() == 4) {
...
void basfx_playRustlingSfxWithParams(void) {
  if (func_8029CEB0() == 4) {
    FUNC_8030E624(SFX_116_DEAF_RUSTLING, 0.7f, 32000);
  } else {
...
void basfx_playRustlingSfxWithVolume(void) {
  if (func_8029CEB0() == 4) {
...
void basfx_stopOwSfxSource(void) { func_8030E394(basfx_owSfxSource); }

void basfx_stopSfxSource2(void) { func_8030E394(D_8037C6C2); }

void basfx_stopSfxSource3(void) { func_8030E394(D_8037C6C3); }
