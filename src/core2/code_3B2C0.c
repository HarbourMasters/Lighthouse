...
/* .code */
void cameraTransition_init(void) {}

void cameraTransition_reset(void) { s_state = 0; }

void cameraTransition_start(f32 duration) {
  s_duration = duration;
...
void cameraTransition_stop(void) { s_state = 0; }

void cameraTransition_update(f32 target_position[3], f32 target_rotation[3]) {
  f32 tmpf_1[3];
  f32 tmpf_2[3];
  f32 tmp;
