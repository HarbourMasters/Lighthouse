...
File *file_open(enum asset_e asset_id) {
  ...
  this = (File *)heap_malloc(sizeof(File));
...
File *file_openMap(enum map_e map_id) { return file_open(map_id + 0x71C); }
...
File *file_allocNew(void) {
  ...
  this->last_expected = -1;
  this->unk80 = -1;
  this->base_ptr = heap_malloc(FILE_DEFAULT_SIZE);
  this->current_ptr = this->base_ptr;
...
void file_getByte(File *file, u8 *dst) { file_read(file, dst, sizeof(u8)); }
...
void file_getFloat(File *file, f32 *dst) { file_read(file, dst, sizeof(f32)); }
...
void file_getWord(File *file, s32 *dst) { file_read(file, dst, sizeof(s32)); }
...
void file_read(File *file, void *dst, s32 len) {
  ...
  if (file->mode == FILE_MODE_2_FROM_ASSET) {
    ...
    if ((u8 *)file->end_ptr < (u8 *)file->current_ptr + len) {
      ...
      while (((u8 *)file->base_ptr + capacity) <
             ((u8 *)file->current_ptr + len)) {
        capacity *= 2;
      }
    ...
    heap_memcpy(file->current_ptr, dst, len);
    file->current_ptr = (u8 *)file->current_ptr + len;
  }
}

void file_getShort(File *file, s16 *dst) { file_read(file, dst, sizeof(s16)); }
...
bool file_isNextByteExpected(File *file, s32 expected) {
  ...
  if ((file->mode == FILE_MODE_1_UNKNOWN) ||
      (file->mode == FILE_MODE_4_ALLOCATED)) {
