...
extern void updateMumboState(Actor *);
extern void updateMumboAnimation(Actor *);

Actor *chMumbo_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx, Vtx **vtx);
void chMumbo_update(Actor *this);

/* .data */
ActorAnimationInfo mumboAnimationInfo[] = {
    {0, 0.0f},
...
ActorInfo mumboActorInfo = {MARKER_6_MUMBO,
                        ACTOR_7_MUMBO,
                        ...
                        0.0f,
                        0};
s32 mumboSearchStartCube[3] = {0.0f, 0.0f, 0.0f};
f32 mumboParticleEffectScale[3] = {1.0f, 0.0f, 2.0f};
s32 mumboParticleEffectColor[4] = {100, 100, 30, 100};

/* .bss */
u8 currentTransformation;
u8 hasDetransformed;
u8 sHasWarnedBanjoAboutDetransform;
u8 storedDetransformWarning;

/* .code */
static bool chMumbo_actorExists(enum actor_e actorId) {
  f32 var[3];
...
enum transformation_e chMumbo_getTransformation(void) {
  s32 ret = TRANSFORM_2_TERMITE;

  chMumbo_actorExists(0x1F);
  if (chMumbo_actorExists(0x20)) {
    ret = TRANSFORM_3_PUMPKIN;
  }
  if (chMumbo_actorExists(0x21)) {
    ret = TRANSFORM_5_CROC;
  }
  if (chMumbo_actorExists(0x22)) {
    ret = TRANSFORM_4_WALRUS;
  }
  if (chMumbo_actorExists(0x23)) {
    ret = TRANSFORM_6_BEE;
  }
  return ret;
}

void chMumbo_setTransformation(void) {
  currentTransformation = chMumbo_getTransformation();
}

static s32 transformation_getCost(enum transformation_e trans_id) {
  switch (trans_id) {
...
static enum file_progress_e
bkProgId_from_transformationId(enum transformation_e trans_id) {
...
bool chMumbo_withinHorzDistToPlayer(s32 x, s32 z, s32 dist) {
  ...
  return (x - player_position[0]) * (x - player_position[0]) +
             (z - player_position[2]) * (z - player_position[2]) <
         dist * dist;
}

bool chMumbo_isActorNearby(s32 arg0) {
  #ifndef LIGHTHOUSE_P
  s32 search_start_cube[3] = mumboSearchStartCube;
  #else
  s32 search_start_cube[3];
  search_start_cube[0] = mumboSearchStartCube[0];
  search_start_cube[1] = mumboSearchStartCube[1];
  search_start_cube[2] = mumboSearchStartCube[2];
  #endif
  return BOOL(
      cubeList_findNodePropByActorIdAndPosition_s32(arg0, search_start_cube));
}

void chMumbo_playSound(Actor *this) {
  if (this->unk44_31 == 0) {
...
void chMumbo_checkPlayerProximity(Actor *this) {
  ...
  if (player_is_within_range || volatileFlag_get(VOLATILE_FLAG_1) ||
    ...
    if (!fileProgressFlag_get(FILEPROG_11_HAS_MET_MUMBO) ||
        (!fileProgressFlag_get(FILEPROG_DC_HAS_HAD_ENOUGH_TOKENS_BEFORE) &&
         this->unk38_0)) {
      func_8028F918(2);
    }
  }
...
void chMumbo_updateState(Actor *this) {
  actor_loopAnimation(this);
  chMumbo_checkPlayerProximity(this);
}

static void chMumbo_textCallback(ActorMarker *caller, enum asset_e text_id,
                                   s32 arg2) {
  Actor *this;
  ...
  case ASSET_D8F_DIALOG_MUMBO_MEET: // L802D1A04
    gcdialog_showText(
        (this->unk38_0) ? ASSET_DAA_DIALOG_MUMBO_HAS_ENOUGH_TOKENS
                        : ASSET_DAB_DIALOG_MUMBO_NOT_ENOUGH_TOKENS,
        0xe, this->position, this->marker, chMumbo_textCallback, NULL);
    return;

  case ASSET_D90_DIALOG_MUMBO_MAGIC_PAID_FOR: // L802D1A40
    fileProgressFlag_set(FILEPROG_12_HAS_TRANSFORMED_BEFORE, TRUE);
    gcdialog_showText(ASSET_D8F_DIALOG_MUMBO_MEET + currentTransformation, 4, NULL, NULL,
                      NULL, NULL);
  ...
  case ASSET_DB2_DIALOG_MUMBO_MISTAKE_2: // L802D1AF8
    currentTransformation = this->unk10_12;
    this->unk10_12 = 1;
    if (this->unk38_31) {
      comusic_playTrackWithVolumeOverride(COMUSIC_2B_DING_B, 28000);
      item_adjustByDiffWithHud(ITEM_1C_MUMBO_TOKEN, -this->unk38_31);
...
void chMumbo_handleTransformation(Actor *this,
                                  enum transformation_e transform_id) {
  if (this->unk10_12 != 0) {
    gcdialog_showText(
        fileProgressFlag_getN(FILEPROG_BB_MUMBO_MISTAKE_INDEX, 2) +
  ...
  if (fileProgressFlag_get(FILEPROG_12_HAS_TRANSFORMED_BEFORE)) {
    if (this->velocity[0] == 0.0f) {
      gcdialog_showText(transform_id + ASSET_D8F_DIALOG_MUMBO_MEET, 6,
                        this->position, this->marker, chMumbo_textCallback,
                        NULL);
      return;
    }
    if (map_get() == MAP_7A_GL_CRYPT && transform_id == TRANSFORM_3_PUMPKIN &&
        !fileProgressFlag_get(FILEPROG_F7_HAS_TRANSFORMED_IN_CRYPT)) {
      gcdialog_showText(ASSET_DAD_DIALOG_MUMBO_XFORM_IN_CRYPT, 6,
                        this->position, this->marker, chMumbo_textCallback,
                        NULL);
  ...
  gcdialog_showText(ASSET_D90_DIALOG_MUMBO_MAGIC_PAID_FOR, 0xE, this->position,
                    this->marker, chMumbo_textCallback, NULL);
}
...
void chMumbo_update(Actor *this) {
  ...
  if (!this->initialized) {
    ...
    if (chMumbo_isActorNearby(0x201)) {
      this->lifetime_value = 1.0f;
      subaddie_set_state(this, 7U);
    } else if (chMumbo_isActorNearby(0x202)) {
  ...
  if (!this->volatile_initialized) {
    ...
    if (player_getTransformation() == TRANSFORM_1_BANJO &&
        !fileProgressFlag_get(bkProgId_from_transformationId(currentTransformation)) &&
        (map_get() != MAP_7A_GL_CRYPT)) {
      this->unk38_31 = transformation_getCost(currentTransformation);
    }
  ...
  ml_vec3f_rotate_direction(sp4C, 0.0f, this->yaw, 0.0f, 0.0f, 10.0f);
  ...
  switch (this->state) {
  case 1: // L802D1F2C
    this->unk130 = updateMumboAnimation;
    chMumbo_playSound(this);
  ...
  case 2: // L802D1F90
    ...
    if (actor_animationIsAt(this, 0.999f)) {
      if (!fileProgressFlag_get(FILEPROG_11_HAS_MET_MUMBO) &&
        ...
        gcdialog_showText(ASSET_D8F_DIALOG_MUMBO_MEET, 0xE, this->position,
                          this->marker, chMumbo_textCallback, NULL);
      ...
      if (!fileProgressFlag_get(FILEPROG_DC_HAS_HAD_ENOUGH_TOKENS_BEFORE) &&
          ...
          this->unk38_0) {
        subaddie_set_state(this, 3);
        gcdialog_showText(ASSET_DAA_DIALOG_MUMBO_HAS_ENOUGH_TOKENS, 0xE,
                          this->position, this->marker, chMumbo_textCallback,
                          NULL);
        fileProgressFlag_set(FILEPROG_DC_HAS_HAD_ENOUGH_TOKENS_BEFORE, TRUE);
        break;
  ...
  case 4: // L802D20E4
    actor_loopAnimation(this);
    sp48 = (map_get() == MAP_7A_GL_CRYPT)
               ? chMumbo_withinHorzDistToPlayer(0x442, 0, 0x3C)
               : chMumbo_withinHorzDistToPlayer(0, -0x5A, 0x3C);
    if (sp48 && player_movementGroup() == BSGROUP_0_NONE && func_8028F20C() &&
        func_8028EFC8()) {
      controller_copyFaceButtons(0, face_buttons);
      if (face_buttons[FACE_BUTTON(BUTTON_B)] == 1) {
        if (currentTransformation == TRANSFORM_7_WISHWASHY) {
          this->unk38_31 = 0;
        } else if (player_getTransformation() == TRANSFORM_1_BANJO &&
                   !fileProgressFlag_get(
                       bkProgId_from_transformationId(currentTransformation)) &&
                   map_get() != MAP_7A_GL_CRYPT) {
          this->unk38_31 = transformation_getCost(currentTransformation);
        }
        this->unk38_0 = (currentTransformation == TRANSFORM_7_WISHWASHY) ||
                        (item_getCount(ITEM_1C_MUMBO_TOKEN) >= this->unk38_31);
        if (this->unk38_0) {
          ...
          if (sp44 && map_get() != MAP_7A_GL_CRYPT &&
              ...
              randf() < 0.01 && sp48) {
            gcdialog_showText(ASSET_DAE_DIALOG_MUMBO_TREX_START, 6, NULL,
                              this->marker, chMumbo_textCallback, NULL);
            fileProgressFlag_set(FILEPROG_BA_HAS_SEEN_TREX_TEXT, 1);
            this->has_met_before = TRUE;
            subaddie_set_state(this, 3);
          } else if (sp44 && map_get() != MAP_7A_GL_CRYPT && !this->unk138_23 &&
                     (sp40 = fileProgressFlag_getN(
                          FILEPROG_BB_MUMBO_MISTAKE_INDEX, 2),
                      sp40 < 3) &&
                     randf() < 0.05 && sp48) {
            this->unk138_23 = TRUE;
            this->unk10_12 = currentTransformation;
            currentTransformation = 7;
            fileProgressFlag_setN(FILEPROG_BB_MUMBO_MISTAKE_INDEX, ++sp40, 2);
            subaddie_set_state(this, 5);
          } else {
            if (this->unk38_31) {
              comusic_playTrackWithVolumeOverride(SFX_2B_BULL_MOO_1, 28000);
              item_adjustByDiffWithHud(ITEM_1C_MUMBO_TOKEN, -this->unk38_31);
            }
            subaddie_set_state(this, 5);
          }
        ...
        if ((levelSpecificFlags_get(LEVEL_FLAG_3E_UNKNOWN) == FALSE) &&
            (gcdialog_showText(ASSET_DAC_DIALOG_MUMBO_FAIL_TO_BUY, 0, NULL,
                               NULL, NULL, NULL) != 0)) {
  ...
  case 5: // L802D2488
    ...
    if (actor_animationIsAt(this, 0.01f)) {
      ...
      } else if (func_8028FB88(currentTransformation)) {
        if (currentTransformation != TRANSFORM_7_WISHWASHY) {
          if (fileProgressFlag_getAndSet(
                  bkProgId_from_transformationId(currentTransformation), TRUE)) {
    ...
    if (actor_animationIsAt(this, 0.79f)) {
      playMusicWithFade(-1, 1000);
    }
    if (actor_animationIsAt(this, 0.999f)) {
      if (!this->has_met_before) {
        func_8028F918(0);
      }
      comusic_stopTrack(COMUSIC_1D_MUMBO_TRANSFORMATION);
      if (player_getTransformation() != TRANSFORM_1_BANJO) {
        subaddie_set_state(this, 3);
        chMumbo_handleTransformation(this, currentTransformation);
        break;
      }
      if (this->has_met_before) {
        subaddie_set_state(this, 3);
        gcdialog_showText(ASSET_DAF_DIALOG_MUMBO_TREX_MISTAKE, 6, NULL,
                          this->marker, chMumbo_textCallback, NULL);
        break;
      }
      gcpausemenu_80314AC8(1);
  ...
  case 7: // L802D2704
    ...
    if (volatileFlag_get(VOLATILE_FLAG_11) == 0) {
      ...
      if (sp48 != 0) {
        gcdialog_showText(ASSET_DA7_DIALOG_MUMBO_CCW_SUMMER, 7, NULL, NULL,
                          NULL, NULL);
  ...
  case 8: // L802D2790
    ...
    if (volatileFlag_get(VOLATILE_FLAG_12) == 0) {
      ...
      if (sp48 != 0) {
        gcdialog_showText(ASSET_DA8_DIALOG_MUMBO_CCW_AUTUMN, 7, NULL, NULL,
                          NULL, NULL);
...
Actor *chMumbo_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx, Vtx **vtx) {
  ...
  f32 sp44[3];
  #ifndef LIGHTHOUSE_P
  f32 sp38[3] = mumboParticleEffectScale;
  #else
  f32 sp38[3];
  sp38[0] = mumboParticleEffectScale[0];
  sp38[1] = mumboParticleEffectScale[1];
  sp38[2] = mumboParticleEffectScale[2];
  #endif
  ...
  if (marker->unk14_21 && out->state == 8) {
    if (actor_animationIsAt(out, 0.2f) || actor_animationIsAt(out, 0.28f) ||
        actor_animationIsAt(out, 0.36f)) {
      func_8034A174(func_80329934(), 9, sp44);
      spawnParticleEffect(sp44, sp38, mumboParticleEffectColor, 0, 0.9f, 0.0f, 0x46, 0x14, 0);
    }
  }
  return out;
}
...
void chMumbo_detransformWarn(NodeProp *arg0, ActorMarker *arg1) {
  ...
  if (xform == TRANSFORM_1_BANJO || xform == TRANSFORM_7_WISHWASHY ||
      sHasWarnedBanjoAboutDetransform)
    return;

  sHasWarnedBanjoAboutDetransform++;
  if (storedDetransformWarning)
    return;

  gcdialog_showText(
      fileProgressFlag_getAndSet(FILEPROG_83_MAGIC_GET_WEAK_TEXT, TRUE)
          ? ASSET_F5C_DIALOG_MUMBO_MAGIC_GET_WEAK_ABREV
          : ASSET_F5B_DIALOG_MUMBO_MAGIC_GET_WEAK_FULL,
      0xe, NULL, NULL, NULL, NULL);
}
...
void chMumbo_detransformTrigger(NodeProp *arg0, ActorMarker *arg1) {
  ...
  if (xform == TRANSFORM_1_BANJO || xform == TRANSFORM_7_WISHWASHY ||
    ...
    return;
  gcdialog_showText(
      fileProgressFlag_getAndSet(FILEPROG_84_MAGIC_ALL_GONE_TEXT, TRUE)
...
void chMumbo_resetWarnings(void) {
  sHasWarnedBanjoAboutDetransform = storedDetransformWarning = hasDetransformed = 0;
}

void chMumbo_storeWarnings(void) {
  storedDetransformWarning = sHasWarnedBanjoAboutDetransform;
