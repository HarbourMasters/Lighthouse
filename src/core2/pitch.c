...
/* .code */
static void pitch_update(f32 limit, f32 angVel) {
  f32 diff;
  f32 val;
  ...
  pitch_degree =
      (mlAbsF(val) <= mlAbsF(diff)) ? pitch_degree + val : idealPitch_degree;
...
void pitch_update2(void) {
  pitch_update(pitchVelocityLimit_degreePerSec, pitchVelocity_degreePerSec);
}
...
void pitch_set(f32 pitch_deg) { pitch_degree = mlNormalizeAngle(pitch_deg); }

void pitch_applyIdeal(void) { pitch_degree = idealPitch_degree; }

f32 pitch_get(void) { return pitch_degree; }

f32 pitch_getIdeal(void) { return idealPitch_degree; }
