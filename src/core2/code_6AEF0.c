#include "functions.h"
#include "variables.h"
#include <ultra64.h>

//dust_particle.c

/* .bss */
u8 displayListBuffer;

/* .code */
void resetDustParticleEmitter(void) { freeParticleEmitter(displayListBuffer); }

void initializeDustParticleEmitter(void) { displayListBuffer = allocateParticleEmitter(0x10); }

ParticleEmitter *createDustParticleEmitter(f32 *position) {
  static s32 D_80368AA0[3] = {0xC8, 0xC8, 0xE6};

  ParticleEmitter *p_emitter;

  p_emitter = getOrCreateParticleEmitter(displayListBuffer);
  particleEmitter_setPosition(p_emitter, position);
  particleEmitter_setSprite(p_emitter, ASSET_700_SPRITE_DUST);
  particleEmitter_setParticleVelocityRange(p_emitter, -10.0f, 10.0f, -10.0f,
                                           10.0f, 240.0f, 10.0f);
  particleEmitter_setRGB(p_emitter, D_80368AA0);
  particleEmitter_setFade(p_emitter, 0.2f, 0.3f);
  particleEmitter_setStartingScaleRange(p_emitter, 0.2f, 0.4f);
  particleEmitter_setFinalScaleRange(p_emitter, 0.85f, 0.9f);
  particleEmitter_setParticleLifeTimeRange(p_emitter, 0.3f, 0.35f);
  return p_emitter;
}
