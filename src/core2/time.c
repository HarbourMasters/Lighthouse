...
s32 time_getDeltaReal_frames(void) { return s_dTimeReal_frames; }

f32 time_getDelta(void) { return s_dTimeReal_sec * s_timeMultiplier; }

f32 time_func_8033DDB8(void) { return viMgr_getFrameCount() * (1 / 60.0); }

f32 time_getDelta_frames(void) { return time_getDelta() * 30.0; }

void time_setMultiplier(f32 multiplier) { s_timeMultiplier = multiplier; }
