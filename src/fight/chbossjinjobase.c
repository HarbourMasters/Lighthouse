...
ActorInfo chBossJinjoBase = {MARKER_27A_JINJO_STATUE_BASE,
                             ACTOR_3A2_JINJO_STATUE_BASE,
                             ASSET_543_MODEL_JINJO_STATUE_BASE,
                             1,
                             NULL,
                             chBossJinjoBase_update,
...
ParticleScaleAndLifetimeRanges chBossJinjoBase_SmokeScaleAndLifetimeSettings = {
    {1.0f, 1.0f}, {1.7f, 2.7f}, {0.0f, 0.05f}, {2.6f, 3.4f}, 0.0f, 0.1f};

void chbossjinjobase_spawnStoneJinjo(ActorMarker *marker) {
  Actor *actor_bossjinjobase = marker_getActor(marker);
  Actor *childactor_stonejinjo =
      spawn_child_actor(ACTOR_3A1_STONE_JINJO, &actor_bossjinjobase);

  childactor_stonejinjo->unkF4_8 = actor_bossjinjobase->unkF4_8;
  childactor_stonejinjo->position_y =
      childactor_stonejinjo->position_y + 172.0f;
...
void chBossJinjoBase_createSmokeParticles(f32 position[3], s32 count) {
  ...
  particleEmitter_setSpawnPositionRange(pCtrl, -90.0f, 0.0f, -80.0f, 80.0f,
                                        60.0f, 80.0f);
  particleEmitter_setParticleVelocityRange(pCtrl, -170.0f, 0.0f, -170.0f,
                                           170.0f, 100.0f, 170.0f);
  particleEmitter_setScaleAndLifetimeRanges(
      pCtrl, &chBossJinjoBase_SmokeScaleAndLifetimeSettings);
  particleEmitter_emitN(pCtrl, count);
}

void chBossJinjoBase_movePart(Actor *this, s32 arg1, f32 arg2, f32 arg3,
                                   f32 arg4) {
  s32 tmp = func_8034C2C4(this->marker, arg1);

  if (tmp) {
    collisionTri_isHitFromAboveByActor(tmp, arg2, arg3, arg4, 1);
  }
}
...
void chBossJinjoBase_getHitByEgg(ActorMarker *this, ActorMarker *other) {
  ...
  if (actor_bossjinjobase->state !=
      CHBOSSJINJOBASE_STATE_3_SPAWNED_BOSS_JINJO) {
    comusic_playTrack(COMUSIC_2B_DING_B);
    actor_bossjinjobase->unk38_31++;

    if (actor_bossjinjobase->unk38_31 >= 3) {
      subaddie_set_state(actor_bossjinjobase,
                         CHBOSSJINJOBASE_STATE_3_SPAWNED_BOSS_JINJO);
      chstonejinjo_breakOpen(actor_bossjinjobase->unk100);
      chBossJinjoBase_movePart(actor_bossjinjobase, 0x19A, -100.0f, 0.0f,
                                    1.2f);
      playSoundEffectWithPositionAtTime(1.2f, SFX_90_SWITCH_PRESS, 1.0f, 32000,
                    actor_bossjinjobase->position, 1000.0f, 2000.0f);
    }
  }
}
...
void chBossJinjoBase_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    marker_setCollisionScripts(this->marker, NULL, chBossJinjoBase_getHitByEgg,
                               NULL);
    ...
    chBossJinjoBase_movePart(this, 0x19a, 0.0f, -200.0f, 0.0f);
    ...
    spawnQueue_add_4((GenFunction_4)spawnQueue_actor_f32,
                       ACTOR_3AD_JINJONATOR_STATUE_BASE_ACTIVATOR,
                       *(s32 *)&this->unk1C[0], *(s32 *)&this->unk1C[1],
                       *(s32 *)&this->unk1C[2]);
    chBossJinjoBase_createSmokeParticles(this->unk1C, 0x10);
    SPAWNQUEUE_ADD_ONE(chbossjinjobase_spawnStoneJinjo, this->marker);
  } else {
    if (this->state == CHBOSSJINJOBASE_STATE_1_RAISE) {
      ...
      if (this->position_y + y_delta < this->unk1C[1]) {
        ...
        actor_stonejinjo->position_x =
            position_delta[0] + actor_stonejinjo->position_x;
        actor_stonejinjo->position_y =
            position_delta[1] + actor_stonejinjo->position_y;
        actor_stonejinjo->position_z =
            position_delta[2] + actor_stonejinjo->position_z;
      } else {
