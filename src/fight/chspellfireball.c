...
BKCollisionTri *func_80320C94(f32 [3], f32[3], f32, f32[3], s32, u32); // [port] was bool — returns BKCollisionTri*, truncated on 64-bit
...
void chSpellFireball_createExplosionParticles(ActorMarker *marker) {
    ...
    if (actor->state != SPELLFIREBALL_STATE_2_DESPAWN) {
        SFX_PlayFade(SFX_148_GRUNTY_SPELL_LANDING, 1.0f, 32000, actor->position, 1000, 3500);
