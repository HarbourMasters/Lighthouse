...
//should be sns.c, but thats taken by what should be snspayload.c
...
void sns_save_and_update_global_data(void) {
  ...
  if (savedata_8033CA9C(&gSaveData)) {
    ...
    gSaveData.snsw =
        gSaveData.snsw << SNS_NUM_FLAGS >> SNS_NUM_FLAGS ^ gSaveData.snsw;
...
void sns_init_base_payloads(void) {
  snsBasePayloadPtr3 =
      snspayload_init_new_payload((struct SnsPayload *)0x803FFF00);
  snsBasePayloadPtr4 =
      snspayload_init_new_payload((struct SnsPayload *)0x803A5C00);
  snsBasePayloadPtr1 = snspayload_init_new_payload(
      (struct SnsPayload *)heap_allocate_temp_block(0x100));
  snsBasePayloadPtr2 = snspayload_init_new_payload(
      (struct SnsPayload *)heap_allocate_and_free_temp_block(0x100));
}

bool sns_get_or_set_key(bool state, struct SnsPayload *payload, s32 key,
                        s32 mode) {
...
void sns_commit_collected_flags(s32 mode, struct SnsPayload *payload) {
  gSaveData.sns.cEggYellow = sns_get_or_set_key(
      gSaveData.sns.cEggYellow, payload, SNS_ITEM_COLLECTED_EggYellow, mode);
  gSaveData.sns.cEggRed = sns_get_or_set_key(gSaveData.sns.cEggRed, payload,
                                             SNS_ITEM_COLLECTED_EggRed, mode);
  gSaveData.sns.cEggGreen = sns_get_or_set_key(
      gSaveData.sns.cEggGreen, payload, SNS_ITEM_COLLECTED_EggGreen, mode);
  gSaveData.sns.cEggBlue = sns_get_or_set_key(gSaveData.sns.cEggBlue, payload,
                                              SNS_ITEM_COLLECTED_EggBlue, mode);
  gSaveData.sns.cEggPink = sns_get_or_set_key(gSaveData.sns.cEggPink, payload,
                                              SNS_ITEM_COLLECTED_EggPink, mode);
  gSaveData.sns.cEggCyan = sns_get_or_set_key(gSaveData.sns.cEggCyan, payload,
                                              SNS_ITEM_COLLECTED_EggCyan, mode);
  gSaveData.sns.cIceKey = sns_get_or_set_key(gSaveData.sns.cIceKey, payload,
                                             SNS_ITEM_COLLECTED_IceKey, mode);
}

void sns_commit_unlocked_flags(s32 mode, struct SnsPayload *payload) {
  gSaveData.sns.uEggYellow = sns_get_or_set_key(
      gSaveData.sns.uEggYellow, payload, SNS_ITEM_UNLOCKED_EggYellow, mode);
  gSaveData.sns.uEggRed = sns_get_or_set_key(gSaveData.sns.uEggRed, payload,
                                             SNS_ITEM_UNLOCKED_EggRed, mode);
  gSaveData.sns.uEggGreen = sns_get_or_set_key(
      gSaveData.sns.uEggGreen, payload, SNS_ITEM_UNLOCKED_EggGreen, mode);
  gSaveData.sns.uEggBlue = sns_get_or_set_key(gSaveData.sns.uEggBlue, payload,
                                              SNS_ITEM_UNLOCKED_EggBlue, mode);
  gSaveData.sns.uEggPink = sns_get_or_set_key(gSaveData.sns.uEggPink, payload,
                                              SNS_ITEM_UNLOCKED_EggPink, mode);
  gSaveData.sns.uEggCyan = sns_get_or_set_key(gSaveData.sns.uEggCyan, payload,
                                              SNS_ITEM_UNLOCKED_EggCyan, mode);
  gSaveData.sns.uIceKey = sns_get_or_set_key(gSaveData.sns.uIceKey, payload,
                                             SNS_ITEM_UNLOCKED_IceKey, mode);
}
...
void sns_write_payload_over_heap(void) {
  ...
  heap_copyWordMemory(snsBasePayloadPtr3, snsBasePayloadPtr2, sizeof(*snsBasePayloadPtr2));
  heap_copyWordMemory(snsBasePayloadPtr4, snsBasePayloadPtr2, sizeof(*snsBasePayloadPtr2));

  for (i = 1; i < 5; i++) {
    u32 val1 = heap_getNthFreeBlock(&val2, i);
    ...
    if (val1 && val2 > 0x4000) {
      ...
      for (val1 += 0x2C00; val1 < val3; val1 += 0x2000)
        heap_copyWordMemory((void *)val1, snsBasePayloadPtr2, sizeof(*snsBasePayloadPtr2));
    }
  }
}
 ...
 */
bool DEBUG_use_special_bootmap(void) { return FALSE; }

/**
 * Dev flag: purpose unknown?
 */
bool func_8025B818(void) { return TRUE; }

bool sns_get_item_state(enum StopNSwop_Item item, s32 set) {
  switch (item) {
  case SNS_ITEM_EGG_YELLOW:
    return set == SNS_COLLECTED ? gSaveData.sns.cEggYellow
                                : gSaveData.sns.uEggYellow;
  case SNS_ITEM_EGG_RED:
    return set == SNS_COLLECTED ? gSaveData.sns.cEggRed : gSaveData.sns.uEggRed;
  case SNS_ITEM_EGG_GREEN:
    return set == SNS_COLLECTED ? gSaveData.sns.cEggGreen
                                : gSaveData.sns.uEggGreen;
  case SNS_ITEM_EGG_BLUE:
    return set == SNS_COLLECTED ? gSaveData.sns.cEggBlue
                                : gSaveData.sns.uEggBlue;
  case SNS_ITEM_EGG_PINK:
    return set == SNS_COLLECTED ? gSaveData.sns.cEggPink
                                : gSaveData.sns.uEggPink;
  case SNS_ITEM_EGG_CYAN:
    return set == SNS_COLLECTED ? gSaveData.sns.cEggCyan
                                : gSaveData.sns.uEggCyan;
  case SNS_ITEM_ICE_KEY:
    return set == SNS_COLLECTED ? gSaveData.sns.cIceKey : gSaveData.sns.uIceKey;
  default:
    return FALSE;
  }
}
...
void sns_set_item_state(enum StopNSwop_Item item, s32 set, s32 state) {
  if (set == SNS_COLLECTED) {
    switch (item) {
    case SNS_ITEM_EGG_YELLOW:
      gSaveData.sns.cEggYellow = state;
      return;
    case SNS_ITEM_EGG_RED:
      gSaveData.sns.cEggRed = state;
      return;
    case SNS_ITEM_EGG_GREEN:
      gSaveData.sns.cEggGreen = state;
      return;
    case SNS_ITEM_EGG_BLUE:
      gSaveData.sns.cEggBlue = state;
      return;
    case SNS_ITEM_EGG_PINK:
      gSaveData.sns.cEggPink = state;
      return;
    case SNS_ITEM_EGG_CYAN:
      gSaveData.sns.cEggCyan = state;
      return;
    // debug string?
    case SNS_ITEM_ICE_KEY:
      gSaveData.sns.cIceKey = state;
      if (0)
        ;
      return;
    default:
  ...
  } else // SNS_UNLOCKED
  {
    switch (item) {
    case SNS_ITEM_EGG_YELLOW:
      gSaveData.sns.uEggYellow = state;
      return;
    case SNS_ITEM_EGG_RED:
      gSaveData.sns.uEggRed = state;
      return;
    case SNS_ITEM_EGG_GREEN:
      gSaveData.sns.uEggGreen = state;
      return;
    case SNS_ITEM_EGG_BLUE:
      gSaveData.sns.uEggBlue = state;
      return;
    case SNS_ITEM_EGG_PINK:
      gSaveData.sns.uEggPink = state;
      return;
    case SNS_ITEM_EGG_CYAN:
      gSaveData.sns.uEggCyan = state;
      return;
    case SNS_ITEM_ICE_KEY:
      gSaveData.sns.uIceKey = state;
      return;
    default:
      return;
    }
...
void sns_set_item_and_update_payload(enum StopNSwop_Item item, s32 set,
                                     s32 state) {
...
void sns_backup_items_and_unlock_all(void){
    ...
    if (snsToRestoreItems) return;
    sns_write_payload_over_heap();
    snsToRestoreItems = TRUE;
    // statements on same line for match (why?)
    //it is likely due to IDO optimization settings, or even just the original code had this
    //clang-format off
