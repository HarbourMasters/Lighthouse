...
// The round up divide is not technically needed, but will come in handy for
// modding
...
s32 eeprom_writeBlocks(s32 file, s32 offset, void *buffer, s32 count) {
  ...
  pfsManager_setBusyState(3);
  ret = osEepromLongWrite(pfsManager_getFrameReplyQ(), address, buffer,
                          count * EEPROM_BLOCK_SIZE);
...
s32 eeprom_readBlocks(s32 file, s32 offset, void *buffer, s32 count) {
  ...
  pfsManager_setBusyState(3);
  ret = osEepromLongRead(pfsManager_getFrameReplyQ(), address, buffer,
                         count * EEPROM_BLOCK_SIZE);
  pfsManager_setBusyState(0);
