...
s32 osPfsChecker(OSPfs *pfs) {
  ...
  for (j = 0; j < pfs->dir_size; j++) {
    ERRCK(__osContRamRead(pfs->queue, pfs->channel, pfs->dir_table + j,
                          (u8 *)&tmp_dir));
    if (tmp_dir.company_code != 0 && tmp_dir.game_code != 0) {
      ...
      cl = 0;
      bank = 255;
      while (next_page.ipage >= pfs->inodeStartPage &&
      ...
      ERRCK(__osContRamWrite(pfs->queue, pfs->channel, pfs->dir_table + j,
                             (u8 *)&tmp_dir, FALSE));
      fixed++;
    } else {
      if (tmp_dir.company_code == 0 && tmp_dir.game_code == 0)
  ...
  for (j = 0; j < pfs->dir_size; j++) {
    ...
    if (tmp_dir.company_code != 0 && tmp_dir.game_code != 0 &&
        tmp_dir.start_page.ipage >=
            ((__OSInodeUnit *)&(pfs->inodeStartPage) + 1)->ipage) // weird
    {
    ...
    for (j = 0; j < pfs->dir_size; j++) {
      while (file_next_node[j].inode_t.bank == bank &&
             file_next_node[j].ipage >=
                 ((__OSInodeUnit *)&(pfs->inodeStartPage) + 1)->ipage) {
        u8 pp = file_next_node[j].inode_t.page;
        file_next_node[j] = checked_inode.inodePage[pp] =
            tmp_inode.inodePage[pp];
      }
    }
    ...
    for (i = offset; i < ARRLEN(tmp_inode.inodePage); i++) {
      ...
      if (tpage.ipage >= pfs->inodeStartPage && tpage.inode_t.bank != bank) {
        n = (tpage.inode_t.page / 4) +
            ((tpage.inode_t.bank % PFS_ONE_PAGE) * BLOCKSIZE);
