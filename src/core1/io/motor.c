...
s32 osMotorStop(OSPfs *pfs) {
  ...
  if (ret == 0 &&
      ramreadformat.datacrc != 0) //!= __osContDataCrc((u8*)&_motorstopbuf))
  {
...
s32 osMotorStart(OSPfs *pfs) {
  ...
  ret = CHNL_ERR(ramreadformat);
  if (ret == 0 &&
      ramreadformat.datacrc != 0xEB) //__osContDataCrc((u8*)&_motorstartbuf))
  {
    ret = PFS_ERR_CONTRFAIL;
  }
  __osSiRelAccess();
...
static void _MakeMotorData(int channel, u16 address, u8 *buffer,
  ...
  ptr = (u8 *)mdata->ram;
  for (i = 0; i < ARRLEN(mdata->ram); i++)
    mdata->ram[i] = 0;
  mdata->status = CONT_CMD_EXE;
  ...
  ramreadformat.hi = (address << 0x5) | __osContAddressCrc(address);
