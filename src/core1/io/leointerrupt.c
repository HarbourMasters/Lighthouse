...
s32 __osLeoInterrupt() {
  ...
  if (info->cmdType == LEO_CMD_TYPE_1) {
    ...
    info->sectorNum++;
    osEPiRawStartDma(__osDiskHandle, OS_WRITE, LEO_SECTOR_BUFF,
                     blockInfo->dramAddr, blockInfo->sectorSize);
    return 1;
  }
  if (info->cmdType == LEO_CMD_TYPE_0) {
    ...
    if ((bm_stat & LEO_BM_STATUS_C1SINGLE &&
         bm_stat & LEO_BM_STATUS_C1DOUBLE) ||
