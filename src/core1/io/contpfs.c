...
s32 __osRepairPackId(OSPfs *pfs, __OSPackId *badid, __OSPackId *newid) {
  ...
  for (j = 0; j < PFS_MAX_BANKS;) {
    ...
    ERRCK(__osContRamRead(pfs->queue, pfs->channel, 0,
                          (u8 *)&temp)); // TODO: fix magic number
    temp[0] = j | 0x80;
    ...
    ERRCK(__osContRamWrite(pfs->queue, pfs->channel, 0, (u8 *)temp,
                           FALSE)); // oddr 0, don't force
  ...
  for (i = 0; i < ARRLEN(index); i++) {
    ERRCK(__osContRamWrite(pfs->queue, pfs->channel, index[i], (u8 *)newid,
                           TRUE));
  }
...
s32 __osGetId(OSPfs *pfs) {
  ...
  pfs->banks = id->banks;
  pfs->inodeStartPage = pfs->banks * 2 + 3; // TODO: loads of magic
                                              // constants..
  pfs->dir_size = 16;
  pfs->inode_table = 8;
...
s32 __osPfsRWInode(OSPfs *pfs, __OSInode *inode, u8 flag, u8 bank) {
  ...
  if (flag == PFS_WRITE)
    inode->inodePage[0].inode_t.page =
        __osSumcalc((u8 *)&inode->inodePage[offset], (-offset) * 2 + 256);

  for (j = 0; j < 8; j++) {
    addr = ((u8 *)inode->inodePage +
            j * 32); // TODO: don't like this =/ //maybe
                     // &inode->inode_table[j*PFS_ONE_PAGE].ipage or something
    if (flag == PFS_WRITE) {
      ret = __osContRamWrite(pfs->queue, pfs->channel,
                             pfs->inode_table + bank * 8 + j, addr, FALSE);
      ret = __osContRamWrite(pfs->queue, pfs->channel,
                             pfs->minode_table + bank * 8 + j, addr, FALSE);
    } else {
      ret = __osContRamRead(pfs->queue, pfs->channel,
                            pfs->inode_table + bank * 8 + j, addr);
    }
    if (ret != 0)
      return ret;
  }
