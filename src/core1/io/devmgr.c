...
void __osDevMgrMain(void *args) {
  ...
  while (TRUE) {
    ...
    if (mb->piHandle != NULL && mb->piHandle->type == DEVICE_TYPE_64DD &&
      ...
      osEPiRawWriteIo(mb->piHandle, LEO_BM_CTL,
      ...
      while (TRUE) {
        ...
        if (blockInfo->errStatus == LEO_ERROR_29) {
          ...
          osEPiRawWriteIo(mb->piHandle, LEO_BM_CTL,
                          info->bmCtlShadow |
                              LEO_BM_CTL_RESET); // TODO: remove magic constants
          osEPiRawWriteIo(mb->piHandle, LEO_BM_CTL, info->bmCtlShadow);
          osEPiRawReadIo(mb->piHandle, LEO_STATUS, &stat);

          if (stat &
              LEO_STATUS_MECHANIC_INTERRUPT) // TODO: remove magic constants
          {
            osEPiRawWriteIo(mb->piHandle, LEO_BM_CTL,
      ...
      osSendMesg(dm->accessQueue, (OSMesg)NULL, OS_MESG_NOBLOCK);
