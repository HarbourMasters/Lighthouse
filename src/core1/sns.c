...
//should be called snspayload.c IMO
...
bool snspayload_validate(struct SnsPayload *payload) {
  ...
  if ((payload->checksum[0] == checksum[0]) &&
      (payload->checksum[1] == checksum[1]))
    return TRUE;
...
struct SnsPayload *snspayload_find_payload_in_ram(void) {
  ...
  for (payload = (struct SnsPayload *)0x80000000;
       payload < (struct SnsPayload *)0x80400080; payload++)
...
void snspayload_rewind_incoming(void) { snsPayloadInCurrPos = 0; }
...
void snspayload_rewind_outgoing(void) { snsPayloadOutCurrPos = 0; }

void snspayload_append_key_to_outgoing_payload(struct SnsPayload *payload,
                                               s32 key) {
