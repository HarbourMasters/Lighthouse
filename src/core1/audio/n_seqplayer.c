...
static void __n_handleMIDIMsg(N_ALSeqPlayer *seqp,
                              N_ALEvent *event); //__handleMIDIMsg
static void __n_handleMetaMsg(N_ALSeqPlayer *seqp,
                              N_ALEvent *event); //__handleMetaMsg
static void __n_handleNextSeqEvent(N_ALSeqPlayer *seqp);
static void __n_setUsptFromTempo(N_ALSeqPlayer *seqp,
                                 f32 tempo); /* sct 1/8/96 */
...
void n_alSeqpNew(N_ALSeqPlayer *seqp, ALSeqpConfig *c) {
  ...
  seqp->nextEvent.type =
      AL_SEQP_API_EVT; /* start the voice handler "spinning" */
...
ALMicroTime __n_seqpVoiceHandler(void *node) {
  ...
  do {
    ...
    case (AL_SEQP_SEQ_EVT):
      matching_assert(seqp->state != AL_PLAYING, n_seqplayer.c,
                      0x11A); /* Must be done playing to change sequences. */
    ...
    case (AL_SEQP_BANK_EVT):
      matching_assert(seqp->state == AL_STOPPED, n_seqplayer.c,
                      0x123); /* Must be fully stopped to change banks. */

      seqp->bank = seqp->nextEvent.msg.spbank.bank;
      __n_initFromBank(seqp, seqp->bank);
      break;

    /* sct 11/6/95 - these events should now be handled by
     * __n_handleNextSeqEvent */
    case (AL_SEQ_END_EVT):
    case (AL_TEMPO_EVT):
...
static void __n_handleNextSeqEvent(N_ALSeqPlayer *seqp) {
  ...
  default:
    matching_assert(FALSE, n_seqplayer.c,
                    0x162); /* Sequence event type not supported. */
  }
}
...
void __n_handleMIDIMsg(N_ALSeqPlayer *seqp, N_ALEvent *event) {
  ...
  matching_assert(event->type == AL_SEQ_MIDI_EVT ||
  ...
  case (AL_MIDI_NoteOn):
    ...
    if (vel != 0) { /* a real note on */
      ...
      if (inst->vibType) {
        if (seqp->initOsc) {
          deltaTime =
              (*seqp->initOsc)(&oscState, &oscValue, inst->vibType,
      ...
      n_alSynStartVoiceParams(voice, sound->wavetable, pitch, vol, pan, fxmix,
                              deltaTime);
      /*
  ...
  case (AL_MIDI_ProgramChange):
    /* sct 1/16/96 - We must have a valid bank in order to process the program
     * change. */
...
ALSound *__n_lookupSoundQuick(N_ALSeqPlayer *seqp, u8 key, u8 vel, u8 chan) {
  ...
  matching_assert(inst != NULL, n_seqplayer.c,
                  0x3DE); /* sct 10/31/95 - If inst is NULL, then the seqp
                             probably wasn't setup correctly. */
...
s16 __n_vsVol(N_ALVoiceState *vs, N_ALSeqPlayer *seqp) {
  u32 t1, t2;
  t1 = (vs->tremelo * vs->velocity * vs->envGain *
        seqp->chanState[vs->channel].unkA) >>
       13;
...
#define VOICENEEDSNOTEKILL_DEBUG                                               \
  _DEBUG_INTERNAL && 0 /* For debugging voiceNeedsNoteKill routine. */
...
void __n_postNextSeqEvent(N_ALSeqPlayer *seqp) {
  ...
  /* sct 1/5/96 - Do nothing if we're not playing or don't have a target
   * sequence. */
  if ((seqp->state != AL_PLAYING) || (seq == NULL))
    return;

  /* Get the next event time in ticks. */
  /* If false is returned, then there is no next delta (ie. end of sequence
   * reached). */
