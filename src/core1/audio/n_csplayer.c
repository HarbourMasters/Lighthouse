...
void n_alCSPNew(N_ALCSPlayer *seqp, ALSeqpConfig *c) {
  ...
  seqp->nextEvent.type =
      AL_SEQP_API_EVT; /* this will start the voice handler "spinning" */
  ...
  do {
    ...
    case (AL_SEQP_STOPPING_EVT):
      if (seqp->state == AL_PLAYING) {
        ...
        for (vs = seqp->vAllocHead; vs != 0; vs = vs->next) {
          if (__n_voiceNeedsNoteKill((ALSeqPlayer *)seqp, &vs->voice,
                                     KILL_TIME))
    ...
    case (AL_SEQP_SEQ_EVT):
      matching_assert(seqp->state != AL_PLAYING, n_csplayer.c,
                      0x110); /* Must be done playing to change sequences. */
...
void __n_CSPPostNextSeqEvent(ALCSPlayer *seqp) {
   ...
   * reached). */
  if (!cseq_getNextDelta(seqp->target, &deltaTicks))
    return;
...
static void __CSPHandleNextSeqEvent(ALCSPlayer *seqp) {
  ...
  default:
    matching_assert(FALSE, n_csplayer.c,
                    0x161); /* Sequence event type not supported. */
  }
}
...
static void __n_CSPHandleMIDIMsg(N_ALCSPlayer *seqp, ALEvent *event) {
    ...
    if (byte1 == 0x7E) {
      ...
      for (vstate2 = seqp->vAllocHead; vs != 0; vs = vs->next) {
        if (vstate2->channel == byte2)
          __n_seqpReleaseVoice((ALSeqPlayer *)seqp, &vstate2->voice,
                               vstate2->sound->envelope->releaseTime);
      }
      return;
    } else if (byte1 == 0x7F) {
      seqp->chanMask |= 1 << byte2;
  ...
  if ((!((1 << chan) & seqp->chanMask) && (status == 0x90)))
    return;
  switch (status) {
  case (AL_MIDI_NoteOn):
    if (byte2 != 0) /* a real note on */
    {
      ...
      cents = (byte1 - sound->keyMap->keyBase) * 100 + sound->keyMap->detune;
      ...
      vstate->tremelo =
          (u8)oscValue; /* will default if not changed by initOsc */
  ...
  case (AL_MIDI_PolyKeyPressure):
    ...
    n_alSynSetVol(&vstate->voice, __n_vsVol(vstate, (ALSeqPlayer *)seqp),
  ...
  case (AL_MIDI_ControlChange):
        ...
        if ((vs->channel == chan) && (vs->phase != AL_PHASE_RELEASE)) {
          if (byte2 > AL_SUSTAIN) {
            ...
            if (vs->phase == AL_PHASE_SUSTAIN)
              ...
              __n_seqpReleaseVoice((ALSeqPlayer *)seqp, &vs->voice,
    ...
    case 0x76: /* switch 2 */
    case 0x77: /* switch 2 */
      musicTrack_setEvent(seqp->target, byte1, chan);
      break;
      /*                case (AL_MIDI_FX_CTRL_0):
                      ...
                      case (AL_MIDI_FX_CTRL_7):
                          ...
                          if (fxref)
                              alSynSetFXParam(seqp->drvr, fxref, (s16)byte1,
         (void *)byte2); break; case (AL_MIDI_FX3_CTRL): */
    default:
      break;
    }
    break;
