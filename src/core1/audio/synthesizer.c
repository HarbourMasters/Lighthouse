...
Acmd *alAudioFrame(Acmd *cmdList, s32 *cmdLen, s16 *outBuf, s32 outLen) {
  ...
  for (drvr->paramSamples = __nextSampleTime(drvr, &client);
    ...
    client->samplesLeft +=
        _timeToSamplesNoRound(drvr, (*client->handler)(client));
  }

  /* for safety's sake, always store paramSamples aligned to 16 sample boundary.
   * this way, if an voice handler routine gets called outside the
   * ALVoiceHandler routine (alSynAllocVoice) it will get timestamped with an
   * aligned value and will be processed immediately next audio frame.
   */
  drvr->paramSamples &= ~0xf;
