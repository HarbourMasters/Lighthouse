...
int n_alLoadParam(N_PVoice *v, s32 paramID, void *param) {
  ...
  case (AL_FILTER_SET_WAVETABLE):
    ...
    case (AL_ADPCM_WAVE):
       ...
       * frames
       */
      v->dc_table->len = ADPCMFBYTES * ((s32)(v->dc_table->len / ADPCMFBYTES));

      v->dc_bookSize = 2 * v->dc_table->waveInfo.adpcmWave.book->order *
                       v->dc_table->waveInfo.adpcmWave.book->npredictors *
                       ADPCMVSIZE;
      if (v->dc_table->waveInfo.adpcmWave.loop) {
        ...
        alCopy(v->dc_table->waveInfo.adpcmWave.loop->state, v->dc_lstate,
               sizeof(ADPCM_STATE));
      } else {
