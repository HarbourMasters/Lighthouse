...
static Acmd *__n_pullSubFrame(void *filter, s16 *inp, s16 *outp, s32 outCount,
                              Acmd *p); //__n_pullSubFrame
static s16 __n_getRate(f64 vol, f64 tgt, s32 count, u16 *ratel); //envmixer_getRate
  ...
  while (e->em_ctrlList != 0) {
    ...
    case (AL_FILTER_START_VOICE_ALT): {
      ...
      if (param->samples) {
        ...
        e->em_cvolR =
            (e->em_volume * eqpower[EQPOWER_LENGTH - e->em_pan - 1]) >> 15;
      }
    ...
    case (AL_FILTER_SET_VOLUME):
      ...
      if (e->em_delta >= e->em_segEnd) {
         ...
         */
        e->em_cvolL =
            __n_getVol(e->em_cvolL, e->em_delta, e->em_lratm, e->em_lratl);
        e->em_cvolR =
            __n_getVol(e->em_cvolR, e->em_delta, e->em_rratm, e->em_rratl);
      }
...
//clang-format off
static Acmd* __n_pullSubFrame(void *filter, s16 *inp, s16 *outp, s32 outCount, Acmd *p) 
{
  Acmd *ptr = p;
  N_PVoice *e = (N_PVoice *)filter;
  s32 sp68 = 0xb8;
  ...
  if (e->em_first) {
    ...
    e->em_ltgt = (e->em_volume * eqpower[e->em_pan]) >> 15;
    e->em_lratm =
        __n_getRate(e->em_cvolL, e->em_ltgt, e->em_segEnd, &(e->em_lratl));
    e->em_rtgt = (e->em_volume * eqpower[EQPOWER_LENGTH - e->em_pan - 1]) >> 15;
    e->em_rratm =
        __n_getRate(e->em_cvolR, e->em_rtgt, e->em_segEnd, &(e->em_rratl));
	...
	n_aEnvMixer(ptr++, A_INIT | A_NOAUX, e->em_cvolR, osVirtualToPhysical(e->em_state));
      ...
      n_aEnvMixer(ptr++, A_CONTINUE | A_NOAUX, 0,
                  osVirtualToPhysical(e->em_state));
...
static s16 __n_getRate(f64 vol, f64 tgt, s32 count, u16 *ratel) {
  s16 s;
  s16 tmp;
  f64 invn;
  f64 a;
