 ...
 */
static Acmd *envmixer_pullSubFrame(void *filter, s16 *inp, s16 *outp, s32 outCount,
                           s32 sampleOffset, Acmd *p);
static s16 envmixer_getRate(f64 vol, f64 tgt, s32 count, u16 *ratel);

static f32 envmixer_getVol(f32 ivol, s32 samples, s16 ratem, u16 ratel);
...
Acmd *alEnvmixerPull(void *filter, s16 *outp, s32 outCount, s32 sampleOffset,
  ...
  while (e->ctrlList != 0) {
    ...
    case (AL_FILTER_SET_VOLUME):
      ptr = envmixer_pullSubFrame(e, &inp, &loutp, samples, sampleOffset, ptr);
      e->delta += samples;
      if (e->delta >= e->segEnd) {
        /*
         ...
         */
        e->cvolL = envmixer_getVol(e->cvolL, e->delta, e->lratm, e->lratl);
        e->cvolR = envmixer_getVol(e->cvolR, e->delta, e->rratm, e->rratl);
      }
  ...
  if (e->motion == 1) {
    ptr = envmixer_pullSubFrame(e, &inp, &loutp, outCount, sampleOffset, ptr);
    e->delta += outCount;
  }
  /*
...
static Acmd *envmixer_pullSubFrame(void *filter, s16 *inp, s16 *outp, s32 outCount,
  ...
  if (e->first) {
     ...
     */
    e->ltgt = (e->volume * eqpower[e->pan]) >> 15;
    e->lratm = envmixer_getRate((f64)e->cvolL, (f64)e->ltgt, e->segEnd, &(e->lratl));
    e->rtgt = (e->volume * eqpower[EQPOWER_LENGTH - e->pan - 1]) >> 15;
    e->rratm = envmixer_getRate((f64)e->cvolR, (f64)e->rtgt, e->segEnd, &(e->rratl));
    ...
    #ifdef LIGHTHOUSE_P
    aEnvMixer(ptr++, A_INIT | A_AUX, osVirtualToPhysical(e->state), 0, 0, 0, 0, 0, 0, 0);
  } else
    aEnvMixer(ptr++, A_CONTINUE | A_AUX, osVirtualToPhysical(e->state), 0, 0, 0, 0, 0, 0, 0);
    #else
...
f64 envmixer_frexpf(f64 value, s32 *eptr) {
  f64 absvalue;
...
f64 envmixer_ldexpf(f64 in, s32 ex) {
  s32 exp;
...
/*
    envmixer_getRate()  --  This function determines how to go from the
