...
Acmd *_n_loadOutputBuffer(ALFx *r, ALDelay *d, s32 buff,
                          Acmd *p); //_loadOutputBuffer
...
Acmd *n_alFxPull(void) {
  ...
  aMix(ptr++, 0, 0x5a82, N_AL_AUX_R_OUT,
  ...
  aClearBuffer(ptr++, output,
               N_REVERB_OUTCOUNT << 1); /* clear the AL_AUX_R_OUT */
...
Acmd *__n_filterBuffer(ALLowPass *lp, s32 buff, Acmd *p) {
  ...
  t8 = buff >> 8;
  n_aPoleFilter(ptr++, lp->first, lp->fgain, t8,
