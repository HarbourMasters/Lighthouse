...
static Acmd *decodeChunk(Acmd *ptr, ALLoadFilter *f, s32 tsam, s32 nbytes,
...
Acmd *alAdpcmPull(void *filter, s16 *outp, s32 outCount, s32 sampleOffset,
  ...
  if (looped) {
    ...
    ptr = decodeChunk(ptr, f, tsam, nbytes, *outp, inp, f->first);
    ...
    while (outCount > nSam) {
      ...
      nframes = (tsam + ADPCMFSIZE - 1) >> LFSAMPLES;
      nbytes = nframes * ADPCMFBYTES;
      ptr = decodeChunk(ptr, f, tsam, nbytes, op, inp, f->first | A_LOOP);
      /*
  ...
  if ((nOver - (nOver & 0xf)) < outCount) {
    decoded = 1;
    ptr = decodeChunk(ptr, f, nSam - nOver, nbytes, *outp, inp, f->first);
...
Acmd *alRaw16Pull(void *filter, s16 *outp, s32 outCount, s32 sampleOffset,
  ...
  if ((outCount + f->sample > f->loop.end) && (f->loop.count != 0)) {
    ...
    if (nSam > 0) {
      ...
      #ifdef LIGHTHOUSE_P
      aLoadBuffer(ptr++, dramLoc - dramAlign, 0, 0);
      #else
...
Acmd *decodeChunk(Acmd *ptr, ALLoadFilter *f, s32 tsam, s32 nbytes, s16 outp,
                   s16 inp, u32 flags)
