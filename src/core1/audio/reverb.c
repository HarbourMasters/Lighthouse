...
Acmd *alFxPull(void *filter, s16 *outp, s32 outCount, s32 sampleOffset,
  ...
  aMix(ptr++, 0, 0x5a82, AL_AUX_R_OUT,
       input); /* mix the AuxL and AuxR into the AuxL */
...
#define LENGTH (f->delay[s].output - f->delay[s].input)
  ...
  case LPFILT_PARAM:
    if (f->delay[s].lp) {
      ...
      initLowPassFilter(f->delay[s].lp);
    }
    break;
  }
  return 0;
...
Acmd *_loadOutputBuffer(ALFx *r, ALDelay *d, s32 buff, s32 incount, Acmd *p) {
  ...
  static f32 blob = 0;
  /*
   * The following section implements the chorus resampling. Modulate where you
   * pull the samples from, since you need varying amounts of samples.
   */
  if (d->rs) {
    length = d->output - d->input;
    delta =
        _doModFunc(d, incount); /* get the number of samples to modulate by */
    /*
    ...
    delta =
        (s32)(delta * UNITY_PITCH); /* quantize to value microcode will use */
    delta = delta / UNITY_PITCH;
    fratio =
        1.0 - delta; /* pitch ratio needs to be centered around 1, not zero */

    /* d->rs->delta is the difference between the fractional and integer value
     * of the samples needed. fratio * incount + rs->delta gives the number of
     * samples needed for this frame.
     */
    fincount = d->rs->delta + (fratio * (f32)incount);
    count = (s32)fincount; /* quantize to s32 */
    d->rs->delta =
        fincount - (f32)count; /* calculate the round off and store */
     ...
     * negative of that as an index into the delay buffer. loadBuffer that uses
     * this value then bumps it up if it is below the  delay buffer.
     */
    out_ptr = &r->input[-(d->output - d->rsdelta)];
    ramalign = ((s32)out_ptr & 0x7) >> 1; /* calculate the number of samples
                                           needed to align the buffer*/
#ifdef _DEBUG
#if 0
        if(length > 0) {
...
Acmd *_loadBuffer(ALFx *r, s16 *curr_ptr, s32 buff, s32 count, Acmd *p) {
  ...
  if (updated_ptr > delay_end) {
    ...
    #ifdef LIGHTHOUSE_P
    aLoadBuffer(ptr++, osVirtualToPhysical(curr_ptr), 0, 0);
    #else
    aLoadBuffer(ptr++, osVirtualToPhysical(curr_ptr));
    #endif
    aSetBuffer(ptr++, 0, buff + (before_end << 1), 0, after_end << 1);
    #ifdef LIGHTHOUSE_P
    aLoadBuffer(ptr++, osVirtualToPhysical(r->base), 0, 0);
    #else
...
Acmd *_saveBuffer(ALFx *r, s16 *curr_ptr, s32 buff, s32 count, Acmd *p) {
  ...
  if (updated_ptr > delay_end) { /* if the data wraps past end of r->base */
    ...
    #ifdef LIGHTHOUSE_P
    aSaveBuffer(ptr++, osVirtualToPhysical(curr_ptr), 0, 0);
    #else
    aSaveBuffer(ptr++, osVirtualToPhysical(curr_ptr));
    #endif
    aSetBuffer(ptr++, 0, 0, buff + (before_end << 1), after_end << 1);
    #ifdef LIGHTHOUSE_P
    aSaveBuffer(ptr++, osVirtualToPhysical(r->base), 0, 0);
    #else
