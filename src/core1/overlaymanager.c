...
#define SEGMENT_ENTRY(segname, realname)                                       \
  {                                                                            \
    #realname, segname##_VRAM, segname##_VRAM_END, segname##_ROM_START,        \
        segname##_ROM_END, segname##_TEXT_START, segname##_TEXT_END,           \
        segname##_DATA_START, segname##_RODATA_END, segname##_BSS_START,       \
        segname##_BSS_END                                                      \
  }
...
#define DUMMY_SEGMENT_ENTRY(segname, realname)                                 \
  {                                                                            \
    #realname, segname##_VRAM, segname##_VRAM_END, segname##_ROM_START,        \
        segname##_ROM_END, NULL, NULL, NULL, NULL, NULL, NULL                  \
  }

/* .data */
static OverlayAddressMap overlayAddressMap[] = {
    SEGMENT_ENTRY(core2, gs),        DUMMY_SEGMENT_ENTRY(emptyLvl, coshow),
    SEGMENT_ENTRY(CC, whale),        SEGMENT_ENTRY(MMM, haunted),
    SEGMENT_ENTRY(GV, desert),       SEGMENT_ENTRY(TTC, beach),
    SEGMENT_ENTRY(MM, jungle),       SEGMENT_ENTRY(BGS, swamp),
    SEGMENT_ENTRY(RBB, ship),        SEGMENT_ENTRY(FP, snow),
    SEGMENT_ENTRY(CCW, tree),        SEGMENT_ENTRY(SM, training),
    SEGMENT_ENTRY(cutscenes, intro), SEGMENT_ENTRY(lair, witch),
    SEGMENT_ENTRY(fight, battle),
};
static s32 overlayCount =
    sizeof(overlayAddressMap) / sizeof(overlayAddressMap[0]);
...
/* .code */
OverlayAddressMap *overlayManagergetLargetOverlayAddressMap(void) {
  ...
  for (i = 1; i < overlayCount; i++) {
    if (largest_overlay->ram_end - largest_overlay->ram_start <
        (u32)(overlayAddressMap[i].ram_end - overlayAddressMap[i].ram_start)) {
...
s32 overlayManager_returnZero(void) { return 0; }

s32 overlayManager_calculateHeapOffset(void) {
  int sp24;
  OverlayAddressMap *largest_overlay;
  s32 sp1C;
  s32 sp18;

  largest_overlay = overlayManagergetLargetOverlayAddressMap();
  sp18 = heap_stub_return_zero();
  sp1C = overlayManager_returnZero();

  return ((sp1C + (u8 *)gFramebuffers) - largest_overlay->ram_end) + sp18;
}

void overlayManager_adjustHeapSize(void) {
  s32 sp24;
  int sp20;
  int sp1C;
  int heap_size;
  ...
  sp24 = overlayManager_calculateHeapOffset();
  heap_size = heap_getTotalSize();
  sp20 = heap_stub_return_zero();
  sp1C = heap_size - sp20;
...
int overlayManagergetLoadedId(void) { return overlayMgrLoadedId; }
...
bool overlayManagerload(enum overlay_e overlay_id) {
  ...
  overlay_load(overlay_id, ((OverlayAddressMap *)rom_addr)->ram_start,
...
s32 overlayManagerclearLoadedId(void) { overlayMgrLoadedId = 0; }

void overlayManagerloadCore2(void) {
  #ifndef LIGHTHOUSE_P
  overlayManagerclearLoadedId();
  overlay_load(0, core2_VRAM, core2_VRAM_END, core2_ROM_START, core2_ROM_END,
               core2_TEXT_START, core2_TEXT_END, core2_DATA_START,
               core2_RODATA_END, core2_BSS_START, core2_BSS_END);
  overlayManager_adjustHeapSize();
  #endif
}
