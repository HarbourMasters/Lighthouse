...
/* static */ int inflate_codes(struct huft *tl, struct huft *td, s32 bl,
                               s32 bd) {
   ...
   Huffman tables. */
{
  ...
  for (; i < 288; i++) /* make a complete, but wrong code set */
    l[i] = 8;
  bl = 7;
  huft_build(l, 288, 257, literalCopyLengths, literalExtraBits, &tl, &bl);

  /* set up distance table */
  for (i = 0; i < 30; i++) /* make an incomplete code set */
    l[i] = 5;
  bd = 5;
  huft_build(l, 30, 0, distanceCopyOffsets, distanceExtraBits, &td, &bd);
...
/* static */ int inflate_dynamic(
    void) /* decompress an inflated type 2 (dynamic Huffman codes) block. */
{
  ...
  bl = literalBits;
  huft_build(ll, nl, 257, literalCopyLengths, literalExtraBits, &tl, &bl);
  bd = distanceBits;
  huft_build(ll + nl, nd, 0, distanceCopyOffsets, distanceExtraBits, &td, &bd);
