...
void framebufferdraw_draw_CI4(s32 x, s32 y, BKSprite_s *sprite, s32 frame,
                              s32 alpha_enabled) {
  ...
  // align palette
  for (palette_offset = (s32)(sprite_frame + 1); palette_offset % 8;
       palette_offset++) {
    continue;
  }
          ...
          if ((fb_y >= 0) && (fb_y < gFramebufferHeight)) {
            ...
            if (color1 & 1) {
              ...
              *pxl_ptr = (unsigned long)1;
              palette_offset =
                  (u16 *)(sprite_frame +
                          1); // THIS IS GARBAGE for correct reg alloc
            }
            if (palette[indx2] & 1) {
...
void framebufferdraw_draw_CI8(s32 x, s32 y, BKSprite_s *sprite, s32 frame,
                              s32 alpha_enabled) {
  ...
  for (palette_unaligned = palette; ((s32)palette_unaligned) % 8;
       palette_unaligned++) {
    ;
  }
...
void framebufferdraw_draw_RGBA16(s32 x, s32 y, BKSprite_s *sprite, s32 frame,
                                 bool alpha_enabled) {
...
void framebufferdraw_draw_I4(s32 x, s32 y, BKSprite_s *sprite, s32 frame,
                             bool aplha_enabled) {
...
//clang-format off
void framebufferdraw_draw_IA4(s32 x, s32 y, BKSprite_s *sprite, s32 frame, bool aplha_enabled) {
    BKSpriteFrame *frame_ptr;
    BKSpriteTextureBlock *chunk_ptr;
    s16 *temp_v0;
    u8 *tmem;
...
void framebufferdraw_draw_I8(s32 x, s32 y, BKSprite_s *sprite, s32 frame,
                             s32 alpha_enabled) {
  BKSpriteFrame *frame_ptr;
  BKSpriteTextureBlock *chunk_ptr;
  s16 *pixel_ptr;
        ...
        if (fb_x >= 0) {
          if (fb_x < gFramebufferWidth) {
            ...
            if ((fb_y >= 0) && (fb_y < gFramebufferHeight)) {
              ...
              if (intensity != 0) {
                *pixel_ptr = (intensity << 0xB) | (intensity << 6) |
                             (intensity << 1) | 1;
              } else if (!alpha_enabled) {
                *pixel_ptr = 1;
              }
...
void framebufferdraw_draw_IA8(s32 x, s32 y, BKSprite_s *sprite, s32 frame,
                              bool alpha_enabled) {
  BKSpriteFrame *sprite_frame;
  BKSpriteTextureBlock *chunk_ptr;
  s16 *temp_a1;
  ...
  for (i_chunk = 0; i_chunk < sprite_frame->chunkCnt; i_chunk++) {
    for (var_t2 = (u8 *)(chunk_ptr + 1); (s32)var_t2 % 8; var_t2++)
      ; // align
...
void framebufferdraw_draw_RGBA32(s32 x, s32 y, BKSprite_s *sprite, s32 frame,
                                 s32 alpha_enabled) {
  BKSpriteTextureBlock *chunk_ptr;
  u32 *tmem;
  u32 fb_value;
          ...
          if ((fb_x >= 0) && (fb_x < gFramebufferWidth)) {
            ...
            if ((fb_y >= 0) && (fb_y < gFramebufferHeight)) {
              ...
              if (alpha) { // blend texture with existing pixel color
                red8 = ((_SHIFTR(txtr_value, 24, 8) * alpha) +
                        (((((fb_value >> 11))) * 8) * (0xFF - alpha))) /
                       0xFF;
                green8 = ((_SHIFTR(txtr_value, 16, 8) * alpha) +
                          ((_SHIFTR(fb_value, 6, 5) * 8) * (0xFF - alpha))) /
                         0xFF;
                blue8 = ((_SHIFTR(txtr_value, 8, 8) * alpha) +
                         ((_SHIFTR(fb_value, 1, 5) * 8) * (0xFF - alpha))) /
                        0xFF;

                *pxl_ptr = ((_SHIFTL((red8 >> 3), 11, 5) |
...
} // signature may have more variables passed in
void framebufferdraw_stub2(s32 arg0, s32 arg1, s32 arg2, s32 arg3) {
} // signature may have more variables passed in

void framebufferdraw_drawTexture_CI4(s32 x, s32 y, void *tmem, s32 w, s32 h,
                                     bool alpha_enabled) {
  s32 ix;
  s32 iy;
  s32 icolor;
...
void framebufferdraw_draw(s32 x, s32 y, BKSprite_s *sprite, s32 frame,
                          s32 alpha_enabled) {
  s16 type;
...
// arg4 = alpha enabled?
void framebufferdraw_drawTexture_RGBA16(s32 x, s32 y, s16 *arg2, s32 arg3,
                                        s32 arg4) {
  s32 ix;
  s32 var_t2;
  s32 iy;
  ...
  if (*arg2 == SPRITE_TYPE_CI4) {
    framebufferdraw_drawTexture_CI4(x, y, temp_v0 + 1, temp_v0->w, temp_v0->h,
                                    arg4);
    return;
  }
  // otherwise RGBA16
...
void framebufferdraw_applyMask(s32 x, s32 y, Struct84s *maskList, s32 maskIndex,
                               s32 mX, s32 mY, s32 mW, s32 mH, s32 maskColor,
                               s32 mStride, bool dim, s32 replacementColor) {
  ...
  for (iy = 0; iy < mH; iy += mStride) {
    for (ix = 0; ix < mW; ix++) {
      ...
      if (maskColor != *mask_ptr) {
        ...
        *color_ptr =
            _SHIFTL(C8_TO_C5(C5_TO_C8(RGBA16_R5(*color_ptr)) / 2), 11, 5) |
            _SHIFTL(C8_TO_C5(C5_TO_C8(RGBA16_G5(*color_ptr)) / 2), 6, 5) |
            _SHIFTL(C8_TO_C5(C5_TO_C8(RGBA16_B5(*color_ptr)) / 2), 1, 5) | 1;
      }
...
void framebufferdraw_drawGrid(s32 x, s32 y, s32 arg2, s32 arg3,
                              s32 horz_spacing, s32 vert_spacing) {
  s32 x1;
  s32 y1;
...
void framebufferdraw_setPixel(s32 x, s32 y) {
  if ((x >= 0) && (x < gFramebufferWidth) && (y >= 0) &&
      (y < gFramebufferHeight)) {
    gFramebuffers[sBufferIndex][x + y * gFramebufferWidth] =
        _SHIFTL(sPrimColor_r >> 3, 11, 5) | _SHIFTL(sPrimColor_g >> 3, 6, 5) |
        _SHIFTL(sPrimColor_b >> 3, 1, 5) | _SHIFTL(1, 0, 1);
  }
}
...
void framebufferdraw_drawScaledTexture(s32 x, s32 y, u16 *arg2, s32 arg3,
...
s32 framebufferdraw_getBufferIndex(void) { return sBufferIndex; }
