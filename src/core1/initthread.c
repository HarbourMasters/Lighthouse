...
u8 sInitThreadStack[INIT_THREAD_STACK_SIZE]; // Size based on the previous
                                             // symbol's address
...
void initThread_create(void) {
  osCreateThread(&sInitThread, 1, initThread_entry, NULL,
                 sInitThreadStack + INIT_THREAD_STACK_SIZE, OS_PRIORITY_IDLE);
