...
extern void gu_Perspective(Mtx *, u16 *, f32, f32, f32, f32, f32);
...
void viewport_moveAlongZAxis(f32 offset) {
  ...
  ml_vec3f_rotate_direction(delta_position, sViewportRotation[0],
                            sViewportRotation[1], 0.0f, 0.0f, offset);
...
f32 viewport_getYaw(void) { return sViewportRotation[1]; }
...
void viewport_setRenderViewportAndOrthoMatrix(Gfx **gfx, Mtx **mtx) {
  ...
  guOrtho(*mtx, -(2 * (f32)gFramebufferWidth), (2 * (f32)gFramebufferWidth),
          -(2 * (f32)gFramebufferHeight), (2 * (f32)gFramebufferHeight), 1.0f,
          20.0f, 1.0f);
...
void viewport_setRenderPerspectiveMatrix(Gfx **gfx, Mtx **mtx, f32 near,
                                         f32 far) {
  u16 perspNorm;
  ...
  gu_Perspective(*mtx, &perspNorm, sViewportFOVy, sViewportAspect, near, far,
                0.5f);
  gSPPerspNormalize((*gfx)++, perspNorm);
  gSPMatrix((*gfx)++, OS_PHYSICAL_TO_K0((*mtx)++),
            G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_PROJECTION);
...
void viewport_setRenderPerspectiveMatrixDefaultNearAndFar(Gfx **gfx,
                                                          Mtx **mtx) {
  viewport_setRenderPerspectiveMatrix(gfx, mtx, sViewportNear, sViewportFar);
}
...
f32 viewport_debug3(void) { return sViewportUnused1; }

void viewport_pushFramebufferExtendsToVpStack(void) {
  viewport_pushVpScaleAndTranslation(
      (s32)((f32)gFramebufferWidth / 2), (s32)((f32)gFramebufferHeight / 2),
      (s32)((f32)gFramebufferWidth / 2), (s32)((f32)gFramebufferHeight / 2));
}
...
void viewport_debug4(int arg0) { sViewportUnused2 = arg0; }
...
void viewport_pushVpScaleAndTranslation(s32 scale_x, s32 scale_y,
                                        s32 translation_x, s32 translation_y) {
  ...
  osWritebackDCache(&sViewportStack[sViewportStackIndex],
                    VIEWPORT_STACK_SIZE * sizeof(Vp));
}

void viewport_unused_pushVpScaleAndTranslation(f32 scale_x, f32 scale_y,
                                               f32 translation_x,
                                               f32 translation_y) {
  sViewportStackIndex = (sViewportStackIndex + 1) % 8;
...
void viewport_update(void) {
  ml_vec3f_rotate_direction(sViewportFrustumPlanes[0], sViewportRotation[0],
                            sViewportRotation[1], -89.21774f, 0.0f,
                            45.168514251708984f);
  ml_vec3f_rotate_direction(sViewportFrustumPlanes[1], sViewportRotation[0],
                            sViewportRotation[1], 89.21774f, 0.0f,
                            45.168514251708984f);
  ml_vec3f_rotate_direction(sViewportFrustumPlanes[2], sViewportRotation[0],
                            sViewportRotation[1], 0.0f, 93.9692611694336f,
                            34.20201110839844f);
  ml_vec3f_rotate_direction(sViewportFrustumPlanes[3], sViewportRotation[0],
                            sViewportRotation[1], 0.0f, -93.9692611694336f,
                            34.20201110839844f);
  ...
  sViewportFrustumPlanes[0][3] =
      -(sViewportPosition[0] * sViewportFrustumPlanes[0][0] +
        sViewportPosition[1] * sViewportFrustumPlanes[0][1] +
        sViewportPosition[2] * sViewportFrustumPlanes[0][2]);
  sViewportFrustumPlanes[1][3] =
      -(sViewportPosition[0] * sViewportFrustumPlanes[1][0] +
        sViewportPosition[1] * sViewportFrustumPlanes[1][1] +
        sViewportPosition[2] * sViewportFrustumPlanes[1][2]);
  sViewportFrustumPlanes[2][3] =
      -(sViewportPosition[0] * sViewportFrustumPlanes[2][0] +
        sViewportPosition[1] * sViewportFrustumPlanes[2][1] +
        sViewportPosition[2] * sViewportFrustumPlanes[2][2]);
  sViewportFrustumPlanes[3][3] =
      -(sViewportPosition[0] * sViewportFrustumPlanes[3][0] +
        sViewportPosition[1] * sViewportFrustumPlanes[3][1] +
        sViewportPosition[2] * sViewportFrustumPlanes[3][2]);
...
void viewport_getFrustumPlanes(f32 arg0[4], f32 arg1[4], f32 arg2[4],
                               f32 arg3[4]) {
  s32 i;
...
void viewport_setFrustumPlanes(f32 arg0[4], f32 arg1[4], f32 arg2[4],
                               f32 arg3[4]) {
  s32 i;
...
bool viewport_isBoundingBoxInFrustum(f32 min[3], f32 max[3]) {
  ...
  if (((sViewportFrustumPlanes[0][0] * min[0] +
        sViewportFrustumPlanes[0][1] * min[1] +
        sViewportFrustumPlanes[0][2] * min[2] + sViewportFrustumPlanes[0][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[0][0] * min[0] +
        sViewportFrustumPlanes[0][1] * min[1] +
        sViewportFrustumPlanes[0][2] * max[2] + sViewportFrustumPlanes[0][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[0][0] * min[0] +
        sViewportFrustumPlanes[0][1] * max[1] +
        sViewportFrustumPlanes[0][2] * min[2] + sViewportFrustumPlanes[0][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[0][0] * min[0] +
        sViewportFrustumPlanes[0][1] * max[1] +
        sViewportFrustumPlanes[0][2] * max[2] + sViewportFrustumPlanes[0][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[0][0] * max[0] +
        sViewportFrustumPlanes[0][1] * min[1] +
        sViewportFrustumPlanes[0][2] * min[2] + sViewportFrustumPlanes[0][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[0][0] * max[0] +
  ...
  if (((sViewportFrustumPlanes[1][0] * min[0] +
        sViewportFrustumPlanes[1][1] * min[1] +
        sViewportFrustumPlanes[1][2] * min[2] + sViewportFrustumPlanes[1][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[1][0] * min[0] +
        sViewportFrustumPlanes[1][1] * min[1] +
        sViewportFrustumPlanes[1][2] * max[2] + sViewportFrustumPlanes[1][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[1][0] * min[0] +
        sViewportFrustumPlanes[1][1] * max[1] +
        sViewportFrustumPlanes[1][2] * min[2] + sViewportFrustumPlanes[1][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[1][0] * min[0] +
        sViewportFrustumPlanes[1][1] * max[1] +
        sViewportFrustumPlanes[1][2] * max[2] + sViewportFrustumPlanes[1][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[1][0] * max[0] +
        sViewportFrustumPlanes[1][1] * min[1] +
        sViewportFrustumPlanes[1][2] * min[2] + sViewportFrustumPlanes[1][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[1][0] * max[0] +
  ...
  if (((sViewportFrustumPlanes[2][0] * min[0] +
        sViewportFrustumPlanes[2][1] * min[1] +
        sViewportFrustumPlanes[2][2] * min[2] + sViewportFrustumPlanes[2][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[2][0] * min[0] +
        sViewportFrustumPlanes[2][1] * min[1] +
        sViewportFrustumPlanes[2][2] * max[2] + sViewportFrustumPlanes[2][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[2][0] * min[0] +
        sViewportFrustumPlanes[2][1] * max[1] +
        sViewportFrustumPlanes[2][2] * min[2] + sViewportFrustumPlanes[2][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[2][0] * min[0] +
        sViewportFrustumPlanes[2][1] * max[1] +
        sViewportFrustumPlanes[2][2] * max[2] + sViewportFrustumPlanes[2][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[2][0] * max[0] +
        sViewportFrustumPlanes[2][1] * min[1] +
        sViewportFrustumPlanes[2][2] * min[2] + sViewportFrustumPlanes[2][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[2][0] * max[0] +
  ...
  if (((sViewportFrustumPlanes[3][0] * min[0] +
        sViewportFrustumPlanes[3][1] * min[1] +
        sViewportFrustumPlanes[3][2] * min[2] + sViewportFrustumPlanes[3][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[3][0] * min[0] +
        sViewportFrustumPlanes[3][1] * min[1] +
        sViewportFrustumPlanes[3][2] * max[2] + sViewportFrustumPlanes[3][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[3][0] * min[0] +
        sViewportFrustumPlanes[3][1] * max[1] +
        sViewportFrustumPlanes[3][2] * min[2] + sViewportFrustumPlanes[3][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[3][0] * min[0] +
        sViewportFrustumPlanes[3][1] * max[1] +
        sViewportFrustumPlanes[3][2] * max[2] + sViewportFrustumPlanes[3][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[3][0] * max[0] +
        sViewportFrustumPlanes[3][1] * min[1] +
        sViewportFrustumPlanes[3][2] * min[2] + sViewportFrustumPlanes[3][3]) >=
       0.0f) &&
      ((sViewportFrustumPlanes[3][0] * max[0] +
...
bool viewport_isPointOutsideFrustum_3f(f32 x, f32 y, f32 z) {
  if ((sViewportFrustumPlanes[0][0] * x + sViewportFrustumPlanes[0][1] * y +
           sViewportFrustumPlanes[0][2] * z + sViewportFrustumPlanes[0][3] <=
       0.0f) &&
      (sViewportFrustumPlanes[1][0] * x + sViewportFrustumPlanes[1][1] * y +
           sViewportFrustumPlanes[1][2] * z + sViewportFrustumPlanes[1][3] <=
       0.0f) &&
      (sViewportFrustumPlanes[2][0] * x + sViewportFrustumPlanes[2][1] * y +
           sViewportFrustumPlanes[2][2] * z + sViewportFrustumPlanes[2][3] <=
       0.0f) &&
      (sViewportFrustumPlanes[3][0] * x + sViewportFrustumPlanes[3][1] * y +
           sViewportFrustumPlanes[3][2] * z + sViewportFrustumPlanes[3][3] <=
       0.0f))
    return TRUE;
...
bool viewport_isPointPlane_3f(f32 arg0, f32 arg1, f32 arg2) {
  return ((sViewportFrustumPlanes[2][0] * arg0 +
           sViewportFrustumPlanes[2][1] * arg1 +
           sViewportFrustumPlanes[2][2] * arg2 +
           sViewportFrustumPlanes[2][3]) <= 0.0f);
}

MtxF *viewport_getMatrix(void) { return &sViewportMatrix; }

MtxF *viewport_getDefaultMatrix(void) { return &sViewportDefaultMatrix; }

f32 viewport_getFOVy(void) { return sViewportFOVy; }

void viewport_setFOVy(f32 fovy) { sViewportFOVy = fovy; }

f32 viewport_getAspectRatio(void) { return sViewportAspect; }

f32 viewport_getNear(void) { return sViewportNear; }

f32 viewport_adjustAngleToRight(f32 pos[3], f32 angle) {
  return mlNormalizeAngle((sViewportRotation[1] + angle) + 90.0);
}

f32 viewport_transformCoordinate(f32 x, f32 y, f32 viewport_translation[3],
  ...
  if ((sViewportFOVy != fovy) || (sViewportNear != near)) {
    ...
    scale = (500.0 + sViewportNear) /
            (((gFramebufferHeight / (f32)2) / sinf(fovy_rad)) * cosf(fovy_rad));
    fovy = sViewportFOVy;
    near = sViewportNear;
  }
  ...
  ml_vec3f_pitch_rotate_copy(viewport_translation, viewport_translation,
                             viewport_rotation[0]);
  ml_vec3f_yaw_rotate_copy(viewport_translation, viewport_translation,
                           viewport_rotation[1]);
...
bool viewport_projectPointToScreen(f32 pos[3], f32 *arg1) {
  ...
  temp_f2 = gu_sqrtf((delta[1] * delta[1]) + (delta[2] * delta[2])) *
            sinf(fovy_radians);
  temp_f2_2 = (((f32)gFramebufferWidth) / ((f32)gFramebufferHeight)) * temp_f2;

  arg1[0] =
      (f32)(((delta[0] / temp_f2_2) + 1) * (((f32)gFramebufferWidth) / 2));
  arg1[1] = (f32)((1 - (delta[1] / temp_f2)) * (((f32)gFramebufferHeight) / 2));

  if ((arg1[0] < (-((f32)gFramebufferWidth))) ||
      ((((f32)gFramebufferWidth) * 2) < arg1[0])) {
    return FALSE;
  }

  if ((arg1[1] < (-((f32)gFramebufferHeight))) ||
      ((((f32)gFramebufferHeight) * 2) < arg1[1])) {
    return FALSE;
  }
...
void viewport_backupState(void) {
  ...
  viewport_getFrustumPlanes(
      sViewportBackupFrustumPlanes[0], sViewportBackupFrustumPlanes[1],
      sViewportBackupFrustumPlanes[2], sViewportBackupFrustumPlanes[3]);
