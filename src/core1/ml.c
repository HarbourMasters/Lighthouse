...
void ml_vec3f_sub_and_rotate(f32 vec1[3], f32 rotation[3], f32 vec2[3],
                             f32 dst[3]) {
  f32 tmp[3];
...
f32 ml_func_802560D0(f32 arg0[3], f32 arg1[3], f32 arg2[3]) {
  ...
  sp34 = ((sp24[0] * sp3C[0] + sp24[1] * sp3C[1] + sp24[2] * sp3C[2]) /
          (sp20 * sp38));
  sp30 = (sp34 * sp38) / sp20;
...
void ml_vec3f_project(f32 vec1[3], f32 vec2[3], f32 vec3[3]) {
  f32 tmp = vec2[1] == 0 ? vec1[1] * 100 : vec1[1] / -vec2[1];

  TUPLE_SCALE(vec2, tmp)
  TUPLE_ADD_COPY(vec3, vec1, vec2)
}

f32 ml_vec3f_length_unused(f32 vec[3]) { return LENGTH_VEC3F(vec); }
...
void ml_vec3f_rotate(f32 val1, f32 val2, f32 x, f32 y, f32 z, f32 *dstX,
                   f32 *dstY, f32 *dstZ) {
  f32 tmp;
...
f32 ml_vec2f_angle_between_points(f32 val1, f32 val2) {
  f32 tmp = ml_acosf(ml_f_sin_of_angle_between_points_2D(0, 100, val1, val2));
...
f32 ml_angle_between_points_2D(f32 val1, f32 val2, f32 val3, f32 val4) {
  return ml_vec2f_angle_between_points(val3 - val1, val4 - val2);
}

f32 ml_vec3f_angle_between_points(f32 vec1[3], f32 vec2[3]) {
  return ml_vec2f_angle_between_points(vec2[0] - vec1[0], vec2[2] - vec1[2]);
}

void ml_vec3f_angles_between_points(f32 x1, f32 y1, f32 z1, f32 x2, f32 y2, f32 z2, f32 *o1,
                   f32 *o2) {
  f32 dz;
  f32 dy; // unused
...
void ml_init(void) {
  ...
  // Allocate table
  D_80276CB8 = (u16 *)heap_malloc(10001 * sizeof(u16));
...
f32 ml_sin_half_pi(f32 val) { return sinf(val * BAD_PI / 2); }

f32 ml_smoothstep_twice(f32 val) { return ml_smoothstep(ml_smoothstep(val)); }
...
void ml_defrag(void) {
  ...
  if (!heap_checkMemoryStatus() && D_80276CB8 != NULL)
...
void ml_find_closest_point_on_triangle(f32 dst[3], f32 arg1[3], f32 arg2[3][3]) {
  ...
  ml_vec3f_closest_point_on_line_segment(sp40, arg2[0], arg2[1], arg1);
  ml_vec3f_closest_point_on_line_segment(sp34, arg2[1], arg2[2], arg1);
  ml_vec3f_closest_point_on_line_segment(sp28, arg2[2], arg2[0], arg1);
...
void ml_vec3f_project_onto_plane(f32 dst[3], f32 vec1[3], f32 vec2[3]) {
  ...
  ml_vec3f_project_point_onto_plane(dst, vec1, vec2, tmp3);
}
...
bool ml_vec3f_within_bounds(f32 vec[3], f32 x1, f32 z1, f32 x2, f32 z2) {
  return x1 <= vec[0] && x2 >= vec[0] && z1 <= vec[2] && z2 >= vec[2];
}
...
f32 ml_map_f(f32 val, f32 in_min, f32 in_max, f32 out_min, f32 out_max) {
  ...
  if (in_max != in_min) {
    if (out_min < out_max) {
      result = (((val - in_min) / (in_max - in_min)) * (out_max - out_min)) +
               out_min;
...
f32 ml_mapSmoothstep(f32 a, f32 b, f32 c, f32 d, f32 e) {
  return ml_mapFunction_f(a, b, c, d, e, ml_smoothstep);
}

f32 ml_mapSmoothstepTwice(f32 a, f32 b, f32 c, f32 d, f32 e) {
  return ml_mapFunction_f(a, b, c, d, e, ml_smoothstep_twice);
}

f32 ml_mapAbsRange_f(f32 val, f32 in_min, f32 in_max, f32 out_min,
                     f32 out_max) {
  if (val < 0)
...
bool ml_vec3f_inside_box_f(f32 vec[3], f32 minX, f32 minY, f32 minZ, f32 maxX,
                           f32 maxY, f32 maxZ) {
  return vec[0] > minX && vec[0] < maxX && vec[1] > minY && vec[1] < maxY &&
         vec[2] > minZ && vec[2] < maxZ;
}

bool ml_vec3f_inside_box_vec3f(f32 vec[3], f32 min[3], f32 max[3]) {
  return vec[0] > min[0] && vec[0] < max[0] && vec[1] > min[1] &&
         vec[1] < max[1] && vec[2] > min[2] && vec[2] < max[2];
}

bool ml_vec3w_inside_box_w(s32 vec[3], s32 minX, s32 minY, s32 minZ, s32 maxX,
                           s32 maxY, s32 maxZ) {
  return vec[0] > minX && vec[0] < maxX && vec[1] > minY && vec[1] < maxY &&
         vec[2] > minZ && vec[2] < maxZ;
}
...
f32 ml_sin_deg(f32 angle_deg) { return sinf(angle_deg * BAD_DTOR); }

f32 ml_cos_deg(f32 angle_deg) { return cosf(angle_deg * BAD_DTOR); }
...
f32 ml_max_f(f32 arg0, f32 arg1) { return arg0 > arg1 ? arg0 : arg1; }

f32 ml_min_f(f32 arg0, f32 arg1) { return arg0 < arg1 ? arg0 : arg1; }

int ml_max_w(int arg0, int arg1) { return arg0 > arg1 ? arg0 : arg1; }

int ml_min_w(int arg0, int arg1) { return arg1 > arg0 ? arg0 : arg1; }

f32 mlAbsF(f32 arg0) { return arg0 > 0 ? arg0 : -arg0; }

f32 ml_vec3f_abs_sum_xz(f32 arg0[3]) { return mlAbsF(arg0[0]) + mlAbsF(arg0[2]); }

int ml_abs_w(int arg0) { return arg0 > 0 ? arg0 : -arg0; }
...
void ml_vec3f_rotate_and_project(f32 vec1[3], f32 arg1, f32 vec2[3], f32 *arg3, f32 *arg4,
                   f32 *arg5) {
  f32 t1[3];
  f32 t2[3];
  ...
  *arg5 = ml_f_sin_of_angle_between_points_2D(t2[0], t2[2], t1[0], t1[2]);
...
void ml_vec3f_copy(f32 dst[3], f32 src[3]) { TUPLE_COPY(dst, src) }

void ml_vec3f_diff_copy(f32 dst[3], f32 src1[3], f32 src2[3]) {
  TUPLE_DIFF_COPY(dst, src1, src2)
}

void ml_vec3f_diff(f32 dst[3], f32 src[3]) { TUPLE_DIFF(dst, src) }
...
void ml_vec3f_scale(f32 vec[3], f32 scale) { TUPLE_SCALE(vec, scale) }
...
void ml_vec3w_to_vec3f(f32 dst[3], s32 src[3]) { TUPLE_COPY(dst, src) }

void ml_vec3h_to_vec3f(f32 dst[3], s16 src[3]) { TUPLE_COPY(dst, src) }

void ml_vec3f_to_vec3w(s32 dst[3], f32 src[3]) { TUPLE_COPY(dst, src) }

void ml_vec3f_to_vec3h(s16 dst[3], f32 src[3]) { TUPLE_COPY(dst, src) }
...
void ml_vec3f_adjust_rotation(f32 arg0, f32 arg1[3], f32 arg2[3], f32 arg3) {
  ...
  ml_vec3f_clear(arg2);
  tmp = ml_vec3f_angles_from_vector(arg1, &sp40, &sp44);
  sp44 *= arg3;
  if (tmp) {
    diff = arg0 - sp40;
    if (diff < -180.0f)
...
bool ml_vec3f_point_within_horizontal_distance(f32 vec[3], f32 x, f32 z,
                                               f32 distance) {
  f32 diff[3];
...
bool ml_vec3f_within_horizontal_distance(f32 vec1[3], f32 vec2[3],
                                         f32 distance) {
  f32 diff[3];
...
bool ml_vec3w_within_horizontal_distance(s32 vec1[3], s32 vec2[3],
                                         s32 distance) {
  s32 diff[3];
...
f32 ml_vec3f_project_point_onto_line(f32 dst[3], f32 vec1[3], f32 vec2[3], f32 vec3[3]) {
  ...
  if (mag == 0.0) // f64
  {
    ...
    tmp3 = ml_acosf((ml_vec3f_sin_of_angle_between_vectors(tmp1, tmp2) * mag) / mag);
...
void ml_vec3f_closest_point_on_line_segment(f32 a0[3], f32 a1[3], f32 a2[3], f32 a3[3]) {
  ...
  ml_vec3f_project_point_onto_line(a0, a1, a2, a3);
