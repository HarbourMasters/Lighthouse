...
void piMgr_read(void *vaddr, s32 devaddr, s32 size) {
  ...
  for (i = 0; i < block_cnt; i++) {
    osPiStartDma(&sPiMgrIOMesg, OS_MESG_PRI_NORMAL, OS_READ, devaddr, vaddr,
                 VER_SELECT(0x20000, 0x8000, 0, 0), &sPiMgrReadMesg.queue);
  ...
  osPiStartDma(&sPiMgrIOMesg, OS_MESG_PRI_NORMAL, OS_READ, devaddr, vaddr,
               block_remainder, &sPiMgrReadMesg.queue);
  osRecvMesg(&sPiMgrReadMesg.queue, NULL, 1);
...
void piMgr_init(void) {
  ...
  osCreatePiManager(OS_PRIORITY_PIMGR, &sPiMgrMesgQueue, sPiMgrMesgBuffer,
                    PIMGR_MESGBUFFER_SIZE);
}
