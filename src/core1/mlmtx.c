...
void gu_RotateF(f32 mf[4][4], f32, f32, f32, f32);
f32 func_80263FF0(f32);

#if VERSION == VERSION_USA_1_0
s32 D_80276570 = 0xFF62C2B8; // WHAT IS THIS?
s32 currentControllerState = 0x84D7B4F8; // CORE2_DATA_CRC2
#elif VERSION == VERSION_PAL
s32 D_80276570 = 0xF71CBE76; // WHAT IS THIS?
s32 currentControllerState = 0xA2B6F47C; // CORE2_DATA_CRC2
#endif
...
void mlMtxApply(Mtx *mPtr){
    guMtx_convertF2L_2(s_mtx_stack, mPtr);
}
//clang-format on

void mlMtx_multiply(MtxF *arg0) {
  s32 row;
  s32 col;
  s32 i;
  f32 sum;
...
void mlMtx_multiply_3x3(f32 arg0[3][3]) {
  f32 var_f0;
  s32 var_v0;
  s32 var_v1;
  s32 i;
...
void mlMtxPop(void) { s_mtx_stack--; }
...
void mlMtx_push_multiplied(f32 *l_mtx) {
  ...
  for (i = 0; i < 4; i++, l_mtx += 4) {
    for (j = 0; j < 4; j++) {
      var_a2->mf[i][j] =
          l_mtx[0] * s_mtx_stack->mf[0][j] + l_mtx[1] * s_mtx_stack->mf[1][j] +
          l_mtx[2] * s_mtx_stack->mf[2][j] + l_mtx[3] * s_mtx_stack->mf[3][j];
    }
  }
  s_mtx_stack = var_a2;
}
...
void mlMtx_push_multiplied_2(MtxF *l_mtx, MtxF *r_mtx) {
  ...
  for (row = 0; row < 4; row++, r_mtx = (MtxF *)&r_mtx->mf[1][0]) {
    for (col = 0; col < 4; col++) {
      dst->mf[row][col] = (r_mtx->mf[0][0] * l_mtx->mf[0][col] +
...
void mlMtxRotate(f32 a, f32 x, f32 y, f32 z) {
  gu_RotateF((s_mtx_stack + 1)->mf, a, x, y, z);
  guMtx_CatF((s_mtx_stack + 1)->mf, s_mtx_stack->mf, s_mtx_stack->mf);
}
...
void mlMtx_apply_vec3f(f32 dst[3], f32 src[3]) {
  ...
  for (i = 0; i < 3; i++) {
    dst[i] = sp0[0] * s_mtx_stack->mf[0][i] + sp0[1] * s_mtx_stack->mf[1][i] +
             sp0[2] * s_mtx_stack->mf[2][i] + s_mtx_stack->mf[3][i];
  }
}
...
void mlMtx_apply_vec3f_restricted(f32 dst[3], f32 src[3]) {
  ...
  for (i = 0; i < 3; i++) {
    dst[i] = src[0] * s_mtx_stack->mf[0][i] + src[1] * s_mtx_stack->mf[1][i] +
             src[2] * s_mtx_stack->mf[2][i] + s_mtx_stack->mf[3][i];
  }
}

void mlMtx_apply_f3(f32 dst[3], f32 x, f32 y, f32 z) {
  dst[0] = x * s_mtx_stack->mf[0][0] + y * s_mtx_stack->mf[1][0] +
           z * s_mtx_stack->mf[2][0] + s_mtx_stack->mf[3][0];
  dst[1] = x * s_mtx_stack->mf[0][1] + y * s_mtx_stack->mf[1][1] +
           z * s_mtx_stack->mf[2][1] + s_mtx_stack->mf[3][1];
  dst[2] = x * s_mtx_stack->mf[0][2] + y * s_mtx_stack->mf[1][2] +
           z * s_mtx_stack->mf[2][2] + s_mtx_stack->mf[3][2];
}
...
void mlMtx_apply_vec3s(s16 dst[3], s16 src[3]) {
  ...
  spC[0] = sp0[0] * temp_v0[0][0] + sp0[1] * temp_v0[1][0] +
           sp0[2] * temp_v0[2][0] + temp_v0[3][0];
  spC[1] = sp0[0] * temp_v0[0][1] + sp0[1] * temp_v0[1][1] +
           sp0[2] * temp_v0[2][1] + temp_v0[3][1];
  spC[2] = sp0[0] * temp_v0[0][2] + sp0[1] * temp_v0[1][2] +
           sp0[2] * temp_v0[2][2] + temp_v0[3][2];
...
void mlMtx_apply_to_bounding_box(s32 arg0[3], s32 arg1[3], s32 arg2[3],
                                 s32 arg3[3]) {
  s32 var_t0;
  s32 j;
  f32 sp48[8][3];
...
void mlMtxTranslate(f32 x, f32 y, f32 z) {
  ...
  for (phi_v1 = 0; phi_v1 < 3; phi_v1++) {
    ...
    s_mtx_stack->mf[3][phi_v1] +=
        phi_f18 + phi_f16 + (s_mtx_stack->mf[2][phi_v1] * z);
  }
}
...
void mlMtx_translate_vec3f(f32 x, f32 y, f32 z) {
  ...
  for (var_v1 = 0; var_v1 != 3; var_v1++) {
    s_mtx_stack->mf[3][var_v1] += s_mtx_stack->mf[0][var_v1] * x +
                                 s_mtx_stack->mf[1][var_v1] * y +
                                 s_mtx_stack->mf[2][var_v1] * z;
  }
}

void mlMtx_transform(f32 arg0[3], f32 arg1[3], f32 rotation[3], f32 scale,
                     f32 arg4[3]) {
  f32 sp1C[3];
...
void mlMtx_transform_with_scale(f32 arg0[3], f32 rotation[3], f32 scale,
                                f32 arg3[3]) {
  if (arg0 != NULL)
...
void mlMtx_inverse_transform_with_scale(f32 position[3], f32 rotation[3],
                                        f32 scale, f32 arg3[3]) {
  if (arg3 != NULL)
    mlMtxTranslate(arg3[0], arg3[1], arg3[2]);
...
void mlMtx_translate_inverse_rotate_translate(f32 position[3],
                                              f32 rotation[3]) {
  mlMtxTranslate(position[0], position[1], position[2]);
  mlMtxRotYaw(-rotation[1]);
  mlMtxRotPitch(-rotation[0]);
