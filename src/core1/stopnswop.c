...
{
    // [port] SNS scans N64 RDRAM (0x80000000-0x80400080) for payloads.
    // Non-functional on PC — just zero out the parsed keys.
    s32 i;
...
{
    // [port] SNS (Stop 'N' Swop) writes to hardcoded N64 RDRAM addresses.
    // This N64 cartridge-swap feature is non-functional on PC.
#if 0
