...
// [port] Lair audio continuity — logic lives in port/enhancements/LairContinuity.cpp
extern bool lairContinuity_onTrackSet(u8 slot, s32 newTrackId);
extern void lairContinuity_onTick(void);
extern void lairContinuity_onPreStop(s32 slotIndex);
extern void lairContinuity_onTrackStart(u8 slot, s32 newTrackId);
extern s32  lairContinuity_consumeSeek(void *player);
extern int  lairContinuity_hasPendingSeek(void);

// [port] Aliases for callers that use the old names
s32 lairAudio_consumePendingSeek(ALCSPlayer *player) { return lairContinuity_consumeSeek(player); }
int lairAudio_hasPendingSeek(void) { return lairContinuity_hasPendingSeek(); }
...
void func_8024FA98(u8 arg0, s32 arg1){
    ...
    if(arg1 == sp2C || sp2C == -1){
        ...
        if (sp2C == -1 && arg1 >= 0)
            lairContinuity_onTrackStart(arg0, arg1);
...
void func_8024FC1C(u8 arg0, s32 arg1){
    ...
    if (lairContinuity_onTrackSet(arg0, arg1))
        return;
