...
enum bundle_e mudHut_bundleTypes[6] = {
    BUNDLE_A_BGS_HUT_SHOCKSPRING_PAD, BUNDLE_A_BGS_HUT_SHOCKSPRING_PAD,
    BUNDLE_B_BGS_HUT_MUSIC_NOTE,      BUNDLE_A_BGS_HUT_SHOCKSPRING_PAD,
    BUNDLE_A_BGS_HUT_SHOCKSPRING_PAD, BUNDLE_C_BGS_HUT_JIGGY};

ActorInfo chMudHut = {MARKER_D5_BGS_MUD_HUT,
                      ACTOR_C_MUD_HUT,
...
/* .code section */
void chMudHut_checkPlayerState(void) {
  if ((game_getMode() != GAME_MODE_7_ATTRACT_DEMO) &&
      (1.5 < player_stateTimer_get(STATE_TIMER_2_LONGLEG))) {
    player_stateTimer_set(STATE_TIMER_2_LONGLEG, 1.5);
  }
}

void chMudHut_checkPiRead(void) {
  u32 sp1C;
  osPiReadIo(0xD10, &sp1C);
  if (sp1C = (u16)(sp1C - 0x400)) {
    chMudHut_checkPlayerState();
  }
}
...
void chMudHut_spawnExplosion(ActorMarker *this) {
  ...
  thisActor = marker_getActor(this);
  thisActor = actor_spawnWithYaw_f32(ACTOR_D_WOOD_DEMOLISHED,
                                     thisActor->position, NULL);
  thisActor =
      actor_spawnWithYaw_f32(ACTOR_4D_STEAM_2, thisActor->position, NULL);
  if (this)
    ;
}
  ...
  if (func_80334904() == 2) {
    ...
    case 1:
      ...
      if ((150.0f < diffPos[1]) &&
          ...
          (func_8028F20C()) && (LENGTH_VEC3F(diffPos) < 350.f)) {
        ...
        FUNC_8030E8B4(SFX_5B_HEAVY_STUFF_FALLING, 1.0f, 28000, this->position,
                      0x12C, 0xBB8);
        ...
        spawnQueue_add_1((GenFunction_1)chMudHut_spawnExplosion,
                           reinterpret_cast(s32, this->marker));

        if (tmp < 5) {
          spawnQueue_add_4((GenFunction_4)spawnQueue_bundle_f32,
                             mudHut_bundleTypes[tmp], reinterpret_cast(s32, diffPos[0]),
