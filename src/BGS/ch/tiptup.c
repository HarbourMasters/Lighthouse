...
ActorInfo chTiptup = {MARKER_19A_TIPTUP,
                      ACTOR_27A_TIPTUP,
...
void chTiptup_setState6(ActorMarker *this) {
...
void chTiptup_sfxCorrectHit(void) { comusic_playTrackWithVolumeOverride(COMUSIC_2B_DING_B, 28000); }

void chTiptup_sfxIncorrectHit(void) { comusic_playTrackWithVolumeOverride(COMUSIC_2C_BUZZER, 28000); }
...
void chTiptup_correctHit(ActorMarker *this) {
  ...
  if (volatileFlag_get(VOLATILE_FLAG_2_FF_IN_MINIGAME)) {
    ...
    chTiptup_setState6(thisActor->marker);
  }
}
...
void chTiptup_setState(Actor *this, s32 arg1) {
  ...
  if (this->state == 2) {
    ...
    if (!this->has_met_before) {
      ...
      if (unqPtr->unkA == 0) {
        gcdialog_showText(ASSET_C72_DIALOG_UNKNOWN, 0xe, this->position,
                          this->marker, chTiptup_textEvent, 0);
      } else {
        gcdialog_showText(ASSET_C74_DIALOG_UNKNOWN, 0xf, this->position,
                          this->marker, chTiptup_textEvent, 0);
      }
    } else {
      gcdialog_showText(D_80390854[unqPtr->unkA], 0xf, this->position,
                        this->marker, chTiptup_textEvent, 0);
    }
  }
  if (this->state == 3) {
    ...
    for (; i < D_80390864[unqPtr->unkA]; i++) {
      ...
      while ((rand2 = randi2(1, 7)) == prev_member)
        ;
      timedFunc_set_2(tmpf, (GenFunction_2)chTiptup_activatePattern,
                      reinterpret_cast(s32, this->marker), rand2);
      timedFunc_set_2(tmpf + 0.1, (GenFunction_2)chTiptup_activatePattern,
                      reinterpret_cast(s32, this->marker), 0);
      prev_member = rand2;
    } // L80388B7C

    timed_exitStaticCamera(tmpf += 2.5);
    if (!unqPtr->unkA && !volatileFlag_get(VOLATILE_FLAG_2_FF_IN_MINIGAME)) {
      showDelayedTextAtTime(tmpf + 0.5, 0xc73, 0xe, this->position, this->marker,
                    chTiptup_textEvent, 0);
      unqPtr->unkA = 1;
    }
    setCameraModeAtTime(sp54 = tmpf + 0.6, 0);
    if (volatileFlag_get(VOLATILE_FLAG_2_FF_IN_MINIGAME)) {
      timedFunc_set_1(sp54, (GenFunction_1)chTiptup_activateTimer,
  ...
  if (this->state == 4) {
    ...
    for (j = 0; j < vector_size(unqPtr->unk4); j++) {
      ...
      tmpf += randf2(1.0f, 1.5f);
      timedFunc_set_2(tmpf, (GenFunction_2)chTiptup_activatePattern,
                      reinterpret_cast(s32, this->marker), s1->unk0);
      timedFunc_set_2(tmpf + 0.1, (GenFunction_2)chTiptup_activatePattern,
                      reinterpret_cast(s32, this->marker), 0);
    }
    timed_exitStaticCamera(tmpf += 2.5);
    setCameraModeAtTime(tmpf + 0.6, 0);
    vector_clear(unqPtr->unk4);
    this->state = 0x05;
  } // L80388D8C
  if (this->state == 6) {
    ...
    showDelayedTextAtTime(2.0f, D_8039085C[unqPtr->unkA++], 0xF, this->position,
                  this->marker, chTiptup_textEvent, 0);
  }
}

void chTiptup_onCollision(ActorMarker *arg0, ActorMarker *arg1) {
  func_8028F428(2, arg0);
}

s32 chTiptup_getChoirState(ActorMarker *this) {
  Actor *ptr;
...
void chTiptup_choirHitReaction(ActorMarker *this, s32 arg1) {
  ...
  if ((s32)unqPtr->unk0 >= (s32)vector_size(unqPtr->unk4)) {
    if (!mapSpecificFlags_get(0) &&
        gcdialog_showText(ASSET_C76_DIALOG_UNKNOWN, 0, 0, 0, 0, 0))
      mapSpecificFlags_set(0, 1);
  } else {
    tmp = (Struct_BGS_2270_0s *)vector_at(unqPtr->unk4, unqPtr->unk0);
    if (arg1 == tmp->unk0) {
      timedFunc_set_1(0.5f, (GenFunction_1)chTiptup_correctHit,
                      reinterpret_cast(s32, thisActor->marker));
    } else {
      ...
      if (!mapSpecificFlags_get(1) &&
          ...
          gcdialog_showText(0xc75, 0, NULL, NULL, NULL, NULL))
        mapSpecificFlags_set(1, TRUE);
    }
  }
}

void chTiptup_cleanup(Actor *this) {
  ActorLocal_Tiptup *local;
...
void chTiptup_updateChoirState(ActorMarker *this, s32 *arg1, f32 *arg2) {
  Actor *thisActor;
...
void chTiptup_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    this->marker->actorFreeFunc = chTiptup_cleanup;
    marker_setCollisionScripts(this->marker, chTiptup_onCollision, NULL, NULL);
    ...
    unqPtr->unk9 = 0;
    unqPtr->unkA =
        fileProgressFlag_getN(FILEPROG_0_TIPTUP_MINIGAME_PROGRESS, 2);
    unqPtr->unkB = 0;
    unqPtr->unkC = 0.0f;
    if (volatileFlag_get(VOLATILE_FLAG_2_FF_IN_MINIGAME)) {
      unqPtr->unkA = 3;
    ...
    ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f,
                                player_position, &sp38, &sp34, &sp30);
