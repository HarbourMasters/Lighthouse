...
typedef struct ch_bgs_2270_s{
    u32 unk0;
    bk_vector(Struct_BGS_2270_0s) *unk4; // vector?
    u8  unk8;
    u8  unk9;
    u8  round;
    u8  unkB;
    ...
    if(this->state == CH_TIPTUP_STATE_3_FF_GIVE_SEQUENCE){
        ...
        for(; current_member_index < chTiptupSequenceLength[unqPtr->round]; current_member_index++){
            ...
            timedFunc_set_2(
                ...
                reinterpret_cast(uintptr_t, this->marker),
                rand2);
            timedFunc_set_2(
                sequence_time + 0.1,
                (GenFunction_2)chTiptup_activatePattern,
...
void chTiptup_choirHitReaction(ActorMarker *this, s32 arg1){
    ...
    if((s32)unqPtr->unk0 >= (s32) bk_vector_size(unqPtr->unk4)){
        ...
        if(arg1 == tmp->unk0){
            timedFunc_set_1(0.5f, (GenFunction_1)chTiptup_correctHit, reinterpret_cast(uintptr_t, thisActor->marker));
        }
        else{
