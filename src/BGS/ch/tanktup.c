...
extern void spawnQueue_add_3(GenFunction_3, s32, s32, s32);
...
ActorAnimationInfo BGS_D_80390C20[] = {
    {0, 0.0f}, {0x101, 7.5f}, {0x102, 1.75f}, {0x107, 1.75f}};

ActorInfo chTanktup = {MARKER_6C_TANKTUP,
                       ACTOR_E8_TANKTUP,
                       ASSET_3EE_TANKTUP,
                       0x01,
                       BGS_D_80390C20,
...
/* .code */
// tanktups legs spawn
void spawn_tanktup_leg(ActorMarker *this, s32 arg1,
                       enum chtanktup_leg_e leg_id) {
  Actor *thisActor;
  f32 pad;
  Actor *leg;
  f32 sp18[3];
  ...
  leg = actor_spawnWithYaw_f32(leg_id + ACTOR_E9_TANKTUP_LEG_FL, sp18,
                               (s32)thisActor->yaw);
...
void handle_tanktup_leg_collision(Actor *this) {
  Actor *spawnPtr;
  spawnPtr = actorArray_findClosestActorFromActorId(this->position,
                                                    ACTOR_E8_TANKTUP, -1, 0);
  spawnPtr->tanktup.unk0[this->unk10_12] = 1;
  spawnPtr->tanktup.unk10 = 1;
}

s32 chTanktup_isInState3(s16 *arg0) {
  f32 pos[3];
  Actor *spawnPtr;
  ...
  pos[2] = (f32)arg0[2];
  spawnPtr =
      actorArray_findClosestActorFromActorId(pos, ACTOR_E8_TANKTUP, -1, 0);
  return spawnPtr->state == 3;
}

void chTanktup_exitStaticCamera(ActorMarker *marker, enum asset_e text_id, s32 arg2) {
  timed_exitStaticCamera(0.0f);
}

void chTanktup_handlePosition(Actor *this) {
  f32 sp24[3];
  ...
  if (!this->volatile_initialized) {
    ...
    for (sp44 = 0; sp44 < 4; sp44++) {
      if (local->unk0[sp44] == 0) {
        spawnQueue_add_3((GenFunction_3)spawn_tanktup_leg,
                           *((s32 *)(&this->marker)), local->unk0[sp44], sp44);
      }
    }
  ...
  case 3:
    ...
    if (actor_animationIsAt(this, 0.9f) != 0) {
      ...
      if (jiggyscore_isCollected(JIGGY_26_BGS_TANKTUP) == 0) {
        gcdialog_showText(0xC7F, 0xF, this->position, this->marker,
                          chTanktup_exitStaticCamera, 0);
      } else {
        chTanktup_exitStaticCamera(this->marker, 0xC7F, -1);
      }
    }
    break;
  }
