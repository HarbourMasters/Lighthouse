...
ActorInfo chLongSwampSwitch = {MARKER_F5_BGS_ELEVATED_WALKWAY_SWITCH,
                               ACTOR_14E_BGS_ELEVATED_WALKWAY_SWITCH,
                               ASSET_3F5_BGS_JIGGY_SWITCH,
                               1,
                               chSwampSwitchAnimations,
                               chLongSwampSwitch_update,
                               ...
                               0.0f,
                               0};
ActorInfo chShortSwampSwitch = {MARKER_FD_BGS_MAZE_SWITCH,
                                ACTOR_1FB_BGS_MAZE_SWITCH,
                                ASSET_3F5_BGS_JIGGY_SWITCH,
                                1,
                                chSwampSwitchAnimations,
                                chShortSwampSwitch_update,
                                actor_update_func_80326224,
                                actor_draw,
                                0,
                                0,
...
void chLongSwampSwitch_update(Actor *this) {
  chMudHut_checkPiRead();
  if (mapSpecificFlags_get(7)) {
    mapSpecificFlags_set(7, 0);
    if (!mapSpecificFlags_get(3) || (item_getCount(0) > 0)) {
      setHourglassTimer(45);
    }
  }
  if (!mapSpecificFlags_get(5)) {
    updateActorStateBasedOnFlags(this, 1, 6, 7); // something with switch collision
  }
  ...
  if (this->velocity_x != 0.0f && !mapSpecificFlags_get(3) && func_802BB270()) {
    ...
    setHourglassTimer(45); // adjusts timer
...
void chShortSwampSwitch_update(Actor *this) {
  ...
  if (!mapSpecificFlags_get(9)) {
    updateActorStateBasedOnFlags(this, 0xA, 6, 7);
  }
  ...
  if (this->velocity_x != 0.0f && !mapSpecificFlags_get(0xC) &&
      func_802BB270()) {
    mapSpecificFlags_set(0xC, 1);
    setHourglassTimer(10); // adjusts timer
    item_set(ITEM_6_HOURGLASS, 1);
  } // L8038EFB4
}
