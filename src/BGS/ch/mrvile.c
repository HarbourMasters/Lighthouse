...
/* .data */
ActorInfo mrVile_actorInfo = {MARKER_C8_MR_VILE,
                        ACTOR_13A_MR_VILE,
...
f32 mrVile_speedModifiers[7] = {0.0f, 0.8f, 0.9f, 1.0f, 0.9f, 0.95f, 1.0f};

/* .code */
// adjust_position_based_on_collision
void adjust_vile_position(f32 arg0[3], f32 arg1[3], f32 arg2, f32 arg3,
                          s32 arg4) {
  f32 sp64[3];
  f32 pad60;
  f32 sp54[3];
  ...
  sp2C[1] = sp38[1];
  sp2C[2] = sp38[2];
  if (findCollisionTriWithOffsetAndFlags(sp44, sp38, arg3, sp54, 3, arg4)) {
...
void handle_vile_health_update(ActorMarker *arg0) {
  Actor *this;
  ActorLocal_MrVile *local;

  this = marker_getActor(arg0);
  local = (ActorLocal_MrVile *)&this->local;
  if (chvilegame_getCurrentRound(local->game_marker) >= 3) {
...
void chvile_setAnimation(Actor *this, s32 arg1) {
  ActorLocal_MrVile *local;
  ...
  if (arg1 == 103) {
    ...
    if (this->state == 4) {
      ...
      timedFunc_set_1(0.31f, (GenFunction_1)handle_vile_health_update,
  ...
  if (arg1 == 104) {
    local->unk14 = 1.0f;
    timed_playSfx(randf2(1.2f, 1.3f), SFX_97_BLUBBER_BURPS, randf2(0.8f, 1.1f),
                  randi2(25000, 32000));
  }
  local->unkC = arg1;
}

void chvile_initialize(Actor *this) {
  ActorLocal_MrVile *local;
  ...
  local->unk28[1] = 0.0f;
  local->unk28[2] = 0.0f;
  chvile_setAnimation(this, 101);
}

void chvile_updatePosition(Actor *this) {
  ActorLocal_MrVile *local;
  ...
  if ((local->unkC == 102) || (local->unkC == 103) || (local->unkC == 104)) {
    ...
    ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f, local->target_position,
                  &sp58, &sp54, &sp50);
  ...
  if (this->position[1] > 100.0f) {
    adjust_vile_position(start_position, this->position, 90.0f, 70.0f, 0);
  }
}
...
void chvile_setState(Actor *this, s32 next_state) {
  ...
  if (next_state == 3)
    chvile_setAnimation(this, 102);

  if (next_state == 4)
    chvile_setAnimation(this, 102);

  if (next_state == 5) {
    local->target_position[0] = local->target_position[1] =
        local->target_position[2] = 0.0f;
    chvile_setAnimation(this, 102);
  }
...
Actor *chvile_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx, Vtx **vtx) {
  ...
  local = (ActorLocal_MrVile *)&this->local;
  if ((local->unkC == 104) && (local->unk14 > 0.0f) &&
...
bool chvile_isPlayerClose(ActorMarker *marker) {
...
bool BGS_chvile_isInState1(ActorMarker *marker) {
  Actor *this;
...
void chvile_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    chvile_initialize(this);
    chvile_setState(this, 1);
    return;
  }
  if (local->game_marker == NULL) {
    local->game_marker =
        actorArray_findClosestActorFromActorId(this->position, 0x138, -1, &sp90)
            ->marker;
  }
  player_getPosition(player_position);
  ...
  if (this->state == 2) {
    ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f, player_position, &sp84,
                  &sp80, &sp7C);
  ...
  if (this->state == 3) {
    var_v1 = chvilegame_find_closest_piece(local->game_marker, this->position,
                                           this->yaw, local->target_position) &&
    ...
    if (local->game_marker != NULL) {
      ...
      local->unk10 *= mrVile_speedModifiers[chvilegame_getCurrentRound(local->game_marker)];
    }
    if (chvilegame_getCurrentRound(local->game_marker) < 7) {
      ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f, local->target_position,
                    &sp70, &sp6C, &sp68);
      if (local->unkC == 102) {
        if ((-0.8 < sp68) && (sp68 < 0.8) && (sp70 <= 150.0f) && var_v1) {
          chvile_setAnimation(this, 103);
        }
      }
      if (local->unkC == 103) {
        if (sp70 <= 50.0f) {
          if (chvilegame_cpu_consume_piece(local->game_marker,
                                           local->target_position)) {
            chvile_setAnimation(this, 104);
          } else {
            chvile_setAnimation(this, 102);
          }
  ...
  if (this->state == 4) {
    ...
    local->unk10 = 500.0f;
    if ((local->unkC == 102) &&
        (ml_vec3f_distance(this->position, local->target_position) < 200.0f)) {
      chvile_setAnimation(this, 103);
    }
    if ((local->unkC == 103) &&
  ...
  if ((this->state == 6)) {
    ...
    if ((ml_vec3f_distance(this->position, local->target_position) < 100.0f) ||
      ...
      for (i = 0; i < 10; i++) {
        ...
        if ((i == 0) ||
            (ml_vec3f_distance(player_position, random_position) >
             ml_vec3f_distance(player_position, local->target_position))) {
