...
Actor *chYellowFlibbit_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx,
                            Vtx **vtx);
void chYellowFlibbit_update(Actor *this);

/* .data */
ActorInfo BGS_D_80390AE0 = {MARKER_C5_FLIBBIT_YELLOW,
                            ACTOR_137_FLIBBIT_YELLOW,
                            ASSET_385_MODEL_FLIBBIT_YELLOW,
                            0,
                            NULL,
                            chYellowFlibbit_update,
                            NULL,
                            chYellowFlibbit_draw,
                            0,
                            0,
                            1.0f,
                            0};
...
/* .code */
void chYellowFlibbit_emitSmoke(Actor *this) {
  ...
  particleEmitter_setParticleVelocityRange(temp_s0, -70.0f, 50.0f, -70.0f,
                                           70.0f, 100.0f, 70.0f);
...
//clang-format off
s32 chYellowFlibbit_setPosition(Actor *this, f32 *arg1, bool arg2) {
    ...
    playSoundEffectWithPositionAtTime(0.2f, SFX_3F2_UNKNOWN, randf2(0.7f, 1.3f), randi2(25000, 27000), this->position, 500.0f, 2500.0f);
    return TRUE;
}
//clang-format on

bool chYellowFlibbit_checkPlayerProximity(Actor *this) {
  ...
  ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f,
                              player_position, &sp50, &sp4C, &sp48);
  if (func_80329210(this, &player_position)) {
    if ((-0.7 <= sp48) && (sp48 <= 0.7)) {
      ...
      if (func_80329210(this, sp30)) {
        return chYellowFlibbit_setPosition(this, sp30, 1);
      }
    }
  }
  return FALSE;
...
bool chYellowFlibbit_checkTargetProximity(Actor *this) {
  f32 sp64[3];
  f32 sp60;
  f32 sp5C;
  f32 sp58;
  ...
  ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f, sp64, &sp60,
                              &sp5C, &sp58);
  if ((-0.9 <= sp58) && (sp58 <= 0.9)) {
    ...
    if (func_80329210(this, sp3C) != 0) {
      return chYellowFlibbit_setPosition(this, sp3C, 0);
    }
  }
  return FALSE;
}

s32 chYellowFlibbit_findNewPosition(Actor *this) {
  f32 player_position[3];
  f32 sp80;
  f32 sp7C;
  f32 sp78;
  ...
  player_getPosition(player_position);
  ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f,
                              player_position, &sp80, &sp7C, &sp78);
  for (var_s1 = 0; var_s1 != 10; var_s1++) {
    ...
    ml_vec3f_rotate_direction(sp68, 0.0f, var_f2, 0.0f, 0.0f, 210.0f);
    sp68[0] = this->position[0] + sp68[0];
    sp68[1] = this->position[1] + sp68[1];
    sp68[2] = this->position[2] + sp68[2];
    if (func_80329210(this, sp68)) {
      return chYellowFlibbit_setPosition(this, sp68, 0);
    }
  }
  return FALSE;
}

bool chYellowFlibbit_decideNextAction(Actor *this) {
  f32 player_position[3];
  s32 var_v1;

  player_getPosition(player_position);
  if (func_80329210(this, &player_position)) {
    var_v1 = chYellowFlibbit_checkPlayerProximity(this);
    if (var_v1 == 0) {
      if (randf() > 0.5) {
        return FALSE;
      }
    ...
    var_v1 = chYellowFlibbit_checkTargetProximity(this);
  }

  if (var_v1 == 0) {
    var_v1 = chYellowFlibbit_findNewPosition(this);
  }
  return var_v1;
}
...
void chYellowFlibbit_setState(Actor *this, s32 next_state) {
  ...
  if (next_state == 1) {
    if (this->state != 0) {
      chFrogMinigame_incrementCounter3();
      chYellowFlibbit_emitSmoke(this);
    }
    actor_collisionOff(this);
  }
  if (next_state == 2) {
    chFrogMinigame_incrementCounter1();
    chYellowFlibbit_emitSmoke(this);
    local->unk1C = 0.5f;
  }
  if (next_state == 3) {
    skeletalAnim_set(this->unk148, ASSET_FA_ANIM_FLIBBIT_IDLE, 0.2f,
                     randf2(1.0f, 2.0f));
  ...
  if (next_state == 5) {
    ...
    if (!chYellowFlibbit_decideNextAction(this)) {
      if ((this->state != 6)) {
  ...
  if (next_state == 8) {
    FUNC_8030E8B4(SFX_8E_GRUNTLING_DAMAGE, 1.5f, 32200, this->position, 500,
                  2500);
  ...
  if (next_state == 9) {
    ...
    FUNC_8030E8B4(SFX_115_BUZZBOMB_DEATH, 1.0f, 32200, this->position, 500,
                  2500);
    this->marker->collidable = FALSE;
    this->unk10_1 = FALSE;
    chFrogMinigame_incrementCounter2();
    local->unk18 = 1000.0f;
  }
  if (next_state == 0xA) {
...
void chYellowFlibbit_onCollision(ActorMarker *marker,
...
void chYellowFlibbit_onHit(ActorMarker *marker, ActorMarker *other_marker) {
  Actor *this;
...
void chYellowFlibbit_onDeath(ActorMarker *marker, ActorMarker *other_marker) {
  Actor *this;
...
Actor *chYellowFlibbit_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx,
  ...
  if (local->unk0 && ((this->state == 3) || (this->state == 6))) {
    ...
    for (var_s0 = 0; var_s0 < 2; var_s0++) {
      ...
      sp7C[2] = 1.0f;
      boneTransformList_setBoneScale(temp_a0_2, (var_s0 != 0) ? 0x2D : 0x2E,
                                     sp7C);
    };
  }
...
void chYellowFlibbit_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    marker_setCollisionScripts(this->marker, &chYellowFlibbit_onCollision,
                               &chYellowFlibbit_onHit,
                               &chYellowFlibbit_onDeath);
    local->unk2 = 0U;
  ...
  if (!local->unk2) {
    local->unk2 = TRUE;
    chFrogMinigame_incrementCounter4();
  }
  ...
  if (ml_timer_update(&local->unk28, sp9C) != 0) {
    func_8030E878(0x3F0, randf2(0.9f, 1.1f), randi2(12000, 19000),
                  this->position, 500.0f, 2500.0f);
    local->unk28 = randf2(1.0f, 6.0f);
  }
  local->unk20[0] += sp9C;
  local->unk20[0] =
      (0.2 < (f64)local->unk20[0]) ? randf2(-3.0f, -1.0f) : local->unk20[0];

  local->unk20[1] += sp9C;
  local->unk20[1] =
      (0.2 < (f64)local->unk20[1]) ? randf2(-3.0f, -1.0f) : local->unk20[1];
  ...
  if (this->state == 1) {
    ...
    if (spA4 < 2000.0f) {
      ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f, spB4,
                                  &sp98, &sp94, &sp90);
  ...
  if (this->state == 5) {
    ...
    if (sp8C < 0.8 && 0.8 <= sp88) {
      func_8030E878(SFX_8_BANJO_LANDING_04, randf2(0.8f, 0.9f),
                    randi2(0x61A8, 0x6978), this->position, 100.0f, 1500.0f);
    }
    if (skeletalAnim_getLoopCount(this->unk148) > 0) {
      ...
      if ((0.2 <= sp88) && (sp88 <= 0.8)) {
        ...
        this->position_x =
            local->unk4[0] + (local->unkA[0] - local->unk4[0]) * temp_f12;
        this->position_y =
            local->unk4[1] + (local->unkA[1] - local->unk4[1]) * temp_f12;
        this->position_z =
            local->unk4[2] + (local->unkA[2] - local->unk4[2]) * temp_f12;

        if (local->unk1 != 0) {
          if (sp88 <= 0.5) {
            this->position[1] =
                (f32)((f64)this->position[1] + ((sp88 / 0.5) * 70.0));
          } else {
            this->position[1] = (f32)((f64)this->position[1] +
                                      ((1.0 - ((sp88 - 0.5) / 0.5)) * 70.0));
          }
        }
        sp60[0] = (f32)(local->unkA[0] - local->unk4[0]);
        sp60[1] = (f32)(local->unkA[1] - local->unk4[1]);
        sp60[2] = (f32)(local->unkA[2] - local->unk4[2]);
        ml_vec3f_rotate_and_project(BGS_D_80390B10, this->yaw - 90.0f, sp60,
                                    &sp74, &sp70, &sp6C);
  ...
  if (this->state == 6) {
    ml_vec3f_rotate_and_project(this->position, this->yaw - 90.0f, spB4, &sp5C,
                                &sp58, &sp54);
