...
/* .data */
ActorInfo D_80390A40 = {MARKER_C7_YUMBLIE,
                        ACTOR_139_YUMBLIE,
...
/* .code */
bool chyumblie_shouldAppear(Actor *this) {
  ActorLocal_Yumblie *local;
  s32 temp_v0;

  local = (ActorLocal_Yumblie *)&this->local;
  temp_v0 = chvilegame_getCurrentRound(local->game_marker);
...
void chyumblie_set_state(Actor *this, enum chyumblie_state_e next_state) {
  ...
  if (next_state == 2) {
    ...
    s0->unk4 = chyumblie_shouldAppear(this);
    chvilegame_new_piece(s0->game_marker, this->marker, this->position,
                         s0->unk4);
    skeletalAnim_set(this->unk148,
                     (s0->unk4) ? ASSET_128_ANIM_GRUMBLIE_APPEAR
  ...
  if (next_state == 3) {
    ...
    skeletalAnim_set(this->unk148,
                     (s0->unk4) ? ASSET_12A_ANIM_GRUMBLIE_IDLE
                                : ASSET_127_ANIM_YUMBLIE_IDLE,
                     0.1f, randf2(0.5f, 1.0f));
    skeletalAnim_setBehavior(this->unk148, SKELETAL_ANIM_1_LOOP);
    if (s0->unk4) {
      func_8030E6A4(SFX_C4_TWINKLY_MUNCHER_GRR, randf2(1.0f, 1.2), 30000);
    } else {
      func_8030E878(SFX_C3_HEGH, randf2(1.0f, 1.2), 30000, this->position,
                    500.0f, 3000.0f);
    }
  }
  ...
  if (next_state == 4) {
    ...
    skeletalAnim_set(this->unk148,
                     (s0->unk4) ? ASSET_129_ANIM_GRUMBLIE_HIDE
  ...
  if (next_state == 5) {
    ...
    chvilegame_remove_piece(s0->game_marker, this->marker);
    func_8030E878((s0->unk4) ? SFX_C4_TWINKLY_MUNCHER_GRR : SFX_C3_HEGH, 1.4f,
                  32000, this->position, 500.0f, 3000.0f);
  }
...
Actor *chyumblie_draw(ActorMarker *this, Gfx **gfx, Mtx **mtx, Vtx **vtx) {
  ...
  if (sp40->unk4 && sp40->game_marker) {
    modelRender_draw(gfx, mtx, sp44, sp38, 1.0f, NULL,
                     chvilegame_get_grumblie_model(sp40->game_marker));
  } else {
    modelRender_draw(gfx, mtx, sp44, sp38, 1.0f, NULL,
                     marker_loadModelBin(this));
  }
  return thisActor;
}
...
void chyumblie_update(Actor *this) {
  ...
  if (s0->game_marker == NULL) {
    s0->game_marker = actorArray_findClosestActorFromActorId(
                          this->position, ACTOR_138_VILE_GAME_CTRL, -1, &sp48)
                          ->marker;
  }
  ...
  if (this->state == YUMBLIE_STATE_1_UNDER_GROUND) {
    if (ml_timer_update(&s0->unk8, sp4C)) {
      if (mapSpecificFlags_get(6) &&
          (12 > chvilegame_get_piece_count(s0->game_marker))) {
  ...
  if (this->state == YUMBLIE_STATE_2_APPEAR) {
    ...
    if ((tmp < 0.5) && (0.5 <= s0->unk0)) {
      func_8030E878(SFX_C5_TWINKLY_POP, randf2(1.0f, 1.2f), 30000,
                    this->position, 500.0f, 3000.0f);
    }
