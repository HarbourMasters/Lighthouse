...
/* .data section */
u8 leafBoat_alphaPattern[6] = {0, 0, 0, 1, 1, 1};

ActorInfo chLeafBoat = {MARKER_DA_LEAF_BOAT,
                        ACTOR_F1_LEAF_BOAT,
                        ASSET_30D_MODEL_LEAF_BOAT,
                        0x01,
                        NULL,
                        chLeafBoat_update,
                        chLeafBoat_update,
...
void chLeafBoat_onCollision(ActorMarker *this, ActorMarker *other) { this->unk3E_1 = 1; }
...
void chLeafBoat_update(Actor *this) {
  ...
  ActorLocal_Leafboat *local;
    ...
    int i;
    for (i = 0; i < 6; i++) {
      tmp[i] = leafBoat_alphaPattern[i];
    }
  #else
    u8 tmp[6] = leafBoat_alphaPattern;
  #endif
  ...
  if (!this->initialized) {
    ...
    marker_setCollisionScripts(this->marker, chLeafBoat_onCollision, NULL, NULL);
  ...
  if (func_80294660() == 0x100 && func_8028F20C() && this->marker->unk3E_1) {
  ...
  local->unk18[1] =
      10 * sinf((((this->lifetime_value * local->unk6C) / 180.0) * BAD_PI));
  this->unk1C[0] =
      4.5 * cosf((((this->lifetime_value * local->unk6C) / 180.0) * BAD_PI));
  this->unk1C[1] =
      2 * sinf((((this->lifetime_value * local->unk6C) / 180.0) * BAD_PI));
