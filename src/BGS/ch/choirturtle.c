...
/* .data */
ActorInfo chChoirTurtleYellow = {MARKER_19B_CHOIR_TURTLE_YELLOW,
                                 ACTOR_27B_CHOIR_TURTLE_YELLOW,
                                 ASSET_3F9_MODEL_CHOIR_TURTLE,
                                 0,
                                 NULL,
                                 chChoirTurtle_update,
                                 NULL,
                                 chChoirTurtle_draw,
                                 0,
                                 0,
                                 1.5f,
                                 0};
ActorInfo chChoirTurtleCyan = {MARKER_19C_CHOIR_TURTLE_CYAN,
                               ...
                               1.5f,
                               0};
ActorInfo chChoirTurtleBlue = {MARKER_19D_CHOIR_TURTLE_BLUE,
                               ...
                               1.5f,
                               0};
ActorInfo chChoirTurtleRed = {MARKER_19E_CHOIR_TURTLE_RED,
                              ...
                              1.5f,
                              0};
ActorInfo chChoirTurtlePink = {MARKER_19F_CHOIR_TURTLE_PINK,
                               ...
                               1.5f,
                               0};
ActorInfo chChoirTurtlePurple = {MARKER_1A0_CHOIR_TURTLE_PURPLE,
...
void chChoirTurtle_returnState(ActorMarker *marker) {
  ...
  func_8030E878(SFX_C5_TWINKLY_POP, randf2(1.0f, 1.2f), 25000, this->position,
                500.0f, 3000.0f);
}
...
void chChoirTurtle_setState(Actor *this, s32 next_state) {
  ...
  if (next_state == 1) {
    skeletalAnim_set(this->unk148, ASSET_12D_ANIM_CHOIR_TURTLE_IDLE, 0.2f,
                     randf2(2.0f, 2.5f));
    skeletalAnim_setProgress(this->unk148, randf2(0.0f, 1.0f));
  }
  if (next_state == 2) {
    skeletalAnim_set(this->unk148, ASSET_12E_ANIM_CHOIR_TURTLE_SING, 0.1f,
                     1.6666666f);
    timed_playSfx(0.8333333, SFX_86_TIPTUP_CHORUS_AH, local->unk8, 0x7FFF);
  }
  if (next_state == 3) {
    func_8030E6A4(SFX_86_TIPTUP_CHORUS_AH, local->unk8, 0x7FFF);
    skeletalAnim_set(this->unk148, ASSET_12F_ANIM_CHOIR_TURTLE_HURT, 0.1f,
                     1.75f);
    skeletalAnim_setCallback_1(this->unk148, 0.9f,
                               (GenFunction_1)chChoirTurtle_returnState,
                               (s32)this->marker);
...
void chChoirTurtle_onHit(ActorMarker *marker, ActorMarker *other_marker) {
  Actor *this;
...
Actor *chChoirTurtle_draw(ActorMarker *marker, Gfx **gfx, Mtx **mtx,
                          Vtx **vtx) {
...
void chChoirTurtle_update(Actor *this) {
  ...
  if (!this->volatile_initialized) {
    ...
    marker_setCollisionScripts(this->marker, NULL, chChoirTurtle_onHit, NULL);
    local->unk0 = 1;
    local->unkC = 0;
  ...
  local->unkC++;
  if (local->unkC == 2) {
    tiptup = actorArray_findClosestActorFromActorId(
        this->position, ACTOR_27A_TIPTUP, -1, &sp30);
    local->tiptup_marker = (tiptup != NULL) ? tiptup->marker : NULL;
    if (local->tiptup_marker != NULL) {
      chTiptup_updateChoirState(local->tiptup_marker, &local->unk0,
                                &local->unk8);
    }
  }
  if (this->state == 1) {
    if (local->tiptup_marker != NULL &&
        (chTiptup_getChoirState(local->tiptup_marker) == local->unk0)) {
